...
| UT-S11-bootstrap-01 | bootstrap=skipped 时 Phase 1~3 不报错 | status 逻辑 | 模块 bootstrap=skipped，Phase 1~3 目录为空 | status | Phase 1~3 输出「已跳过」，不输出错误或未完成 |
| UT-S11-bootstrap-02 | bootstrap=skipped JSON 输出含 bootstrap 字段 | status --format json | 模块 bootstrap=skipped | status --format json | JSON 中 modules[].bootstrap = skipped |
...
| ST-S11-bootstrap-01 | 快速接入状态面板正确显示已跳过 | Step 1→8（skip 分支） | adopt 完成，无活跃提案 | 执行 status | Phase 1~3 显示「文档基线已跳过（快速接入）」 |
