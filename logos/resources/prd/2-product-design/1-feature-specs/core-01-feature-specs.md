...
## 2.38 公共 AI Tool Adapter Registry 与 ZCode 功能规格
...
## 2.39 Qoder 薄 Adapter 与原生插件功能规格
...
## 2.41 TRAE capability BLOCKED 与 non-deployable 功能规格
...
## 2.44.9 OpenLogos 0.14.1 本地全局 patch 候选交付
...
## 2.44.10 Merge Transaction Preflight 与局部 Reopen
...
## 2.46 Merge Transaction 嵌套章节锚同源解析
...
本功能收口两处「写入端各自猜测权威事实形态」造成的缺陷：`logos-project.yaml` 的资源索引补录用正则拼接文本，flow overlay 的 `extends` 用字面量复制内置版本号。两者都以「写者自带一份对权威的猜测」为形态，因此都要求把猜测换成对唯一权威的读取——前者读 YAML 文档树，后者读 loader 版本映射。

职责边界：本功能只改写入端与降级呈现，不改变资源索引的扫描规则与 desc 推断规则，不改变 overlay 的四种操作语义与版本告警的判定条件，也不改变机器通道既有字段结构。

### 2.47.2 资源索引结构化写入

补录必须经 YAML 文档树完成：解析 → 定位或创建 `resource_index` 节点 → 追加条目 → 序列化写回，过程中保留注释与既有键序。禁止在写回路径上出现按 `conventions:` 之类的锚点做文本拼接的分支。

必须覆盖的三种既有形态与期望产出：

| 目标当前形态 | 期望产出 |
|---|---|
| `resource_index: []`（空 flow sequence） | 转为承载条目的 block sequence，条目挂在该键下 |
| `resource_index:`（空 block） | 直接在该键下追加条目 |
| 键缺失 | 创建 `resource_index` 键后追加条目，不影响其它顶层键位置 |

后置判据是**产物可被 CLI 捆绑的 YAML 解析器无异常解析**，而不是「产物包含某段字符串」。既有条目、其它顶层键与注释逐项守恒；已收录路径不重复追加；重复执行不产生键序或注释漂移。写回失败时不留半成品，目标文件字节保持原样。

同一约束适用于 `logos-project.yaml` 的其它程序化写者（合并事务的元数据写入、决策计数器写入）：它们统一消费同一套结构化读写入口，其中决策计数器的插入锚不得再假定 `resource_index` 只以空 block 形态出现。

### 2.47.3 降级可见性与处置归属

`logos-project.yaml` 解析失败后，恢复器只抢救 `modules` / `scenarios` / `deployment_gates`，`resource_index` 等未列入恢复范围的字段被丢弃。此时：

- `status` 与 `next` 的人类可读输出必须打印可见告警，内容至少包含解析状态（`recovered` / `error`）与未被恢复的字段名（至少含 `resource_index`），并指向 `openlogos index` 这一重建入口；
- 机器通道的诊断字段语义与结构不变，人类可读通道与机器通道对同一状态的判断必须一致；
- 健康项目的输出逐字节不变，不得因本功能新增任何常态输出；
- CLI **不自动改写**用户已损坏的文件。降级的处置权归人：原索引条目已随解析失败丢失，任何自动重建都会用规则推断的 desc 覆盖人工撰写的条目且不可回滚。

### 2.47.4 内置模板内容版本的单一权威

内置模板内容版本的唯一权威是 loader 维护的版本映射。所有消费者一律读取该映射：

- overlay 写入端把 `extends` 组装为 `builtin:<lifecycle>@<映射值>`，产物中不得出现字面量版本号；
- 版本不匹配告警的判定端读取同一映射，写入端与判定端因此不可能失同步；
- 方法论规格文档不得复制版本枚举，只描述「唯一来源是 loader 映射」；
- 测试断言引用该映射，不得把某个具体版本号固化进 fixture 或期望值。

自检判据：CLI 新写出的 overlay 经 overlay 解析后 `warnings` 不含版本不匹配告警。

### 2.47.5 存量 overlay 的有条件迁移

同步命令遇到内容版本落后的存量 overlay 时按引用可解析性分流：

- 该 overlay 引用的**全部** node id 在新版本内置模板中仍可解析 → 自动提升 `extends` 至当前映射值并在输出中提示已迁移；用户自定义的 overlay 操作原样保留；
- 存在任一失效 node id → 保持原 `extends` 不变，继续产生版本不匹配告警。

这条判据的作用是把告警重新对准它本来的对象：「真的引用了已删除节点、需要人工复核的老 overlay」。迁移是幂等的，重复同步对已是当前版本的 overlay 为 no-op。

### 2.47.6 兼容、失败与发布边界

- 命令集、phase、classification 与公共 JSON envelope 的字段结构零变化；overlay 四操作语义与版本告警判定条件零变化。
- 资源索引的扫描范围与 desc 推断规则零变化。
- package / plugin / asset identity 统一提升为本地 candidate `0.14.5`；全局 `0.14.4` 作为固定回滚制品，不允许同 semver 不同字节。
- 不执行 npm publish、dist-tag、Git tag、GitHub Release、官网发布或 git push。

### 2.47.7 验收与追溯

- UT：UT-S08-43～46、UT-S09-275～278、UT-S11-75～77。
- ST：ST-S08-30～31、ST-S09-108、ST-S11-44。
- 安装态：SMOKE-core-169。
- 需求：AC-YAMLW-01～08；场景：S08、S09、S11；架构：§三十八。

## 2.48 归档事务只读寻址与 smoke 不适用显式声明

### 2.48.1 功能目标与职责边界

本功能收口两件事：让**已终结变更的事务重新可读**，以及让**环境不具备变成可审计的账本条目**。两者的共同点是——某个真实存在的事实此前没有出口：前者是归档后事务不可寻址，后者是不适用被静默吞掉。

职责边界：只放开读取路径与补写记录，不改变事务状态机、不新增写入口、不改动 smoke 门禁的判定公式，也不改动 `skip` 的既有统计口径（该口径已在 S19 合并生效，本功能只要求实现遵守它）。

### 2.48.2 归档提案的只读寻址

事务身份解析按固定顺序定位提案目录，命中即止：
...
- slug 来源不变：显式 `--slug` 优先，否则读活跃 guard 的 `activeChange`。归档查找只在第 1 步未命中时启用。
- 归档目录按 `<时间戳>-<slug>` 后缀匹配；同一 slug 命中多个归档目录时 fail-closed 并要求显式消歧，不得取第一个。
- 命中归档提案时，只读动作（`status`）正常返回；写动作（`submit-content` / `seal` / `apply` / `recover` / `abort`）一律拒绝，给出稳定 classification，且不触碰事务文件、receipt 与任何 marker。
- **查找逻辑单点**：只存在一个实现，runner、跨仓工具与其它消费方一律调用它，不得复制第二套目录拼接规则；也不得通过改写 guard 指向已归档提案来「绕」出可读性。

### 2.48.3 一次性用例的可重放终态判据

迁移 / 恢复类 smoke 用例（其被测对象是一次性的外部事务）必须以**终态可重放**为判据：

- 判据表述为「该事务处于 completed 且 receipt 与 slot 集合自洽」，而不是「该事务当前处于某个中间相位」；
- 用例必须能寻址到目标事务本身（经 `--slug` 显式定位），而不是依赖它恰好挂在活跃 guard 下；
- 目标事务已完成时走幂等重放：只读核对身份、相位、slot 计数与 receipt 摘要即通过，不重复提交、不重新 seal/apply、不新建事务；
- 禁止把会被消费掉的中间状态（missing slot、collecting 相位）写成通过的必要条件——那会让用例在被测能力正常工作之后反而永久失败。

### 2.48.4 环境不具备的显式声明

runner 判定自身无法执行真实断言时（缺第三方宿主客户端、缺历史候选制品、缺必需 env），必须：

1. 为其**全部** owned 用例写 `status: "skip"` 记录，而不是只写一条或一条不写；
2. 在记录中携带机器可读的不适用原因（缺失项名称，如具体 env 名或制品名），供 `smoke-report.md` 与 JSON 审计；
3. 以成功状态退出——不适用不是失败；
4. **禁止**静默零记录退出，**禁止**以伪造 `pass` 填补覆盖，**禁止**把真实失败降级为 skip。

这不是新语义：S19 已合并的口径规定 runner 对每个用例写 `pass|fail|skip`，且 skip 表示「当前环境缺少部署目标、外部依赖或平台能力」。本节把该要求从「应当」收紧为实现必须遵守的硬约束，并明确「全部 owned 用例」与「携带原因」两点。
...
- 命令集、phase、classification 与公共 JSON envelope 的字段结构零变化。
- 未归档提案的解析行为、事务状态机与 skip 统计口径零回归。
- package / plugin / asset identity 统一提升为本地 candidate `0.14.6`；全局 `0.14.5` 作为固定回滚制品。
- 不执行 npm publish、dist-tag、Git tag、GitHub Release、官网发布或 git push。

### 2.48.7 验收与追溯

- UT：UT-S09-279～282、UT-S19-29～32。
- ST：ST-S09-109、ST-S19-18。
- 安装态：SMOKE-core-170（并复核 SMOKE-core-168）。
- 需求：AC-TXADDR-01～05、AC-SMOKE-NA-01～04；场景：S09、S19；架构：§三十九。

## 2.49 资源索引候选范围与描述推断锚定

### 2.49.1 功能目标与职责边界

本功能修正资源索引的**输入端**：哪些磁盘文件构成候选集合，以及每个候选路径应被描述成什么。前次变更已把索引的**写入端**定案为结构化 AST 写入，本功能不触碰写入方式，也不改变索引条目的结构。

三条缺陷有共同形状：**判据比它应当覆盖的范围更宽或更窄**——候选范围宽到把事务工作区当规格文档，路径判据宽到让嵌套路径冒充权威，而描述规则又窄到认不出 CLI 自己定义的产出物。

### 2.49.2 候选集合的排除规则

`logos/resources/verify/` 的目录语义是**运行产物与报告**，二者层级不同：

| 层级 | 内容 | 是否进候选集合 |
|---|---|---|
| 顶层文件 | `acceptance-report.md`、`smoke-report.md` 等验收/冒烟报告 | ✅ 收录 |
| 子目录 | 事务工作区（`baseline-seed-runs/<run>/{staging,resolved,backup}/`）、证据包（`*-evidence/`）、部署制品（`deployment-artifacts/`） | ❌ 一律排除 |

规则按**结构**划线而非枚举目录名：顶层是报告、子目录是产物。新增的证据目录因此自动被排除，不需要每次再改一次判据。

其余扫描根（`prd/`、`api/`、`database/`、`test/`、`scenario/`、`decisions/`、`implementation/`、`spec/`）的递归行为不变。

**该排除不是新约束**：`logos/spec/baseline-closure.md` 已明文禁止 baseline-seed staging 进入 effective view，而 `resource_index` 正是喂给 AI 的资源视图。本功能是让实现符合既有规范。

### 2.49.3 路径判据的起始锚

全部描述推断规则的路径判据必须从项目根锚定。未锚定时，嵌套路径中内嵌的那段权威路径会整体命中权威规则：

```
logos/resources/verify/baseline-seed-runs/<run>/resolved/logos/resources/prd/3-technical-plan/1-architecture/core-system-map.md
                                                        └────────── 未锚定时这一段整体命中架构规则 ──────────┘
```

后果是快照被贴上与权威文档**逐字相同**的描述，AI 从索引上无法分辨哪个是权威。

因此：**规则只匹配从项目根开始的规范路径**。这条要求独立于 §2.49.2 的排除范围——即便将来某个内部目录被有意放行，也不允许嵌套路径冒充权威文档的描述。归档、备份、临时拷贝同理。

### 2.49.4 kind 与描述规则的对齐

baseline-seed 定义了四个 kind（`system-map`、`scenario-candidates`、`dependency-map`、`entry-points`），其中前两个是必需 kind。描述规则必须能识别每个 kind 的规范产出路径。

场景实现目录此前只认 `SXX-` 命名，而 `scenario-candidates` 按方法论定义就是「尚未分配 SXX 的候选清单」，本就不该有 `SXX-`。因此该目录需要**按目录的兜底规则**：

- `SXX-` 文档：沿用既有的场景时序图描述（不变）；
- 非 `SXX-` 文档：兜底为场景实现目录的语义描述，覆盖该目录下的全部合法 kind，而不是只为单个文件名开洞。

**对齐由测试锚定**：每个 kind 的规范产出路径都必须能推断出非空描述。这条锚是防漂移的机制——kind 集合与描述规则分处两个模块，没有机器判据就会像本次一样再次失配。

### 2.49.5 既有条目不被改写

本功能只改候选集合与描述推断，**不改写任何既有 `resource_index` 条目**。修复后污染不再新增，但此前已被收录的快照条目仍在原处；它们的删除属于对用户正式文档的改写，处置权归人（可手工删除或用 `openlogos index` 重建）。

理由与既有的降级处置一致：CLI 无法区分「污染条目」与「人工特意添加的条目」，自动剔除会删除用户文档中的既有行。

### 2.49.6 兼容、失败与发布边界

- 命令集、公共 JSON envelope 字段结构、索引条目结构与写入方式零变化。
- 其余扫描根的递归行为、`SXX-` 文档的既有描述、幂等与守恒语义零回归。
- package / plugin / asset identity 统一提升为本地 candidate `0.14.7`；全局 `0.14.6` 作为固定回滚制品。
- 不执行 npm publish、dist-tag、Git tag、GitHub Release、官网发布或 git push。
...
## 2.52 SQL delta 的分层校验与能力缺失降级

### 2.52.1 功能目标与职责边界

本功能不放宽任何既有校验，只做三件事：把「结构完整度」与「方言真实性」两层分开；让方言层按本机可用适配器路由；让不可用变成**可留痕的降级**而非交付阻断。

要解决的形状是：**环境能力的缺失被当成了校验失败**。适配器未实现（PostgreSQL / MySQL）与未安装（`sqlite3` 二进制缺失）是同一件事的两种表现，都不是提案的缺陷。

### 2.52.2 两层判据的职责划分

| 层 | 判据 | 与方言的关系 | 不可用时 |
|---|---|---|---|
| 结构完整度 | `CREATE TABLE` / 主键 / 约束 / 索引 / 迁移回滚语义 | **无关**，五项在全部方言下一致生效 | 不存在「不可用」——纯文本判定 |
| 方言真实性 | 语法解析或隔离执行 | 相关，需对应适配器 | 降级为跳过并留痕 |

结构层永远执行。此前的实现已经是这样——报错逐级前进（`缺 CREATE TABLE` → `缺主键` → … → `适配器不可用`）本身就证明了这一点。本功能只是让第二层在不可用时不再阻断。

### 2.52.3 方言路由表

| 方言 | 实际执行层级 | 适配器 | 强度 |
|---|---|---|---|
| SQLite | 结构 + **隔离执行**（`BEGIN` → payload → 计数 → `ROLLBACK`） | `sqlite3` 二进制 | 最强：真执行 |
| PostgreSQL | 结构 + **语法解析** | libpg_query 的 WASM 编译产物 | 权威语法级 |
| MySQL | 结构 | 无 | 仅结构 |
| 任意方言但适配器不可用 | 结构 | 无 | 仅结构 |

**层级不对称是有意的，且必须在输出中可见。** PostgreSQL 得到的是语法级校验而非执行级——它不会发现「引用了不存在的表」这类只有执行才能发现的问题。规格明说这一点，好过让用户以为三种方言得到了同等强度的保证。

**MySQL 为何不接解析器**：现有唯一候选（`node-sql-parser`）实测对合法的 MySQL 分区表与 PG 生成列误拦（9 通过 / 2 误拦），且安装体积 89 MB。用它等于把「完全阻断」换成「随机阻断」——对用户是更糟的体验。等出现不会误拦合法 SQL 的权威解析器再补。
...
1. **降级原因**——适配器未实现，还是未安装；
2. **缺失项**——具体缺什么（方言名 / 二进制名）；
3. **实际执行到的层级**——「已完成结构检查 5/5，未执行 postgresql 隔离执行预检」。

这与 S19 已冻结的 smoke「不适用留痕」契约同形：**能力不具备时留下可归因的痕迹，而不是让「不适用」与「没跑」在结果里同形**。

### 2.52.5 层级自述要求

> **不得以「层级不足」冒充「已通过该层级」。**

只做了结构检查，就不能表述为「SQL 校验通过」而不加限定。判定结果必须携带其实际层级，消费方据此如实呈现。

这条与架构 §四十一.4 第三形态「冻结不是锚」同理：**证明力弱的东西不能声称成了强的**。哈希冻结不能冒充一致性锚，结构检查同样不能冒充方言校验。

### 2.52.6 绝不跨方言冒充

任何方言的 payload 都不得送入其它方言的校验器。这是原实现的正确意图，本功能完整保留：

- PostgreSQL / MySQL payload **绝不**进入 sqlite 执行路径；
- 降级时是**不执行**方言层，而非用别的方言执行；
- 不存在「用 SQLite 兜底」这一路径。

既有 `UT-S39-27`（用例名「不冒充通过」）锁的正是这个意图，但实现方式是「必须硬失败」。本功能将其改写为正向断言，保住意图而移除阻断。

### 2.52.7 留痕的输出通道

降级留痕经 `change-lint` 的既有 `warnings` 通道输出：

- **不计入 violations**，不影响 L9 通过与否；
- `warnings` 非空才出现，为空时字段整体省略（零漂移）；
- 三个消费方（change-lint、merge、baseline-apply）对同一 payload 与方言得到同一层级结论与同一留痕。

### 2.52.8 新增运行时依赖的边界

引入的 WASM 解析器**只做本地语法解析**：不建立网络连接、不接触凭据、不执行 SQL、不连接数据库实例；解析输入仅为提案自带的 delta payload。与 SQLite 分支的 `spawnSync` 真执行相比攻击面更小——不落盘、不起进程、不需数据库。

部署阶段须从解包 tarball 核验：随包分发、可加载、无 postinstall 脚本、体积增量在预期内。

### 2.52.9 兼容、失败与发布边界

- 判定结果只可能从「失败」变为「通过 + 留痕」，不会有反向变化；结构检查的判定逐字不变。
- package / plugin / asset identity 统一提升为本地 candidate `0.14.10`；全局 `0.14.9` 作为固定回滚制品。
- 不执行 npm publish、dist-tag、Git tag、GitHub Release、官网发布或 git push。

### 2.52.10 验收与追溯

- UT：UT-S39-59～UT-S39-64、UT-S35-132～UT-S35-133。
- ST：ST-S39-28、ST-S35-25。
- 安装态：SMOKE-core-174。
- 需求：AC-SQLGATE-01～09；场景：S35、S39；架构：§四十二。

## 2.54 机器读取命令的并发语义与读锁有界重试

### 功能目标

消除并发纯只读命令之间的 `baseline_commit_in_progress` 假阳性：读者之间靠既有排他锁**串行化**、靠**有界重试**吸收毫秒级读临界区的碰撞；`baseline_commit_in_progress` 只在「写事务确实在飞行且预算内等不到锁」或「journal 不可恢复」时出现。

### 2.54.1 读者串行化 + 有界重试（不重造共享读锁）

- 模块级事务锁保持**排他**语义与 `linkSync` O_EXCL 仲裁不变（架构 §4.4 锁权威逐字保留）；不引入 reader 计数或共享读锁协议。
- 三个读路径入口——`withBaselineReadLock`、`withRecoveredReadLocks`、`readGate`——的锁获取由「一次尝试立即失败」改为「有界指数退避重试」：
  - 默认总预算 **2000ms**；
  - 退避序列 **25 / 50 / 100 / 200 / 400ms**，此后每次封顶 400ms，直至预算耗尽；
  - 等待为**同步阻塞**实现（如 `Atomics.wait`），不引入 async 侵入、不改变三入口的同步函数签名；
  - **预算与时钟可注入**，测试不依赖真实墙钟。
- 预算内取到锁 → 走既有恢复门四步逻辑，行为与无竞争时逐字段一致。
- 预算耗尽仍被占用 → 维持现状返回/抛出 `baseline_commit_in_progress`。

### 2.54.2 `baseline_commit_in_progress` 的收窄语义

- 错误码名称、JSON error envelope 结构、非零退出行为**全部不变**；变化仅为出现条件收窄：
  - 收窄前：任何时刻锁被占（含另一个只读命令的毫秒级读临界区）；
  - 收窄后：写事务确实在飞行且读者预算内等不到锁，或 journal 不可恢复。
- 属兼容收敛：原本假阳性失败的并发只读调用改为成功，下游消费者（RunLogos 面板等）无需适配；对该错误的既有「瞬时可恢复」处理只会更少触发。

### 2.54.3 写路径与硬门不变量

- `begin` / `commit` 的锁获取保持 **fail-fast**（`run_locked` / `baseline_commit_in_progress` 立即返回）：writer-writer 竞争是真实互斥冲突，等待只会掩盖上层编排问题；「同模块新 `begin` 使旧 run superseded」的显式语义不变。
- 死锁回收协议（marker 仲裁、孤儿检测、原子替换）一字不动。
- 不可恢复 journal 的硬门零回退：读者即使经重试取到锁，前滚与回滚均失败时仍硬报 `baseline_commit_in_progress`，不读半新集合、不派生正常投影。

### 2.54.4 覆盖范围

- 所有经读取门的机器读取入口同等受益：`status`、`next`、覆盖率重算、resource-index 扫描、S39 EvidenceScanner 等复用同一 `acquireLock` 重试入口，不得由各消费者复制各自的重试分支。
- `status` 单次执行含多个读锁区间；每个区间独立适用同一重试语义。
...
## 2.58 合并事务的终态出路（spec-mutability）

### 功能目标

给合并事务补齐终态出路，消除三个同族死锁面（abort 死锁 / fatal failed 死锁 / completed 无二次 merge 通道），使任何终态都存在受控、留痕、可审计的重来路径；不改变 seal/apply/preflight/receipt 的任何既有判据与原子性语义。同构先例：切片事务 §2.53.6.1（活跃名额只含非终态）与 §2.56（completed 受控重划回边）。

### 2.58.1 活跃名额与终态归档让位

同一提案同时至多一个**活跃**合并事务；「活跃」只认非终态：

| phase | 占用活跃名额 |
|---|---|
| `collecting` / `ready` / `sealed` / `applying` | 是 |
| `failed`（含 `aborted` 与 fatal 分类） | **否**——可归档历史 |
| `completed` | 否，但**受保护**：仅经 2.58.3 的显式 reopen 后让位，禁止静默重建 |

`openlogos merge` 创建入口的分支语义：

- 非终态在场 → 幂等返回既有事务（逐字节不变的既有行为）；
- `failed` 在场 → 归档至提案目录 `merge-transactions/<transaction_id>.json` 后按当前 delta 重新规划创建（新 transaction_id、新 plan/target set）；
- `completed` 在场且 `SPEC_MERGED` 完好 → 拒绝静默重建，文案指向 `merge transaction reopen` 入口；经 reopen 后事务已归档，照常重建。

### 2.58.2 fatal failed 的受控出路

- `failed` 且 classification 非 `recovery_required`（如 `internal_failure`）的 `allowed_actions` 由 `[]` 扩充为 `['abort']`——abort 转 `aborted` 后经 2.58.1 重建。
- 既有 `recovery_required → ['recover']` 与 `applying → ['recover']` 逐字不变；abort 的既有拒绝面（存在正式 receipt/marker 时拒绝破坏性清理）逐字不变。

### 2.58.3 completed reopen 准入矩阵（二次 merge 通道）

- **命令面**：`openlogos merge transaction reopen --reason "<非空原因>" [--confirm-spec-merged]`。`reopen` 仅进入 `completed` 的 `allowed_actions`；其余 phase 不接受该动作。
- **准入矩阵**：

| 前置 | 结果 |
|---|---|
| `phase=completed` 且 `SPEC_MERGED` 不在场（异常残局） | 允许重开 |
| `phase=completed` 且 `SPEC_MERGED` 在场、附 `--confirm-spec-merged` | 允许重开，并**作废** `SPEC_MERGED`（旧合并事实不得冒充新规格基础） |
| `phase=completed` 且 `SPEC_MERGED` 在场、未附确认 | 拒绝，文案给出附 `--confirm-spec-merged` 的可执行指引、零副作用 |
| `--reason` 缺失或为空白 | 拒绝——留痕必须有非空原因 |
| 事务文件不可读 / marker 状态不可判定 | fail-closed 拒绝，无任何写副作用 |
| 其它 phase | `action_not_allowed`，既有出路不变 |

- 重开在单一动作内按序完成，任一步失败整体不生效：① `MERGE_REOPENS.jsonl` 追加留痕；② 旧终态事务归档至 `merge-transactions/`（receipt 与哈希可审计、不销毁）；③ 作废 `SPEC_MERGED`（仅确认路径）。重开后由 `openlogos merge` 按当前（已修正的）delta 重新规划新事务，重走 submit-content → seal → apply。
- 与 0.14.2 sealed 内 preflight-reopen（退回 collecting、事务身份不变）**不互相顶替**：preflight-reopen 修「seal 后发现的可修复缺陷」，本节 reopen 修「completed 后发现的规格错误」。

### 2.58.4 留痕与归档合同

`MERGE_REOPENS.jsonl`（提案目录内，append-only，每行一条）：

```json
{"schema":"openlogos/merge-reopen@1","old_transaction_id":"mtx_…","reopened_at":"<ISO8601>","reason":"<非空原因>","spec_merged_present":true,"confirmed":true}
```

- 历史行不得改写或删除；重复重开各自成行。
- 归档事务与 receipt 只供审计，不参与新事务的任何判定（不作 fallback 真相源）。

### 2.58.5 SPEC_MERGED 作废与指纹自然传导（C01 采纳）

- 重开**只**作废 `SPEC_MERGED` 并留痕，**不级联删除**任何下游产物（切片事务、`TEST_SLICE_MANIFEST.json`、`SLICES_APPROVED`、已实现代码）。
- 重合并 apply 成功后重写 `SPEC_MERGED` 与 receipt；`spec_fingerprint` 变化使既有 manifest 经 `deriveSliceVerificationState()` 既有判定自然判 stale，走切片侧既有恢复（`manifest-recovery`）或重划（`reopen`）回边收敛——复用 0.14.12/0.14.15 已闭环机制，不新建第二套级联清理。
- 被否方案（仅限未进入切片阶段可 reopen）：会制造「切片已 completed 而规格需重合并」的新死锁面，与本能力消死锁的目标相悖。
- `SPEC_MERGED` 作废期间 flow 派生自然退回 merge 前沿（`SPEC_MERGED` 缺失即未 spec-complete），无需新增驻留态。

### 2.58.6 投影一致与兼容

- 投影与拒绝文案必须由事务真实 `phase` / `classification` / marker 状态推出；「已归档」「待重开」「可重建」三种状态不得复用彼此模板文案。
- `openlogos/merge-transaction@1` 字段与 exit code 零变化；仅扩充终态 `allowed_actions`（`completed`→`reopen`、fatal `failed`→`abort`）与消费者动作映射（`reopen`→`reopen`）。属只增合法出路的兼容扩充，消费方未纳入前忽略即可，不做主版本跃迁（0.14.15 切片 reopen 同一先例）。
...
## 2.60 merge 流程契约自洽：前沿事务事实、后置条件二分与机器消费验收
...
## 2.61 reopen 后 test change set 提案级前滚合并
...
## 2.62 Claude guard hook 项目根定位、fail-closed 与 sync 补齐
...
## 2.63 OpenLogos 0.14.21 候选内容与发布验收
...
## 2.64 guard 管辖边界与阻断 reason 双通道输出（含 0.14.22 候选发布验收）
...
## 2.65 next 对 initial-plan 切片事务的问即建与投影输出（含 0.14.23 候选发布验收）
...
## 2.66 guard-check Bash 写命令路径提取与逐路径管辖判定（含 0.14.24 候选发布验收）
...
## 2.67 生命周期命令 fail-closed 与状态对账（含 0.14.25 候选发布验收）
...
## 2.68 切片规划单条受控写入口（slice plan）

### 2.68.0 问题：六动作事务为不存在的并发写付费

切片事务把「写一次切片规划」拆成 `submit-content`（×2 slot）→ `seal` → `apply` 共 6 次 CLI 往返，用 slot 双哈希与 staging 保证「多方并发写 `[code]` 与 manifest 的原子性」。但实际写入方只有**一个顺序执行的 AI**，且 `tasks.md` 与 `TEST_SLICE_MANIFEST.json` 均受 git 跟踪——误写 `git checkout` 即回滚。实测某提案的 `apply` 曾因 `task_text` 与 `[code]` 条目非逐字一致而拒绝一次，该拒绝不保护任何用户可感知价值。
...
### 2.68.2 结构化事实源的生成权归属（强制）

**流程判断必须使用结构化数据；非结构化文本只用于展示给用户。**

`owned_test_ids` 是 `verify` 计算 `eligible` 集合的输入，属流程判断数据，因此：

- **不得**改从 `tasks.md` 的 `[code]` 段散文解析——实测该解析会把正文中「被提及」而非「被拥有」的测试 ID 一并算入（某提案实测解析得 23 个 ID，而结构化 manifest 为 21 个，多出的两个在正文里是「转历史锚」这类叙述性提及），使 `eligible` 多算并以缺结果误红。
- 结构化产物的**生成权留在 CLI**：AI 产出结构化 `slices.json`，由 `slice plan` 校验后写入；AI 不直接写 `TEST_SLICE_MANIFEST.json`。

### 2.68.3 保留不变（零回归边界）

| 保留项 | 说明 |
|---|---|
| `verify` 的 `slice-checkpoint` 增量验收 | 收窄验收分母（`eligible = baseline ∪ 已确认切片 ∪ 本次切片`）与收窄实际执行（`OPENLOGOS_VERIFY_ELIGIBLE_TEST_IDS`）逐项不变——「每个切片实现后能单独通过验收」是产品能力 |
| `TEST_SLICE_MANIFEST.json` / `SLICE_CHECKPOINTS.jsonl` | 结构化事实源，保留 |
| `deriveSliceVerificationState` / `appendSliceCheckpoint` / manifest 读写 | 结构化数据层，一行不动 |
| `slice_state` / `code_slices_green` / `slice-exit` | 切片循环能力，保留 |
| `slice-planner` 六维打分与删后续证伪门 | 切片划分方法论不变，只是产物经 `slice plan` 落盘 |
