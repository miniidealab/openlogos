...
- **优先级**：P1
- **主路径**：用户执行 `openlogos next --auto`；CLI 取当前停顿步对应的 launched flow gate——若其 `skippable:true`（对应 `ready-to-merge` 的 propose 出口 gate）则视为已通过、放行并输出下一动作建议，同时向活跃提案目录下的 `GATE_AUTO_PASSED` JSONL **追加一行** `{gate_id, proposal_step, timestamp}`（不去重、不覆盖）；若其 `skippable:false`（对应 `ready-to-deploy` 的 deliver 入口 gate）则保持人类停顿。**默认 `next`（无 `--auto`）忽略 `GATE_AUTO_PASSED`、绝不因其存在而自动越过 gate**；其 base data 仍按当前契约输出（S28 起可能含 `next_node`）。
...
- **触发条件**：用户在 `logos/flow/<lifecycle>.yaml` 用 overlay op `set-loop` 把 implement（code/verify）子流程的 `loop` 改为 `max_iters > 1`（`until` 缺省沿用 builtin 的 `tests_green`），希望 `next` / `status` / `watch` 把这条子流程派生为「真迭代」——按「第 N/M 轮是否测试绿、是否收敛、是否达上限升级人类确认点」推进，而不再是 verify 一次失败即停的退化环。
- **价值**：把一次性 gate（verify 失败即停）升级为现代 agentic loop（generate → verify → fix 迭代到测试绿，actor-critic 结构）。严格 **A 被动派生**——OpenLogos 只派生「第几轮 / 是否收敛 / 是否升级 gate」并据此给措辞，**不自驱动跑测试**；迭代由人类/working_agent 修复后重跑 `openlogos verify` 推进。
- **迭代计数来源**：loop 激活时，`openlogos verify` 在算出 gate 结果（PASS/FAIL）后追加一行 `LOOP_ITERS` 账本（`{iter, node:"verify", result, module, timestamp}`，append-only）；`iteration` = 该账本按当前 module 过滤后的行数，`converged` = 末行 `result == "pass"`，`escalated` = `iteration >= max_iters && !converged`。未激活（builtin `max_iters:1`）时 verify **不写账本**、派生 **不输出** `loop_state`。
- **范围边界**：
  - **仅 overlay `set-loop`（`max_iters > 1`）激活**才进入真迭代派生；builtin `initial.yaml` / `launched.yaml` 的 implement loop 保持 `max_iters:1` 不变。
  - `set-loop` 的 `set` 字段**仅允许 `max_iters` / `until`**：`max_iters` 须整数 ≥1、`until` 仅枚举 `tests_green`；出现任何未知 key（如 `exhausted_gate`）或非法取值 → `FLOW_SCHEMA_INVALID`，不静默保留、不进 resolved flow。
  - **达上限升级 = loop 退出 human gate**：`escalated` 时派生为 implement 子流程的退出 gate（`gate_id = gate:implement:loop-exhausted`），**本切片 `skippable` 固定 `false`、不可 overlay 覆盖**；`next --auto` 照常阻塞、不 auto-pass、不写 `GATE_AUTO_PASSED`。「继续迭代」靠人类把 `max_iters` 调大（overlay `set-loop`）解除 escalated，或直接修到测试绿出环；gate 本身不重置计数。
  - **达上限不新增 `proposal_step` 枚举**：launched loop 未收敛时 `proposal_step` 仍为既有值（如 `ready-to-verify` / `verify-failed`）；「是否达上限」只由 `loop_state.escalated` + `next --auto` 的 `gate_id` / `skippable` 表达。
  - **initial 多模块** overlay 即便写了 `max_iters > 1` 也**不激活**（verify 是项目级单次运行、无法归属到某模块的 loop）：不写账本、不输出 `loop_state`、派生退化为旧行为（本切片已知不支持项）。
  - 本切片**不做**：`exhausted_gate.skippable` 的 overlay 覆盖、以及「无人值守放行不绿代码进入交付」（auto bypass 非收敛），其语义危险，留独立切片。
- **验收要点**：
  - **激活后未收敛且未达上限**（`iteration < max_iters`）：`next` 输出「loop 第 `iteration`/`max_iters` 轮未绿 → 让 working_agent 修复后重跑 `openlogos verify`（继续迭代）」，`loop_state` 中 `converged:false`、`escalated:false`，当前节点钉在 implement 内的 verify、不推进到后续子流程。
  - **达上限升级 human gate**（`escalated:true`）：`next` 输出「已达迭代上限 `max_iters` 仍未绿 → 升级人类确认点：继续迭代 / 调整方案 / 放弃」，`next --auto` 输出 `gate_id = gate:implement:loop-exhausted` + `skippable:false`、仍阻塞、不写 `GATE_AUTO_PASSED`。
  - **收敛出环**（末轮测试绿，`converged:true`）：implement 出环，续推到下一节点（deliver / archive）。
  - **未收敛不得推进**：loop 激活且 `!converged` 时，verify/implement 视为未完成，所有判定入口（含 initial 的 verify `done_when: file:acceptance-report.md`——FAIL 也会写报告）一律由 `loop_state.converged` 覆盖，不得把当前推进到后续子流程。
  - **收敛后再 FAIL 的回退**：再次 verify FAIL 时，现有逻辑清除 `VERIFY_PASS` 及下游 `DEPLOY_DONE` / `SMOKE_PASS` / `SMOKE_FAIL` marker → implement loop 重新打开、`converged` 转 false；账本续写、`iteration` 继续增长。
  - **status / watch 只读展示**：`status` / `watch` 输出 `loop_state` 供展示（watch 显示环进度），**只读、不执行测试、不写账本、不推进状态**。
  - **golden 零漂移**：builtin / 无 loop 激活的项目（含所有 golden fixture），`status` / `next` / `watch` 输出逐字节不变（不输出 `loop_state`、verify 不写账本）。
  - **统一引擎**：initial 与 launched 两条 implement 的 loop 派生用同一套逻辑，仅在 overlay 激活处生效。
