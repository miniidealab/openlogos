...
## S35/S09: 防过度设计的规模信号（变更类型 ↔ delta 层面观测 + 最小实现论证模板段）

**动因**：「防过度设计」三批落地的前两批（下游已归档 `20260919-2315-add-necessity-review-anti-overdesign`、`20260920-0418-observe-necessity-self-release`）产出的全部是**提示词文本与观测字段**——告诉 reviewer 要审必要性、告诉主 agent 驳回要举证。它们依赖 agent 读了并且照做，整套机制**没有一条机器可查、不依赖 agent 自觉的硬信号**。原始诊断 6 条中已收口 5 条，剩下的第 ④ 条只能在上游做：**变更类型形同虚设**——近 30 个提案中声明「代码级修复」的，实际改了 3–8 个 delta、跨 2–4 层规格。该条的两个判据源（`proposal.md` 的「变更类型」、delta 层级）都是 OpenLogos 的产物与判据，下游改不了。

第二条动因：proposal 必须论证「为什么不能更小」这条规矩在下游复盘时立下并确有实效，但它**只存在于那两份 proposal 的正文里**——模板没有这一段，没有任何机制提示下一个提案写它。换一个会话、换一个 agent，这条规矩就丢了。

**场景**：① 产出方运行 `openlogos change-lint` 时，CLI 比对提案声明的变更类型与实际铺开的 delta 规格层，不相称即在既有 `warnings[]` 通道报出，不阻断、不改 exit code；② 用户运行 `openlogos change <slug>` 创建提案时，产出的模板在「变更类型」之前带一段「最小实现论证」提示，要求落笔时先检索既有机制、论证为什么不能更小、写明主动砍掉了什么。

**验收条件**：

1. **观测规则的定性（不是方法论判据的投影）**：变更传播规则（`spec/change-management.md` §变更传播规则）表头为「**最少**需要更新」，给的是**下界**不是上界，因此「实际层超出」**不等于违反方法论**。本能力新增的层面对照表是为拿到诊断④ 的量而**新立的一条观测启发式**，命中只说明「声明类型与实际铺开的规格面不相称，值得看一眼」。warning 的 `message` / `fix_hint` **不得**措辞为「违反方法论」或等价表述。

2. **免计层级表（逐项覆盖分类器全部合法类别）**：按声明类型给出**不计入越界**的 delta 层——
   ...
   类别集合以分类器唯一事实源 `DELTA_TO_RESOURCE` 为准（`prd`/`api`/`database`/`scenario`/`test`/`decisions`/`spec`/`skills` 八类），`prd` 细到子目录（`1-product-requirements` / `2-product-design` / `3-technical-plan`），其余按一级类别；**新增 delta 类别时该表须同批扩充**。

3. **只统计参与合并的有效条目**：实际层级取既有 `classifyProposalDeltas()` 输出中 `mergeDisposition === 'mergeable'` 且 `lintValidity === 'valid'` 的条目。`explicitly_ignored`（reference / 隐藏文件）、`invalid`、未知类别与根下直放文件一律不计入——它们本就不进规格层，计入会把既有 L6 已覆盖的形态二次报成层级越界。

4. **声明类型的解析（既有机制不具备的新增能力，非等价提取）**：既有合法性正则是**包含式**检查（正文出现过任一类型即判合法），实测 `代码级（不涉及设计级或需求级变更）` 与 `设计级 / 代码级` 两种正文都通过，**不能**据以确定声明主体。故拆为两个函数、**共享同一份类型词表**（严禁第二份判据的落实方式是共享词表，不是共享正则）——
   - `isValidChangeType`：逐字节保留既有正则与语义，既有 `proposal_change_type_invalid` 的判定结果**零回归**；
   - `resolveChangeType`：取「变更类型」章节**首个非空行**作为声明主体、剥去其中的全角 `（…）` 与半角 `(…)` 说明文字、在剩余文本上全局匹配类型词，**恰好命中一个**才返回该类型，命中 0 个或 ≥2 个返回 `null`。

5. **`null` 静默**：`resolveChangeType` 返回 `null` 时**不告警**（宁可漏报不误报）。歧义不是缺陷形态，不新增违规码；注意歧义正文本身通过既有包含式合法性检查、**不会**触发 `proposal_change_type_invalid`，故两条通道同时静默——既有检查只覆盖「完全不含类型词」这一种形态。

6. **只走 warning 通道、单向告警**：新增码 `change_type_delta_layer_mismatch` 进 `ChangeLintWarningCode`，**不进** `ChangeLintViolationCode` 闭合枚举、**不改** exit code、**不改** 10/10 门禁计数、**不改**既有任何一条违规码的判据与文案；`warnings` 仍仅非空时出现。只在「实际层超出声明类型的免计集合」方向告警，**反向**（声明高层级而实际改得少）不告警——该形态未观测到，不预先造机制。

7. **提案模板新增「最小实现论证」段（刀二）**：`proposalTemplate` 的 zh / en 两套在「变更类型」之前插入 `## 最小实现论证`，含三个占位提示——先检索既有机制（列出查了什么、为什么不够用）、为什么不能更小、本提案主动砍掉了什么。两套模板须同批改动。
