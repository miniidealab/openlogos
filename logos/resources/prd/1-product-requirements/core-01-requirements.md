...
- **触发条件**：一个大功能在 implement 阶段无法一次写完，需要逐片实现、无人值守自愈，直到全部 `[code]` 切片完成且测试绿。
- **用户价值**：一个提案设计一次、切片实现多次；implement loop 逐片闭环 code→verify，靠客观信号（全部切片勾选 + 测试绿）保质量，靠 `next --auto` 保无人值守。
- **优先级**：P1
- **主路径**：内置 launched `implement` 默认激活切片循环（`until: code_slices_green`、`max_iters:30`）。切片清单 = `tasks.md` `[code]` section 每个未勾行；`next` 选第一个未勾切片为当前工作项、`next_node` 钉在 `code` 并带 `slice` 子提示；`verify` 跑全量回归、追加 `LOOP_ITERS`（可带 `slice`）；全部切片勾选且末轮测试绿才出环；达 `max_iters` 仍未达成升级 `gate:implement:loop-exhausted`（`skippable:false`）。

#### 验收条件
##### 正常：逐片推进的当前切片提示
- **GIVEN** launched 提案处于 implement，`[code]` 有未勾切片，loop 未收敛、未达上限
- **WHEN** 用户执行 `openlogos next`
- **THEN** `next_node` 指向 `code` 节点并带 `slice` 子提示（第一个未勾 `[code]` 行标题）；`slice_state` 输出 `{total, done, current, remaining}`

##### 正常：全部切片完成且测试绿才出环（FAIL-safe）
- **GIVEN** loop 激活、`until: code_slices_green`
- **WHEN** 派生判定 implement 是否完成
- **THEN** 仅当 `section_complete:code`（`[code]` 全勾）**且** 末轮测试绿时 `converged=true` 出环；任一不满足则 `converged=false`、不得推进到 deliver/close
...
- **WHEN** 用户执行 `openlogos next`
- **THEN** `slice` 提示仍指向第一个未勾切片（待建）；具体修哪里由全量 verify 失败输出决定、归宿主判（A 被动派生，引擎不代判）
