...
#### 四十三.2.1 原子写入不等于合法产出：终态必须经自身判定器复核

把两个产物收进同一次事务，只保证了它们**同生共死**，没有保证它们**合法**。事务可以原子地写出一对互相一致、但业务上非法的产物，然后宣告成功。

> **事务在提交终态之前，必须用自身的判定器复核它刚写出的产物；判为非法即整体回滚，不得进入成功终态。**

判断一次「原子写入」是否真的兑现了不变量 H，问两个问题，缺一不可：
...
本次实例：`cli/src/lib/test-slice-transaction.ts` 中确实存在

```ts
/** 复核：apply 后 manifest 被既有判定器判为何种状态。不新建第二套 validator。 */
export function verifyAppliedManifest(root, proposalDir) { … }
```

而全仓 `grep` 的结果只有它自己的定义行——**调用方为 0**。复核器被写了出来、注释也写明了意图，却从未接进 `apply` 主路径。于是 `apply` 写盘成功即置 `completed`，同一进程内的判定器随即把这份刚写出的产物判为 `invalid`。

这是 §四十一.6.2「没有失败信号的约束不是约束」的一个具体且极易漏检的形态：**代码里存在一个看起来在守卫这条约束的函数，但没有任何执行路径经过它。** 代码审查看到函数在场就以为约束成立；测试不覆盖它是因为它根本不在主路径上。识别方法是一个可机械执行的检查：

> **这条约束被违反时，具体哪一行会失败？** 答不出来，或答案是一个零调用方的函数，则该约束不存在。

##### 与写入权的关系

判定权与写入权同源（§四十三.1）是复核成为可能的**前提**，不是复核本身。写入方拥有全部判定原语却在提交终态前不调用，属于「拥有能力而不行使」——比权限分离更隐蔽，因为它在架构图上看不出任何缺口。
...
| §四十三.2.1 | 终态的**准入条件** | 事务原子地写出非法产物后宣告成功；复核器存在但零调用方 |
...
## 四十七、merge 前沿推进判据与事务事实同源（marker 影子判据退休）
...
## 四十八、test change set 提案级权威与 reopen 前滚合并
...
### 51.1 缺陷形态：定位用的规范化形式被拿去当落盘内容

合并引擎对章节锚做**规范化**以支持等价匹配——`parseDeltaBlocks` 把锚存成 `stripInlineCode(rawAnchor)`，`resolveSectionAnchor` 对候选标题做同样处理，于是 `` `X` `` 与 `X` 在定位上等价。这是正确的：锚是**寻址坐标**，坐标不该因排版标记而失配。

错的是把同一份规范化结果拿去当**产出内容**。`composeOpenLogosMarkdown` 的 `ADDED` 分支曾这样发射新章节标题：

```ts
const segments = block.anchor.split(' > ').map(item => stripInlineCode(item).trim());
const title = segments[segments.length - 1];   // ← 用的是剥离版
```

而 `stripInlineCode` 是**整段删除**行内代码而非脱掉反引号。于是凡标题含 `` `x` `` 的 `ADDED` 块，合并后该段内容**从标题里消失**：

| delta 里写下的标题 | 落盘的标题 |
|---|---|
| ``Delta `RENAMED` op：章节标题更名`` | `Delta  op：章节标题更名` |
| ``3.17 `state_inconsistency` 对账投影字段`` | `3.17  对账投影字段` |

两处都是已合并规格中的真实损坏，分别发生于 2026-09-11 与 2026-09-07——**两次都无人发觉**。

### 51.2 为什么没有检查者

`verifyAgentMaterialOutcome` 对 `ADDED` 的后置条件是「锚在 final 中唯一命中」。而锚本身就是剥离版，落盘标题也被同样剥离——**两边用同一把尺子量，永远相等**。复验对这类损坏天然盲：它验证了「章节存在」，从未验证「章节叫对了名字」。

这是「判据选错的守卫等于没有守卫」（§49.3）的第三次出现，也是本节要立成原则的理由：

> **规范化形式只用于比较与定位，不得充当落盘内容。**
> 产出内容一律取**原始形式**；产出后必须有一道检查者对账「写下的 == 落盘的」，且该对账**不得使用与产出同一条规范化管道**——否则它只是把同一个错误算了两遍。

### 51.3 边界：哪些是「规范化形式」

| 形式 | 用途 | 可否作为产出 |
|---|---|---|
| `stripInlineCode` 后的锚 | 章节定位、等价匹配 | **否**——落盘标题取 `rawAnchor` |
| `normalizeText` 后的 `task_text` | manifest 与 `[code]` 条目比对 | 否——落盘取原文 |
| `<ROOT>` / `<PKG_VERSION>` 等规范化占位 | golden 比对时抹平环境事实（§四十九） | 否——它们本就只存在于比对侧，不落盘 |

三者是同一条原则的不同面：**比较可以宽松，写入必须精确**。
