...
- **必须 alias-aware（否则误杀合法候选）**：由 §三「身份继承」——重命名/移动锚点时旧 anchor 追加进 `aliases[]`、`key` 保持稳定（不随当前 anchor 变）。故改名继承候选满足 `key === candidateKey(module, 旧anchor∈aliases)` 而非当前 anchor；合并/拆分产生的 `tombstone`（`superseded_by` 指向新 key）其 `key` 由自身原 anchor 或 alias 可重算。单纯用当前 anchor 重算会误杀这些合法候选。
- **判据只认 hash 可重算性、不认前缀**：`isCanonicalKey`（格式校验 `<module>::<12hex>`）不足——示例 key 格式合法却能过格式校验，必须用 `candidateKey` 重算比对。
- **作用面（读侧全链一致）**：`listModuleProvenanceDocs` 与 `scanModuleCandidates` 均按本不变量过滤；由此 `buildBaselineCoverage` 与 `feature-backfill` 同源受益：幽灵示例候选不进分母、不进 `aggregate_hash`、不污染 freshness、不触发 feature-backfill 误报。
- **不改 `candidates[]` 结构、不改覆盖率口径**：本不变量只收紧“哪些候选被采信”，不新增字段、不改分母法（§三）、不改 provenance 派生（§一）。合法基线项目的覆盖率数值与新鲜度逐字节不变。
...
- **规范键** `key` = `<module> + "::" + sha256(normalize(anchor))[:12]`；`normalize` = NFKC 归一 + 去首尾空白 + 小写 + 折叠内部空白。`anchor` 取语义标识（CLI 命令名 / 导出入口符号 / 路由），非易变文件路径。规范值是 hash 形式；可读 slug 只能进 `display`/`anchor`，不得作 `key`。
- **身份继承**：重命名/移动锚点 → 旧 anchor 追加进 `aliases[]`，匹配 alias 即同一候选、不新建；合并/拆分 → 旧 key 保留、`state:tombstone` 且 `superseded_by` 指向新 key；扫描器版本升级 → 迁移映射继承旧 key，无法继承者按新 key 登记、旧 key 转 tombstone。
- **候选三态**：`active`（存活）/ `tombstone`（重扫已消失但未废弃）/ `retired`（已废弃）。
- **候选计数（原“分母”）** = `active ∪ tombstone` 候选数（`retired` 不计入）。覆盖率退化为纯逆向候选计数：只报 `denominator = |active ∪ tombstone|` 与单列 `tombstones`，不再有分子（`human_verified`）或 `coverage` 比值/百分比，按 module 聚合。
- **零候选**：`|active ∪ tombstone| == 0` ⇒ `denominator` 为 0、覆盖率报 `n/a`，不报 100%/0%。
- **不虚增定理**：重扫删除/合并候选只令其 `active→tombstone`（仍计入 `denominator`），计数不因删除而缩小；`retired` 候选移出计数并写事件日志。`status --format json` 的 `baseline_coverage` 不再输出 `human_verified_delta`。
...
- 取值与转换：
  ...
  - 用户/宿主显式选择 `brownfield-adopter` 扫描，产物落盘后经 CLI 命令按 manifest 计算：**≥1 但未全** expected 合法 → `partial`；**全部** expected 合法 → `seeded`。
  - 扫描失败且尚未进入 commit journal → 保持当前值（`required` 或 `partial`），允许重试；重扫按 `key` 覆盖/清理部分产物，不回滚已初始化的 `logos/`。
- **owner 与唯一写入入口（两阶段 staging）**：字段的原子写入 owner 是 **CLI**，唯一合法**状态推进**路径是命令 `openlogos baseline-seed`（`begin`/`commit`/`status`，见 feature-specs §2.27.8）——`adopt` 写初值 `required`；producer（skill/driver）**不直接改 YAML、也不直接写目标 `logos/resources/`**，只把产物写入 run 私有 staging，经 `commit` 让 CLI 校验后原子提交目标文件与状态。此外 `openlogos sync` 的元数据迁移可在恢复门成功后，对**缺失该字段的 legacy adopted 模块**做一次性回填（见下方「缺省语义」，仅回填缺失字段、不推进状态）。
...
- **JSON 映射（仅正常成功 envelope）**：恢复门确认不存在未终结 journal或已成功恢复后，`status`/`next --format json` 的 `baseline_coverage.state` 映射该枚举，`incomplete` 恒为布尔（`partial`→`true`，否则 `false`）；正常成功的 adopted 模块 `modules[].baseline_seed_state` 恒输出 explicit 或缺省派生值。无活跃提案时三态主动作均为 `openlogos change <slug>`；安全 partial 的 seed 重试仅作非阻断 `recovery` advisory。
- **缺省语义（正常路径三入口单一事实源，baseline-seed-legacy-default-unify）**：字段缺失（legacy adopted，字段引入前接入）时的有效状态由**唯一共享 helper** `effectiveBaselineSeedState(root, moduleId, explicit) → { state, legacy }`（`cli/src/lib/baseline-jit.ts`）派生，`next` / `status` / `baseline-seed` 状态机三入口在**恢复门成功之后**只准经该 helper 取有效状态，禁止持有第二份私有缺省规则。派生规则：explicit 优先；缺省时**有候选（`scanModuleCandidates` > 0）且同模块存在 open run record → `partial`**、**有候选无 open run → `seeded`**、**无候选 → `required`**。正常成功输出不存在 `unknown`。**读锁纪律**：调用方先在模块锁内检查/恢复 journal，成功后 helper 才可在同一锁区间读取权威文档与 run 记录（支持外层已持锁复用）；恢复失败不得调用 helper。`legacy: true` 表示派生值，供 legacy 迁移提示与 sync 使用。
- **未终结事务硬门优先于缺省派生**：`prepared`/`committing` 等 journal 无法安全前滚或回滚时，消费者必须在调用 `effectiveBaselineSeedState`、`scanModuleCandidates`、读取 run/index/coverage 之前停止，非零返回统一 `baseline_commit_in_progress` error envelope。错误仅可携从最小安全元数据取得的 module/run/journal 与修复提示；**不承诺输出正常 `modules[]`、`modules[].baseline_seed_state`、coverage 或 suggestion**，也不得猜测枚举。恢复成功后才恢复上述恒输出契约。
- **历史兼容迁移（sync 落盘，legacy 缺省态物理消亡）**：读到旧布尔 `baseline_seed_required: true` ⇒ 映射为 `baseline_seed_state: required`；`false` ⇒ 移除布尔、不直接推断。**两字段皆无的 adopted 模块**（含历史 `skipped` 兼容读取）⇒ `openlogos sync` 在 journal 恢复门成功后调用上述共享 helper 派生并**写入显式枚举**到 `logos-project.yaml`，changes 记录写明派生依据（如 `core: baseline_seed_state 缺省 → required（派生：无逆向候选）`）。已有显式枚举**不覆盖**。迁移幂等、写前备份。恢复失败时 sync 同样硬报 `baseline_commit_in_progress`，不得为完成迁移而扫描半新集合；迁移后运行时派生仅作「迁移尚未执行」的过渡兜底。
...
**恢复门（消除崩溃后、恢复前的半新可见窗口）**：所有机器消费者——`status` / `next` / 覆盖率重算 / resource-index 扫描 / 派生器——在读取该模块任何目标或据其算覆盖率**之前**，必须：① 取模块级事务锁；② 检测是否存在未终结（`prepared`/`committing`）journal；③ 若有则**先执行恢复**（下述前滚/回滚），恢复后再读；④ 若无法恢复则输出 `baseline_commit_in_progress` 并**不把当前目标集合视为权威**。不可恢复分支不调用 legacy seed 状态 helper、不派生正常 `modules[]`/coverage/suggestion；错误只携最小安全 module/run/journal 诊断。因此即使 prior 状态曾为 `seeded`，机器读取者也绝不复用旧状态或据半新集合出权威结论。
