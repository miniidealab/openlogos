...
## 存量逆向基线的 JIT 深化（change-writer advisory，不设硬门）

> 适用范围：`bootstrap: adopted` 模块下，活跃 `openlogos change` 的目标区域只有 `verified: false` 的逆向 spec（种子基线，见 S33）时启用。其余情形 S09 主时序图与步骤零改动。

change 创建后 guard 生效——既不能直接改 `logos/resources/**`，也不能嵌套创建第二个 change（guard 互斥）。因此「先确认现状再打前向 delta」**不采用双 delta**（现行 `deltas/** → 主文档` 为 1:1 映射、`MODIFIED` 替换同名章节，无法表达同一目标文档的两份有序 delta）。唯一合法且可由现行协议表达的路径是：

1. **单份最终态 delta**：每个被触碰目标文档只产出一份最终态 delta，承载「人工确认后的现状 + 本次前向改动」的最终内容，按现行 `MODIFIED` 落盘，走既有映射，**merge CLI / merge-executor / delta schema 一律不改**。
...
3. **现状确认是审计事实、非第二份 delta**：确认了哪些逆向候选、证据、确认人记在该章节 + `tasks.md` `[delta]` 勾稽对应候选 ID，不新增第二个落盘目标。
4. **`human-verified` 仅 merge 后生效**：merge 前主文档仍 `verified: false`，覆盖率**只读已合并主文档**，advisory 不因未合并 delta 提前消失或声称前移。
5. **advisory 不设硬门**：用户可跳过建议直接写前向 delta；该区域 `verified` 保持 `false`、覆盖率不前移。

### EX-9.6: change 触碰未验证逆向区域给 advisory 并单 delta 确认
- **触发条件**：`bootstrap: adopted` 模块、活跃 change 的目标区域只有 `verified: false` 逆向 spec。
- **期望响应**：change-writer 给 advisory（不设硬门），建议在当前 change 的单份最终态 delta 内一并确认现状（`## 逆向基线来源` 置 `verified: true` + `confirmed_by`/`evidence`）；不生成第二份「现状确认 delta」，不直接改 `resources/**`，不嵌套第二个 change。
- **副作用**：用户跳过 advisory 时不阻断（直接写前向 delta）；`human-verified` 仅在该 delta merge 落主文档后生效。

### EX-9.7: merge 前不提前声称覆盖率前移
- **触发条件**：现状确认写入 delta 但尚未 merge。
- **期望响应**：覆盖率只读已合并主文档，未合并 delta 不计入；advisory 不因未合并 delta 消失。
- **副作用**：无。
