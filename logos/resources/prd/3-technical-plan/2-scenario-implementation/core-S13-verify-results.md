...
## S13 写入权转移后 verify 消费零漂移

### 场景目标

明确 verify 对 `TEST_SLICE_MANIFEST.json` 的消费判据与 slice-checkpoint 前沿**不因写入权转移而改变**，并给出可执行的零漂移判定方式。

### 为何需要单独确立这一点

本次变更改的是「谁写 manifest」，不是「manifest 长什么样」，也不是「verify 怎么读它」。但写入权转移触及的是 verify 赖以判定前沿的输入，因此必须显式证明消费侧零漂移——**否则一次意在修复的改动，可能让既有 checkpoint 集体失效**。

### 参与者与前置条件

| 别名 | 组件 | 说明 |
|---|---|---|
| V | `openlogos verify` | 分层执行与 checkpoint 记录 |
| D | `deriveSliceVerificationState()` | manifest 状态与前沿判定 |
| M | `TEST_SLICE_MANIFEST.json` | 产物（写入者已转移） |
...
sequenceDiagram
    participant V as verify
    participant D as deriveSliceVerificationState
    participant M as manifest

    V->>D: Step 1: 求切片验收状态
    D->>M: Step 2: 读 manifest（不关心谁写的）
    D->>D: Step 3: 指纹比对、owned/changed 归属、前沿推进
    D-->>V: Step 4: manifest_status / verify_mode / attempted / eligible / pending
    V->>V: Step 5: 按既有分层执行与 checkpoint 记录
```

**Step 2 是零漂移的根据**：判定器读的是文件内容，不携带任何关于写入者的假设。同一份字节由 Agent 写出还是由事务 apply 写出，`deriveSliceVerificationState()` 的结论逐字相同。

### 不变量

1. **判据不变**：manifest 合法性、`stale` 判定、eligible/pending 计算的实现与本次变更前逐字相同。
2. **前沿不变**：同一份 manifest 字节下，`attempted_slice_id` 与 `confirmed_slice_ids` 的推进结果不变。
3. **既有 checkpoint 不失效**：本次不改 `SLICE_CHECKPOINTS.jsonl` 的形状与写入者（仍是 verify）。
4. **历史归档不迁移**：归档提案内的 manifest 保持原样，verify 对其判定不变。

### 判定方式

零漂移由「同字节同结论」证明：对同一份 manifest 内容，分别以「Agent 写出」与「事务 apply 写出」两种来源构造，断言 `deriveSliceVerificationState()` 的完整返回值深相等。**不以「测试仍然通过」作为零漂移证据**——那只能说明没有明显破坏，不能说明结论逐字相同。

### 异常与边界

- 事务 apply 写出的 manifest 若不合法：verify 照常判 invalid，与来源无关。
- 恢复事务重写 manifest 后：既有 checkpoint 依 `manifest_sha256` 匹配，行为与本次变更前一致。

### 追溯

- 需求：AC-SLICETX-11。
- 功能规格：§2.53.8；架构：§四十三.1。
- 测试：UT-S13-67。
