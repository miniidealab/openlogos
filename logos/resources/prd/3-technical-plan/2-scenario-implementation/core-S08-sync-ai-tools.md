...
细化 Step 5「扫描并补录 resource_index」中**候选集合构成**与**描述推断**两个子步骤。前次变更已把该步骤的写回定死为 AST 写入并规定「产物可解析」的后置判据；本节补齐它的输入端——扫哪些文件、每个文件该被描述成什么。

### 参与者与前置条件

| 别名 | 组件 | 说明 |
|------|------|------|
| IDX | 资源索引补录器 | 扫描候选、推断 desc、结构化写回 |
| FS | 项目磁盘 | `logos/resources/*`、`spec/`、`skills/` |
| RULES | 描述推断规则表 | 路径 → 文档语义，判据从项目根锚定 |
| KIND | baseline-seed kind 集合 | 棕地 seed 的必需/可选产出物定义 |

前置：`logos-project.yaml` 当前可解析（不可解析时按既有降级路径处理，不进入补录）。
...
sequenceDiagram
    participant IDX as 资源索引补录器
    participant FS as 项目磁盘
    participant RULES as 描述推断规则表

    IDX->>FS: Step 5.4a: 扫描各规格根目录
    alt 扫描 logos/resources/verify/
        IDX->>FS: Step 5.4b: 只取顶层文件（报告）
        IDX->>IDX: Step 5.4c: 排除全部子目录（事务工作区/证据包/部署制品）
    else 其余扫描根
        IDX->>FS: Step 5.4d: 递归取全部文件（行为不变）
    end
    loop 每个候选路径
        IDX->>RULES: Step 5.4e: 按从项目根锚定的判据求描述
        alt 命中规则
            IDX->>IDX: Step 5.4f: 记为待补录条目
        else 无匹配
            IDX->>IDX: Step 5.4g: 计入 skipped，不收录
        end
    end
    IDX->>IDX: Step 5.5→5.9: 结构化写回（既有流程不变）
```

### 步骤说明

1. **补录器**扫描各规格根目录。`logos/resources/verify/` 按「顶层是报告、子目录是产物」分层：只取顶层文件，子目录一律不进候选集合。其余扫描根的递归行为逐字不变。
2. 该排除与 `logos/spec/baseline-closure.md` 一致——该规范已禁止 baseline-seed staging 进入 effective view，而 `resource_index` 正是一种 effective view。
3. **补录器**对每个候选路径求描述。判据从项目根锚定：嵌套了权威路径的路径（事务工作区、归档、备份、临时拷贝）一律不命中权威规则。
4. 场景实现目录的推断分两支：`SXX-` 文档沿用既有场景时序图描述；非 `SXX-` 文档走按目录的兜底规则，覆盖该目录下全部合法 kind（含 `scenario-candidates`）。
5. 无匹配规则的路径计入 skipped、不收录——该既有语义不变。因此**描述规则的覆盖面就是索引的收录面**：规则漏了什么，索引就永久缺什么。

### 不变量

- **输入边界**：事务内部工作区、证据包与部署制品不进入候选集合；`verify/` 顶层报告仍正常收录。
- **判据锚定**：路径判据从项目根起，禁止子串命中；副本不得获得与权威逐字相同的描述。
- **kind 对齐**：baseline-seed 每个 kind 的规范产出路径都能推断出非空描述，该对齐由测试锚定。
- **兜底按目录**：场景实现目录的非 `SXX-` 兜底覆盖全部合法 kind，不为单个文件名开洞。
- **既有条目不改写**：本步骤只新增待补录条目，不删除、不重写任何既有 `resource_index` 条目。
- **零回归**：其余扫描根、`SXX-` 文档描述、幂等与守恒语义逐字不变。

### 异常与边界

| 编号 | 触发条件 | 处理 |
|---|---|---|
| EX-S08-IDX-4 | 候选路径位于 `verify/` 子目录 | 不进入候选集合；不计入 skipped 统计（它从一开始就不是候选） |
| EX-S08-IDX-5 | 路径嵌套了权威路径（如 seed 快照） | 判据不命中；若该路径仍在候选集合内则计入 skipped，绝不套用权威描述 |
| EX-S08-IDX-6 | 场景实现目录下的非 `SXX-` 文档 | 命中兜底规则，得到场景实现语义的描述；不得被判为验收报告 |
