...
## [code] 代码实现
- [x] 更新 `website/scripts/generate-releases.mjs` — 读取 `CHANGELOG.md` 中的版本说明摘要并写入静态发布数据
- [x] 更新 `website/src/data/releases.json` 的生成结果结构 — 为版本项提供可展示的说明摘要字段
- [x] 更新 `website/src/pages/releases.astro` — 在版本卡片中展示版本价值、问题修复摘要与外链回退提示
- [x] 更新 `website/scripts/smoke-releases.mjs` — 校验 `/releases` 页面存在版本说明摘要和回退提示
- [x] 更新相关测试用例 — 覆盖 release 摘要解析、页面渲染与烟测校验

## [deploy] 部署任务
- [x] 按部署方案重新部署官网生产环境
- [x] 确认 `/releases` 页面可访问且版本说明已生效
