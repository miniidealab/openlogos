...
const utf8NonAscii = `[\\u{0080}-\\u{D7FF}\\u{E000}-\\u{10FFFF}]`;
...
const atext = `(?:[\\w!#$%&'*+\\-/=?^\`{|}~]|${ucschar}|${utf8NonAscii})`;
const atom = `${atext}+`;
...
const qtextSMTP = `(?:[\\x20-\\x21\\x23-\\x5B\\x5D-\\x7E]|${ucschar}|${utf8NonAscii})`;
...
const mailbox = `${localPart}@(?:${addressLiteral}|(?<domain>${domain}))`;
