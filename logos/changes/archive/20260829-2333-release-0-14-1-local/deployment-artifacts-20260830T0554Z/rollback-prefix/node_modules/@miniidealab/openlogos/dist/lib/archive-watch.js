...
export const nodeArchiveWatchFs = {
    exists: existsSync,
...
export function archiveWatchErrorCodeFor(kind) {
    return ARCHIVE_WATCH_ERROR_CODES[kind];
}
export function normalizeProjectPath(path, platform) {
    if (platform === 'win32') {
        return win32.resolve(path).replaceAll('\\', '/').replace(/\/$/, '').toLowerCase();
    }
    return resolve(path).replaceAll('\\', '/').replace(/\/$/, '');
}
export function hashCanonicalProjectPath(canonicalPath) {
    return createHash('sha256')
        .update(`${ARCHIVE_WATCH_PROTOCOL}\0${canonicalPath}`, 'utf8')
        .digest('hex');
}
export function projectIdForRoot(projectRoot, platform, fs = nodeArchiveWatchFs) {
...
export function resolveArchiveWatchPaths(projectRoot, requestId, instanceId, fs = nodeArchiveWatchFs) {
    if (requestId !== undefined)
        assertIdentifier(requestId, 'request-id');
    if (instanceId !== undefined)
        assertIdentifier(instanceId, 'instance-id');
...
export function assertArchiveWatchPathSafe(paths, fs = nodeArchiveWatchFs) {
    if (!isContained(paths.projectRoot, paths.runtimeRoot)) {
        throw new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.prepare, 'protocol-path-escape');
    }
    for (const target of new Set(Object.values(paths).filter((path) => typeof path === 'string'))) {
...
function ensureRuntimeDirectories(paths, fs) {
    assertArchiveWatchPathSafe(paths, fs);
    fs.mkdir(paths.instancesDir);
    fs.mkdir(paths.requestsDir);
    if (paths.requestDir)
        fs.mkdir(paths.requestDir);
    if (paths.acksDir)
        fs.mkdir(paths.acksDir);
    assertArchiveWatchPathSafe(paths, fs);
}
...
export function atomicWriteArchiveWatchJson(paths, targetPath, value, runtime = {}) {
...
function leaseFromRecord(record) {
    ...
    return {
        ...
        capabilities: record.capabilities.filter((item) => typeof item === 'string'),
    };
}
export function snapshotArchiveWatchInstances(projectRoot, projectId, runtime = {}) {
    const fs = runtime.fs ?? nodeArchiveWatchFs;
    ...
    for (const filename of fs.list(paths.instancesDir).filter(name => name.endsWith('.json')).sort()) {
        ...
        if (!IDENTIFIER_PATTERN.test(instanceId))
            continue;
        ...
        if (!lease || lease.instanceId !== instanceId || lease.projectId !== projectId)
            continue;
...
export function cleanupArchiveWatchRuntime(projectRoot, runtime = {}) {
    const fs = runtime.fs ?? nodeArchiveWatchFs;
    const nowMs = (runtime.now ?? (() => new Date()))().getTime();
    const ttlMs = runtime.ttlMs ?? DEFAULT_ARCHIVE_WATCH_TTL_MS;
    const paths = resolveArchiveWatchPaths(projectRoot, undefined, undefined, fs);
    if (!fs.exists(paths.runtimeRoot))
        return;
    assertArchiveWatchPathSafe(paths, fs);
    if (fs.exists(paths.instancesDir)) {
    ...
    if (fs.exists(paths.requestsDir)) {
        for (const requestId of fs.list(paths.requestsDir)) {
            ...
            if (!requestPaths.requestDir || fs.isSymlink(requestPaths.requestDir))
                continue;
            ...
            if (baseTime !== null && baseTime + ttlMs <= nowMs) {
                try {
                    fs.removeTree(requestPaths.requestDir);
                }
                catch { /* 幂等清理失败不阻断归档 */ }
            }
        }
...
function findInFlightRequest(projectRoot, projectId, slug, fs, nowMs) {
    const paths = resolveArchiveWatchPaths(projectRoot, undefined, undefined, fs);
    if (!fs.exists(paths.requestsDir))
        return null;
    assertArchiveWatchPathSafe(paths, fs);
    for (const requestId of fs.list(paths.requestsDir).sort()) {
        ...
        if (!requestPaths.preparePath || !fs.exists(requestPaths.preparePath))
            continue;
        if (requestPaths.resultPath && fs.exists(requestPaths.resultPath))
            continue;
        const prepare = readJson(requestPaths.preparePath, fs);
        if (!prepare)
            continue;
        const deadline = parseDate(prepare.deadlineAt);
        if (prepare.protocol === ARCHIVE_WATCH_PROTOCOL
...
export function findRecoverableArchiveWatchSession(projectRoot, slug, runtime = {}) {
    ...
    if (!fs.exists(basePaths.requestsDir))
        return null;
    ...
    for (const requestId of fs.list(basePaths.requestsDir).sort()) {
        ...
        candidates.push({
            ...
            session: {
                ...
                projectId,
                requestId,
                expectedInstances: [...new Set(prepare.expectedInstances)].sort(),
                paths,
            },
        });
    }
    if (candidates.length > 1) {
        throw new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.inconsistent, 'multiple-recovery-requests', candidates.map(candidate => candidate.session.requestId ?? '').filter(Boolean));
    }
    return candidates[0] ?? null;
}
export function createArchiveWatchPrepare(projectRoot, prepare, runtime = {}) {
    assertArchiveSlug(prepare.slug);
    const fs = runtime.fs ?? nodeArchiveWatchFs;
    const paths = resolveArchiveWatchPaths(projectRoot, prepare.requestId, undefined, fs);
    if (!paths.preparePath)
        throw new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.prepare, 'prepare-path-missing');
...
export function pollArchiveWatchAcks(projectRoot, requestId, expectedInstances, deadlineAt, runtime = {}) {
    const fs = runtime.fs ?? nodeArchiveWatchFs;
...
export function encodePreparedArchiveWatchToken(payload) {
...
export function validatePreparedArchiveWatchToken(token, expected) {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            return false;
        const value = parsed;
        if (value.protocol !== ARCHIVE_WATCH_PROTOCOL
...
export function writeArchiveWatchResultBestEffort(session, result, runtime = {}) {
    if (session.mode !== 'coordinated' || !session.requestId || !session.paths?.resultPath)
        return false;
    try {
...
function notArchivedResult(session, reason, now, runtime) {
...
export function coordinateArchiveWatch(projectRoot, slug, runtime = {}) {
    const platform = runtime.platform ?? process.platform;
    if (platform !== 'win32')
        return { mode: 'non-windows', expectedInstances: [] };
    assertArchiveSlug(slug);
    const fs = runtime.fs ?? nodeArchiveWatchFs;
    const now = runtime.now ?? (() => new Date());
    ...
    try {
        ...
        if (error instanceof ArchiveWatchError)
            throw error;
    ...
    if (snapshot.incompatible.length > 0) {
        throw new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.prepare, 'incompatible-instance', snapshot.incompatible.map(item => `${item.instanceId}:${item.reason}`));
    }
    ...
    const session = { mode: 'coordinated', projectId, requestId, expectedInstances, paths };
    let poll;
    try {
    ...
    if (poll.status === 'failed') {
        ...
        throw new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.instance, poll.reason, [poll.instanceId]);
    }
...
function readGuardActiveChange(guardPath, fs) {
    if (!fs.exists(guardPath))
        return null;
    const guard = readJson(guardPath, fs);
    return guard && typeof guard.activeChange === 'string' ? guard.activeChange : null;
}
export function reconcileArchiveDiskState(livePath, archivePath, guardPath, fs = nodeArchiveWatchFs, slug) {
...
export function isWindowsArchiveBusyError(error) {
    if (!error || typeof error !== 'object')
        return false;
    const code = error.code;
    return code === 'EPERM' || code === 'EACCES' || code === 'EBUSY';
}
//# sourceMappingURL=archive-watch.js.map