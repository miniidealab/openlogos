...
import { parseTaskSections, isCodeRequiredForProposal, resolveProposalDeploymentDecision, evaluateTestIdEvidence, extractStructuredTestIds, extractTaskSectionItems, } from './proposal-lifecycle.js';
...
import { DELTA_TO_RESOURCE, classifyProposalDeltas, DeltaScanUnreadableError, } from './delta-classify.js';
...
import { canonicalTargetFromDeltaPath, evaluateBaselineClosure, } from './baseline-closure.js';
 ...
 *   行内代码、代码围栏中的引用不得命中。
 */
export function validateMarkdownDelta(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (scan.masked[i])
            continue; // 围栏/缩进代码/HTML 注释内的引用不构成权威结构、不得命中
        ...
        if (markerMatch) {
            if (markerMatch[1] !== 'REMOVED-ITEMS')
                hasMaterialMarker = true;
            const title = stripInlineCode(markerMatch[2] ?? '').trim();
            if (TEMPLATE_PLACEHOLDER_TITLES.has(title))
                hits.push((markerMatch[2] ?? '').trim());
            continue;
        }
        const body = stripInlineCode(trimmed).trim();
        if (TEMPLATE_PLACEHOLDER_BODY_LINES.has(body))
            hits.push(trimmed);
    }
...
function scanHeadings(lines, masked, text) {
    const out = [];
    for (let i = 0; i < lines.length; i++) {
        if (masked[i])
            continue;
...
function makeTableIdentity(subPathKey, ordinal) {
    return `${subPathKey}${IDENTITY_SEP}${ordinal}`;
}
/** 把表身份键解码为人读描述（供违规消息）。总张数 >1 时补「第 N 张 / 共 M 张」以消歧同路径多表。 */
function describeTableIdentity(identity, countBySubPath) {
 ...
 */
function extractRegistryIds(lines, contextTitle = '', chunkRootIsFirstHeading = false) {
    const flat = new Set();
    const scenarioRows = new Map();
    const scenarioTableCount = new Map();
    const content = lines.join('\n');
    for (const id of extractStructuredTestIds(content))
        flat.add(id);
    const scan = authorityScan(lines);
    ...
    const governing = (tableLine) => {
        ...
        if (rootHeading)
            semanticChain.push(rootHeading.text);
        if (contextTitle)
            semanticChain.push(contextTitle);
    ...
    while (i < lines.length) {
        ...
        if (block.length >= 3 && isTableDelimiterRow(block[1])) {
            ...
            if (schemaOk && contextOk) {
                ...
                for (const row of block.slice(2)) {
                    ...
                    if (ID_PATTERN_REGISTRY.scenarioRow.test(first)) {
                        const set = scenarioRows.get(first) ?? new Set();
                        set.add(identity);
            ...
            if (decisionSchemaOk) {
                for (const row of block.slice(2)) {
                    const first = (tableRowCells(row)[0] ?? '').trim();
                    if (ID_PATTERN_REGISTRY.decisionRow.test(first))
                        flat.add(first);
                }
            }
        }
...
function allIdsOf(x) {
    const out = new Set(x.flat);
    for (const id of x.scenarioRows.keys())
        out.add(id);
    return out;
}
/** fence-aware 解析 delta 段标记块（围栏内示例标记不构成块边界——与 L4 同口径；记录 marker 源行号供源位置排序）。 */
function parseDeltaBlocks(deltaContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (m) {
            if (cur)
                blocks.push(cur);
            cur = { op: m[1], anchor: stripInlineCode(m[2] ?? '').trim() || (m[2] ?? '').trim(), markerLine: i, lines: [] };
            continue;
        }
        if (cur)
 ...
 */
function resolveSectionAnchor(headings, anchor) {
    ...
    const matched = candidates.filter(h => {
        ...
        for (let pi = parents.length - 1; pi >= 0; pi--) {
            ...
            while (ai < ancestors.length) {
                if (stripInlineCode(ancestors[ai].text).trim() === parents[pi]) {
                    found = true;
                    ai++;
                    break;
                }
    ...
    if (matched.length === 1)
        return { status: 'ok', hit: matched[0], candidates };
 ...
 */
export function evaluateDeltaConservation(deltaContent, targetContent) {
    ...
    const entries = [];
    let ord = 0;
    const emit = (line, v) => { entries.push({ line, ord: ord++, v }); };
    ...
    for (const b of anchored)
        if (!anchors.includes(b.anchor))
            anchors.push(b.anchor);
    for (const anchor of anchors) {
        ...
        const retained = {
            flat: new Set(), scenarioRows: new Map(), scenarioTableCount: new Map(),
        };
        for (const b of modifiedBlocks) {
            ...
            for (const id of r.flat)
                retained.flat.add(id);
            for (const [id, idents] of r.scenarioRows) {
                const set = retained.scenarioRows.get(id) ?? new Set();
        ...
        if (!wholeRemoved && hasModified) {
            ...
            for (const id of existingAll) {
                if (named.has(id))
                    continue;
                const emitMissing = (formDetail) => emit(modifiedBlocks[0].markerLine, {
                    code: 'delta_implicit_id_removal',
                    anchor,
                ...
                if (identities) {
                    const kept = retained.scenarioRows.get(id) ?? new Set();
 ...
 */
export function resolveModifiedSectionKeys(deltaContent, targetContent) {
    if (targetContent === null)
        return [];
    ...
    for (const b of blocks) {
        ...
        if (r.status === 'ok')
            keys.push(r.hit.line);
    }
    return keys;
}
/** delta 相对路径（`deltas/<category>/<rest>`）→ 项目根相对目标路径；无法映射（unknown 类别 / 根级直放）→ null。 */
export function deltaTargetProjectPath(relativePath) {
 ...
 */
export function readProposalModuleHeader(proposalDir) {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (scan.masked[i])
                continue; // r2 F11：围栏/缩进代码/HTML 注释内的示例头不构成权威 module 声明
 ...
 */
export function resolveProposalModuleContext(root, proposalDir, slug) {
 ...
 */
export function computeDecisionRecordWarnings(proposalContent, tasksContent, hasDecisionsDeltaEntry) {
...
export function isDangerousSlug(slug) {
    if (!slug)
        return true;
    if (slug === '.' || slug === '..')
        return true;
...
function pushViolation(acc, check, v) {
    const flowReason = FLOW_REASON_MAP[v.code];
    const out = flowReason ? { ...v, flow_reason: flowReason } : v;
 ...
 */
export function runChangeLint(root, proposalDir, slug) {
    try {
        const locked = withRecoveredReadLocks(root, new Date().toISOString(), listBaselineSeedModuleIds(root), () => runChangeLintLocked(root, proposalDir, slug));
        if (!locked.ok) {
            return {
                ok: false,
...
function runChangeLintLocked(root, proposalDir, slug) {
    let proposalContent = '';
    let tasksContent = '';
    ...
    try {
        if (existsSync(proposalPath))
            proposalContent = readFileSync(proposalPath, 'utf-8');
    }
    catch {
    ...
    const acc = { violations: [], seq: new Map(), order: new Map() };
    ...
    const clarification = evaluateProposalClarification(proposalContent, resolveProposalDeploymentDecision(proposalDir).deployment_required);
    ...
    if (guiActive) {
        ...
        if (!structure.ok) {
            ...
            if (outcome.code !== 0) {
                const code = CHANGE_LINT_VIOLATION_CODES.includes(outcome.errorCode ?? '')
                    ? outcome.errorCode
                    : 'design_system_mode_invalid';
                pushViolation(acc, 7, {
                    code,
    ...
    const sectionWriters = new Map(); // `${targetRel}#${sectionLine}` → 写者 delta relPath 列表（code-r1 F1 跨文件单写者）
    ...
    const sorted = [...acc.violations].sort((a, b) => {
        ...
        if (oa.check !== ob.check)
            return oa.check - ob.check;
        if (a.path !== b.path)
    ...
    const countFor = (check) => sorted.filter(v => acc.order.get(v).check === check).length;
    const checks = [
...
//# sourceMappingURL=change-lint.js.map