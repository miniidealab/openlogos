...
import { extractTaskSectionItems, isCodeRequiredForProposal, isTasksCodeFilled, isTasksTemplateFilled, parseTaskSections, resolveProposalDeploymentDecision, countMergeableDeltaFiles, } from './proposal-lifecycle.js';
import { PLAN_PACKAGE_CONTRACT_VERSION, PLAN_PACKAGE_SCHEMA, evaluateProposalStructure, sortCompletionIssues, } from './plan-package-contract.js';
function projectRelative(root, path) {
    return relative(root, path).replace(/\\/g, '/');
}
function taskIssue(code, path, message, fixHint, extra = {}) {
    return { code, path, message, fix_hint: fixHint, ...extra };
}
export function evaluatePlanPackage(root, proposalDir, locale) {
...
//# sourceMappingURL=plan-package.js.map