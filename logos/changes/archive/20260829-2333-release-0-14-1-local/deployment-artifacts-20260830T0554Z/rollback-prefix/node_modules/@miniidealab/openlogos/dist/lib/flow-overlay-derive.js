...
import { loadFlow, readOverlay, FlowError, fanoutDone, readProjectCmdTimeout } from './flow.js';
import { listFiles } from './list-files.js';
import { parseTaskSections, isProposalTemplateFilled, isTasksTemplateFilled, isTasksCodeFilled, resolveProposalDeploymentDecision, SLICES_APPROVED_MARKER, } from './proposal-lifecycle.js';
 ...
 */
export const STEP_TO_CURRENT_BUILTIN = {
...
function evalPredicate(pred, node, ctx) {
...
function validateAddedPredicate(node, ctx, pred, field) {
    // 白名单：未知/拼错谓词一律拒绝（避免静默永久 active）——spec §10.3/§12.1
    if (!isKnownPredicate(pred)) {
        throw new FlowError('FLOW_SCHEMA_INVALID', `overlay-add 节点 \`${node.id}\` 的 ${field} \`${pred}\` 非支持的谓词（不接受未知谓词）`);
    }
    ...
    if (launchedOnly && ctx.lifecycle !== 'launched') {
        throw new FlowError('FLOW_SCHEMA_INVALID', `overlay-add 节点 \`${node.id}\` 的 ${field} \`${pred}\` 仅 launched 可用（initial 无提案目录，请改用 file:/dir_nonempty）`);
    }
...
function validateAddedNodePredicate(node, ctx) {
...
function hasCmdPredicate(node) {
    return Boolean(node.done_when?.startsWith('cmd:') || node.fail_when?.startsWith('cmd:'));
}
/** 取节点的 cmd 谓词（fail_when 优先于 done_when，G4）。无则 null。 */
function cmdPredicateOf(node) {
    if (node.fail_when?.startsWith('cmd:'))
        return { field: 'fail_when', pred: node.fail_when };
    if (node.done_when?.startsWith('cmd:'))
        return { field: 'done_when', pred: node.done_when };
    return null;
}
/**
 ...
 */
function addNodeState(node, skipped, ctx, cmdEval) {
    if (skipped)
        return 'skipped';
    // 非 cmd 的 fail_when 优先（G4）：命中即 failed，无需执行 cmd
    if (node.fail_when && !node.fail_when.startsWith('cmd:') && evalPredicate(node.fail_when, node, ctx))
        return 'failed';
    const cmd = cmdPredicateOf(node);
    if (cmd) {
        if (cmdEval && cmdEval.node_id === node.id) {
            if (cmd.field === 'fail_when') {
                if (cmdEval.satisfied)
                    return 'failed';
                // fail_when:cmd 未命中 → 评（非 cmd 的）done_when
                if (node.done_when && !node.done_when.startsWith('cmd:') && evalPredicate(node.done_when, node, ctx))
                    return 'done';
                return 'active';
            }
    ...
    if (node.done_when && evalPredicate(node.done_when, node, ctx))
        return 'done';
    return 'active';
}
/** launched 提案级 when 标志（delta_required / deployment_required / smoke_required）。 */
function launchedWhenFlags(proposalDir, mod) {
    if (!proposalDir)
        return { delta_required: false, deployment_required: false, smoke_required: false };
    const tasks = existsSync(join(proposalDir, 'tasks.md')) ? readFileSync(join(proposalDir, 'tasks.md'), 'utf-8') : '';
    const sections = parseTaskSections(tasks);
...
function evalLaunchedWhen(when, flags) {
    if (!when)
        return true;
    const e = when.trim();
 ...
 */
function buildAnchorMap(root, lifecycle) {
    const map = new Map();
 ...
 */
function isAnchorSkipped(addId, anchorMap, ordered, isBuiltinSkipped, seen = new Set()) {
    const anchorId = anchorMap.get(addId);
    if (!anchorId || seen.has(addId))
        return false;
    seen.add(addId);
    const e = ordered.find(x => x.node.id === anchorId);
    if (!e)
        return false;
    if (e.node.overlay_op === 'add')
        return isAnchorSkipped(anchorId, anchorMap, ordered, isBuiltinSkipped, seen);
 ...
 */
export function deriveOverlayView(root, mod, scenarios, proposalDir, isMultiModule = false, cmdEval, haltAfterSubflow) {
    const lifecycle = mod.lifecycle;
    const loaded = loadFlow(root, { lifecycle, resolved: true });
    if (!loaded.overlay_applied)
        return null; // 无 overlay → 不新增字段
    const flow = loaded.flow;
    const ctx = { root, lifecycle, mod, proposalDir, scenarios, isMultiModule };
        ...
        for (const sub of flow.subflows) {
            for (const node of sub.nodes) {
                if (node.overlay_op === 'skip' || node.overlay_op === 'reorder') {
                    throw new FlowError('FLOW_SCHEMA_INVALID', `launched 流程的内置节点不支持 overlay ${node.overlay_op}（节点 \`${node.id}\`）；本切片仅 add / modify 生效`);
                }
            }
        }
    }
    ...
    for (const sub of flow.subflows) {
        for (const node of sub.nodes) {
            if (node.overlay_op === 'add')
                validateAddedNodePredicate(node, ctx);
            ...
            // 其它内置节点带 cmd: 仍 fail loud（兜底，正常应已在 applyOverlay 拦截）。
                ...
                throw new FlowError('FLOW_SCHEMA_INVALID', `cmd: 谓词不支持内置节点 \`${node.id}\`（仅 overlay-add 节点或 overlay-modify 的 verify/deploy/smoke gate）`);
            }
        }
    }
    ...
    for (const sub of flow.subflows) {
        for (const node of sub.nodes)
            ordered.push({ node, subflowId: sub.id, subflowWhen: sub.when ?? null, index: idx++ });
    }
    ...
    const reachedAdded = [];
    let current = null;
    let proposalStepOverride = null;
    if (lifecycle === 'launched') {
        // launched：以 detectProposalStepViaFlow 为权威前沿，不重复推导 launched builtin 的 when/done。
        const step = proposalDir ? detectProposalStepViaFlow(proposalDir, mod) : 'writing';
        ...
        const launchedBuiltinSkipped = (e) => !evalLaunchedWhen(e.node.when, flags) || !evalLaunchedWhen(e.subflowWhen, flags);
        let passedBuiltin = false;
        for (const entry of ordered) {
            if (frontierIdx !== -1 && entry.index > frontierIdx)
                break; // 前沿之后：未到达
            if (haltMaxIdx >= 0 && entry.index > haltMaxIdx)
                break; // F3：loop 未收敛 → 不越过 implement
            const { node } = entry;
            if (node.overlay_op === 'add') {
                ...
                if (entry.index === frontierIdx) {
                    current = { ...entry, state: 'active' };
                    break;
                } // builtin 前沿即当前
        ...
        const initialBuiltinSkipped = (e) => e.node.skipped === true || !evalNodeWhen(e.node.when, mod);
        for (const entry of ordered) {
            if (haltMaxIdx >= 0 && entry.index > haltMaxIdx)
                break; // F3：loop 未收敛 → 不越过 implement
            const { node } = entry;
            ...
            if (node.overlay_op === 'add') {
                ...
                if (skipped)
                    state = 'skipped';
                else if (node.fail_when && evalPredicate(node.fail_when, node, ctx))
                    state = 'failed';
                else if (node.done_when && evalPredicate(node.done_when, node, ctx))
                    state = 'done';
                else
                    state = 'active';
            }
...
export function isBuiltinNodeId(id) {
...
function requireMaterializedDispatch(n) {
    const d = n.dispatch;
...
function buildRequiresReviewed(n, proposalDir) {
    if (!n.requires_reviewed || n.requires_reviewed.length === 0)
        return null;
    ...
    if (proposalDir && list.includes('delta')) {
        ...
        if (!hasDeltaTasks)
            list = list.filter(x => x !== 'delta');
    }
    return list.length > 0 ? { requires_reviewed: list } : null;
}
export function resolveNextNode(root, mod, opts) {
    if (opts.gateAutoPassed)
        return null; // R4：gate 自动放行 → 省略
    let targetId;
    if (opts.loopBlocking) {
        if (opts.loopEscalated)
            return null; // R7：达上限 → 省略（宿主读 loop_state）
    ...
    if (!targetId)
        return null; // R5 / all_done：无对应 flow 节点 → 省略
...
//# sourceMappingURL=flow-overlay-derive.js.map