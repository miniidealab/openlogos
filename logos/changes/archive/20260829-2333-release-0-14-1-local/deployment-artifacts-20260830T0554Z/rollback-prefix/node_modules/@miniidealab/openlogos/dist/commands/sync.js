...
import { bundledManifestPath, readBundledAssetManifest, validateAssetManifest, writeSyncStamp, } from '../lib/asset-manifest.js';
import { dirname } from 'node:path';
export function syncLogosProjectName(root, projectName) {
    const yamlPath = join(root, 'logos', 'logos-project.yaml');
    if (!existsSync(yamlPath))
        return false;
    const content = readFileSync(yamlPath, 'utf-8');
    const updated = content.replace(/^(\s*name:\s*)"[^"]*"/m, `$1"${projectName}"`);
    if (updated !== content) {
 ...
 */
export function syncScenariosModuleField(root) {
    const yamlPath = join(root, 'logos', 'logos-project.yaml');
    if (!existsSync(yamlPath))
        return 0;
    let yaml;
    ...
    if (existsSync(scenImplDir)) {
        try {
            for (const f of readdirSync(scenImplDir)) {
                if (statSync(join(scenImplDir, f)).isFile())
                    scenImplFiles.push(f);
            }
        }
        catch { /* ignore */ }
    }
    ...
    for (const scenario of scenarios) {
        if (scenario.module !== undefined)
            continue; // already set, skip
        // Find which module has a file matching <moduleId>-<scenarioId>-*.md
        const matched = modules.find(modId => scenImplFiles.some(f => f.startsWith(`${modId}-${scenario.id}-`) || f.startsWith(`${modId}-${scenario.id}.`)));
...
export function sync() {
    ...
    const syncInterval = withRecoveredReadLocks(root, new Date().toISOString(), listProjectModuleIds(root), () => {
        ...
        if (existsSync(yamlPath)) {
            try {
                ...
                if (Array.isArray(yaml?.modules)) {
                    isLaunched = yaml.modules.some(m => m.lifecycle === 'launched');
                }
            }
            catch { /* ignore */ }
        }
        ...
        if (missingProductType.length > 0) {
            console.warn(`  ⚠ PRODUCT_TYPE_CONFIRMATION_REQUIRED: ${missingProductType.length} launched module(s) missing product_type: ${missingProductType.join(', ')}`);
