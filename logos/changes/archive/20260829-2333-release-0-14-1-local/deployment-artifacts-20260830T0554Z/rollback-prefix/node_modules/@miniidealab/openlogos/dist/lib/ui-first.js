...
export const PRODUCT_TYPE_ENUM = ['web', 'desktop', 'mobile', 'cli', 'api', 'library', 'skills', 'service'];
/** GUI 产品类型集合 = {web,desktop,mobile}。 */
export const GUI_PRODUCT_TYPES = new Set(['web', 'desktop', 'mobile']);
/** 方法论 GUI overlay 注入的两个节点 id（按此识别以幂等注入/移除、保留用户 ops）。 */
export const GUI_OVERLAY_NODE_IDS = ['write-ui-prototype', 'verify-ui-provenance'];
export function isValidProductType(pt) {
    return typeof pt === 'string' && PRODUCT_TYPE_ENUM.includes(pt);
}
/** GUI 判定：product_type ∈ {web,desktop,mobile}；缺失/非法一律非 GUI（安全默认）。 */
export function isGuiProductType(pt) {
    return typeof pt === 'string' && GUI_PRODUCT_TYPES.has(pt);
}
/** 取某 module 的 product_type（缺失返回 undefined）。 */
export function moduleProductType(mod) {
    return mod?.product_type;
}
/** 项目是否含 ≥1 个 GUI 模块（overlay 项目实例级注入判据）。 */
export function projectHasGuiModule(data) {
 ...
 */
export function modulesMissingProductType(data) {
    const mods = data?.modules ?? [];
    return mods
...
export function readModulesMissingProductType(root) {
 ...
 */
export function locateUiDeclarationYaml(proposalMd) {
    ...
    for (let i = start + 1; i < lines.length; i++) {
        if (fenceStart === -1) {
            if (!scan.masked[i] && /^#{1,6}\s/.test(scan.text[i]))
                break; // 段结束、无权威 fence
    ...
    if (fenceStart === -1 || fenceEnd === -1)
        return { found: true, yamlText: null };
    return { found: true, yamlText: lines.slice(fenceStart + 1, fenceEnd).join('\n') };
}
export function parseUiUxDeclaration(proposalMd) {
    const empty = { ui_impact: false, pages: [], present: false };
    if (!proposalMd)
        return empty;
...
export function readUiUxDeclaration(proposalDir) {
    const p = join(proposalDir, 'proposal.md');
    if (!existsSync(p))
        return { ui_impact: false, pages: [], present: false };
 ...
 */
export function analyzeUiDeclarationStructure(proposalMd) {
 ...
 */
export function deriveUiImpact(root, moduleId, proposalDir) {
    const data = readProjectYaml(root).data;
    const mod = (data?.modules ?? []).find(m => m.id === moduleId);
    if (!isGuiProductType(mod?.product_type))
        return false;
 ...
 */
export function isValidPrototypeBasename(s) {
    if (typeof s !== 'string' || s.length === 0)
        return false;
    if (s.includes('/') || s.includes('\\'))
        return false;
    if (s.split(/[\\/]/).some(seg => seg === '..') || s.includes('..'))
        return false;
    return BASENAME_RE.test(s);
}
// ── GUI overlay 读取与项目实例注入/移除 ──
/** 定位随 CLI 分发的 spec 根（dev：仓库根 spec/；打包：cli/spec/）。 */
export function resolveSpecRoot(root) {
    const candidates = [
        join(root, 'spec'),
    ...
    for (const c of candidates) {
        if (existsSync(join(c, 'flow', 'overlays', 'gui-ui-first.yaml')))
            return c;
    }
 ...
 */
export function readSessionCapabilities(root) {
 ...
 */
export function buildCapabilities(root) {
 ...
 * 由验收结果机器判定，非人工声称。
 */
export function evaluateReleaseStatus(input) {
...
function readInstanceFlow(root) {
 ...
 * 返回是否发生写入变更。
 */
export function injectGuiOverlay(root) {
    ...
    if (typeof doc.extends !== 'string')
        doc.extends = 'builtin:launched@v1';
    const existing = Array.isArray(doc.overlay) ? doc.overlay : [];
    const guiIds = new Set(GUI_OVERLAY_NODE_IDS);
    const present = new Set(existing.map(overlayOpNodeId).filter(Boolean));
    let changed = false;
 ...
 * 返回是否发生写入变更。
 */
export function removeGuiOverlay(root) {
    const { path, doc } = readInstanceFlow(root);
    if (!existsSync(path))
        return false;
    const existing = Array.isArray(doc.overlay) ? doc.overlay : [];
...
export function instanceGuiOverlayNodeIds(root) {
    const { doc } = readInstanceFlow(root);
    const existing = Array.isArray(doc.overlay) ? doc.overlay : [];
    const guiIds = new Set(GUI_OVERLAY_NODE_IDS);
    return existing.map(overlayOpNodeId).filter((id) => Boolean(id) && guiIds.has(id));
}
/**
 * sync 时按 product_type 幂等对齐 overlay：项目含 ≥1 GUI 模块 → 注入；否则移除。
 * 返回 'injected' | 'removed' | 'unchanged'。
 */
export function syncGuiOverlay(root) {
    const data = readProjectYaml(root).data;
...
//# sourceMappingURL=ui-first.js.map