...
import { buildInitialPhasePlan, deriveModulePhaseProgressViaFlow, flowExplicitSkipPhaseKeys, detectProposalStepViaFlow, detectMintedStepViaFlow, deriveLaunchedCmdGate } from '../lib/flow-derive.js';
...
import { withBaselineReadLock, withRecoveredReadLocks, listBaselineSeedModuleIds, listRunIds, readRunRecord, BaselineCommitInProgressError, } from '../lib/baseline-seed-txn.js';
...
import { readMergeTransactionIfPresent } from '../lib/merge-transaction.js';
function commitInProgressCoverage(seedState) {
    return {
 ...
 */
function buildAdoptedCoverage(root, moduleId, seedState, hasActiveProposal, baselineIndex, opts) {
    const compute = () => {
...
import { canConsumeAutomationDiagnosticAtStep, deriveAutomationDiagnostic } from '../lib/automation-diagnostic.js';
// proposal-lifecycle 函数簇已下沉到 ../lib/proposal-lifecycle.js（断开与 flow-derive.ts 的运行时循环依赖）。
import { countTasks, countMergeableDeltaFiles, parseTaskSections, extractTaskSectionItems, readTaskSectionItems, getDeployTasks, parseProposalDeploymentDecision, resolveDeploymentProgress, resolveDeploymentDocument, resolveProposalDeploymentDecision, detectProposalStep, getCodePlanningDiagnostic, getProposalStepReason, derivePlanState, isCodeRequiredForProposal, deriveProposalFacts, } from '../lib/proposal-lifecycle.js';
 ...
 */
export function buildProductTypeConfirmation(missingIds) {
...
const SKIP_PHASE_MAP = {
    api: 'phase.3-2-api',
    database: 'phase.3-2-db',
    scenario: 'phase.3-4b',
};
function deriveExplicitSkipPhaseKeys(mod) {
...
export function deriveModulePhaseProgress(root, mod, scenarios, isMultiModule = false) {
    const progress = {};
    ...
    for (let i = 0; i < lastDoneIdx; i++) {
        if (!progress[keys[i]].done && !NON_FALLBACK_SKIP_PHASES.has(keys[i]))
...
function buildModuleStatusItem(root, mod, scenarios, locale, guardActiveChange, guardModule, isMultiModule = false, cmdEval, cmdGateEval, baselineIndex) {
    if (mod.lifecycle === 'launched') {
        let activeChange = null;
        // brownfield-adopter（S33）：adopted 且无活跃提案时的现状基线状态与覆盖率（否则保持 undefined）。
        let baselineSeedState;
        ...
        if (guardActiveChange && guardModule === mod.id) {
            ...
            const stepLabel = t(locale, `status.proposalStep.${step}`);
        ...
        if (activeChange && sliceVerificationState) {
            ...
            if (sliceVerificationState.reason)
                activeChange.reason = sliceVerificationState.reason;
        }
        ...
        if (activeChange) {
            ...
            if (gatedStep && gatedStep !== activeChange.proposal_step) {
                ...
                activeChange.proposal_step_label = t(locale, `status.proposalStep.${gatedStep}`);
        ...
        if (activeChange) {
            ...
            if (!derived.ok) {
                ...
                if (derived.value.seed !== undefined)
                    baselineSeedState = derived.value.seed;
                if (derived.value.cov !== undefined)
                    baselineCoverage = derived.value.cov;
        ...
        if (overlay && activeChange && overlay.proposal_step_override) {
            ...
            const mintedOverride = mintStep(overlay.proposal_step_override); // 覆盖点唯一铸造（C1/F5）
            activeChange.proposal_step = mintedOverride.proposal_step;
            activeChange.proposal_step_label = t(locale, `status.proposalStep.${overlay.proposal_step_override}`);
    ...
    const currentPhaseLabel = currentPhase ? t(locale, currentPhase) : null;
    let suggestion;
    if (!currentPhase) {
    ...
    const vIdx = PHASE_KEYS.indexOf('phase.3-6');
    const initialAtVerify = derived.currentPhase === null
        || PHASE_KEYS.indexOf(derived.currentPhase) >= vIdx;
 ...
 */
function gateInitialPhasesForLoop(progress, currentPhase, loopState) {
    if (!loopState || loopState.converged)
        return { progress, currentPhase }; // 出环用 converged 裁决（含 iteration=0 + 旧 report）
    const VERIFY = 'phase.3-6';
    const next = { ...progress };
    if (next[VERIFY])
        next[VERIFY] = { ...next[VERIFY], done: false };
    const keys = PHASE_KEYS;
 ...
 */
function loopHaltSubflow(loopState) {
...
const STEP_PAST_VERIFY = new Set([
    'verify-passed', 'ready-to-deploy', 'deploy-done', 'ready-to-smoke', 'smoke-passed', 'smoke-failed',
]);
function gateLaunchedStepForLoop(step, loopState) {
    if (!loopState || loopState.converged || !step)
        return step;
 ...
 */
function loopStateForOutput(ls, step) {
    if (!ls || !step)
        return ls;
...
function computeProjectLoopState(root, rawModules, lifecycle, activeChange, guardModule) {
    if (rawModules === undefined) {
        const synthMod = { id: 'core', name: 'core', lifecycle };
...
function computeProjectSliceState(root, rawModules, lifecycle, activeChange, guardModule) {
    if (rawModules === undefined) {
        const synthMod = { id: 'core', name: 'core', lifecycle };
        const proposalDir = activeChange ? join(root, 'logos', 'changes', activeChange) : null;
 ...
 */
export function deriveActiveOverlay(root, filterModuleId, cmdEval) {
    ...
    const rawModules = Array.isArray(projectYaml.data?.modules)
    ...
    if (rawModules) {
        ...
        for (const mod of rawModules) {
            ...
            if (gates) {
                if (typeof gates.deployment_required === 'boolean')
                    mod.deployment_required = gates.deployment_required;
                if (typeof gates.smoke_required === 'boolean')
                    mod.smoke_required = gates.smoke_required;
            }
        }
    }
    const scenarios = Array.isArray(projectYaml.data?.scenarios)
    ...
    if (rawModules === undefined) {
        const lifecycle = activeChange ? 'launched' : 'initial';
        const synthMod = { id: 'core', name: 'core', lifecycle };
        const proposalDir = activeChange ? join(root, 'logos', 'changes', activeChange) : null;
        return deriveOverlayView(root, synthMod, scenarios, proposalDir, false, cmdEval, loopHaltSubflow(deriveLoopState(root, synthMod, proposalDir, false)));
    }
    ...
    return deriveOverlayView(root, target, moduleScenarios, proposalDir, isMultiModule, cmdEval, loopHaltSubflow(deriveLoopState(root, target, proposalDir, isMultiModule)));
}
/**
 ...
 */
export function collectStatusData(root, filterModuleId, cmdEval, cmdGateEval) {
    const locked = withRecoveredReadLocks(root, new Date().toISOString(), listBaselineSeedModuleIds(root), () => collectStatusDataLocked(root, filterModuleId, cmdEval, cmdGateEval));
    if (!locked.ok) {
        throw new BaselineCommitInProgressError(`baseline_commit_in_progress — 模块 ${locked.inProgress.join(', ')} 的现状基线提交进行中`);
    }
    return locked.value;
}
function collectStatusDataLocked(root, filterModuleId, cmdEval, cmdGateEval) {
    ...
    const globalSkipPhaseKeys = new Set();
    ...
    const phases = phasePlan.map(item => ({
        key: item.phaseKey,
    ...
    for (const phase of phases) {
        if (phase.skipped)
            continue; // don't scan explicitly skipped dirs
    ...
    for (let i = 0; i < lastDoneIdx; i++) {
        if (!phases[i].done && !NON_FALLBACK_SKIP_PHASES.has(phases[i].key))
    ...
    if (rawModules === undefined && activeChange)
        lifecycle = 'launched';
    ...
    if (allDone) {
        ...
        const topRes = withBaselineReadLock(root, topMod.id, new Date().toISOString(), () => effectiveBaselineSeedState(root, topMod.id, topMod?.baseline_seed_state, { assumeLocked: true }).state);
        if (!topRes.ok) {
            suggestion = locale === 'zh'
    ...
    const legacyAtVerify = !firstIncomplete
        || PHASE_KEYS.indexOf(firstIncomplete.key) >= PHASE_KEYS.indexOf('phase.3-6');
    ...
    if (rawModules === undefined) {
        ...
        if (topOverlay && topOverlay.proposal_step_override && proposalStep) {
            proposalStep = topOverlay.proposal_step_override;
        }
    }
    ...
    return {
        ...
        contract: { version: contractVersion((modules ?? []).some(m => m.features !== undefined), (modules ?? []).some(m => m.active_change?.plan_state?.clarification !== undefined)
                || topPlanState?.clarification !== undefined, (modules ?? []).some(m => m.slice_verification_state !== undefined), (modules ?? []).some(m => m.active_change?.plan_state?.plan_package !== undefined)
                || topPlanState?.plan_package !== undefined, mergeTransaction !== null) },
...
export function status(format = 'text', moduleId) {
    const root = process.cwd();
    const configPath = join(root, 'logos', 'logos.config.json');
    if (!existsSync(configPath)) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('status', 'PROJECT_NOT_INITIALIZED', 'logos/logos.config.json not found.')));
            process.exit(1);
        }
