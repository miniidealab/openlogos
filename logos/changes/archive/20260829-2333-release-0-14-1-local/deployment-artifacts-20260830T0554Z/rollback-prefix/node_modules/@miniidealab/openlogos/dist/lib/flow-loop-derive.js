...
import { loadFlow, findActivatedLoop } from './flow.js';
 ...
 */
export function deriveLoopState(root, mod, proposalDir, isMultiModule = false, flow) {
    ...
    if (mod.lifecycle === 'initial' && isMultiModule)
        return null;
    ...
    const state = { subflow_id: act.subflow_id, until: act.until, max_iters: act.max_iters, iteration, converged, escalated };
    // activated_at 在 exhausted_skippable 之前挂出（spec/cli-json-output.md §3.9 字段表序）；旧空 marker → 省略
    if (activatedAt)
        state.activated_at = activatedAt;
    ...
    if (eg != null)
        state.exhausted_skippable = eg.skippable ?? false;
    return state;
}
/**
 ...
 */
function readCodeSection(proposalDir) {
...
function extractCodeSlices(content) {
    const slices = [];
 ...
 */
export function deriveSliceState(proposalDir, tasksContent) {
 ...
 */
export function deriveSliceStateIfActive(root, mod, proposalDir, isMultiModule = false, flow) {
    const resolved = flow ?? loadFlow(root, { lifecycle: mod.lifecycle, resolved: true }).flow;
    const act = findActivatedLoop(resolved);
    if (!act || act.until !== 'code_slices_green')
        return null;
...
export function loopExhaustedGateId(subflowId) {
 ...
 */
export function isLoopBlocking(loopState, atVerifyFrontier) {
    return Boolean(loopState && !loopState.converged && loopState.iteration >= 1 && atVerifyFrontier);
}
//# sourceMappingURL=flow-loop-derive.js.map