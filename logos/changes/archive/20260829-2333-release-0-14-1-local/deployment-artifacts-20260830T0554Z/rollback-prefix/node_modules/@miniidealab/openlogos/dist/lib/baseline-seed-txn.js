 ...
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync, renameSync, rmSync, readdirSync, copyFileSync, statSync, lstatSync, realpathSync, linkSync, } from 'node:fs';
...
export const REQUIRED_KINDS = ['system-map', 'scenario-candidates'];
export const KIND_ENUM = ['system-map', 'scenario-candidates', 'dependency-map', 'entry-points'];
/**
 * 未终结 journal 无法在读前确定性恢复时的统一硬错误。读取方必须中止，不能降级为 partial 后继续。
 */
export class BaselineCommitInProgressError extends Error {
    code = 'baseline_commit_in_progress';
...
export function runDir(root, runId) { return join(runsRoot(root), runId); }
export function stagingDir(root, runId) { return join(runDir(root, runId), 'staging'); }
/** F4：resolved 区——commit 把「对账后最终内容」持久化于此，供事务提交与崩溃恢复（roll-forward）复现全新集合；staging 保持纯净。 */
export function resolvedDir(root, runId) { return join(runDir(root, runId), 'resolved'); }
export function backupDir(root, runId) { return join(runDir(root, runId), 'backup'); }
export function journalPath(root, runId) { return join(runDir(root, runId), 'commit-journal.json'); }
export function runRecordPath(root, runId) { return join(runDir(root, runId), 'run.json'); }
export function lockPath(root, moduleId) { return join(runsRoot(root), `${moduleId}.commit.lock`); }
export function eventsPath(root) { return join(root, 'logos', 'resources', 'verify', 'baseline-events.jsonl'); }
...
function yamlPath(root) { return join(root, 'logos', 'logos-project.yaml'); }
 ...
 */
export function validateTargetPaths(root, expected) {
    ...
    for (const item of expected) {
        ...
        if (!norm.startsWith('logos/resources/') || !norm.endsWith('.md'))
            return { ok: false, error: 'path_escape' };
        ...
        try {
            ...
            while (ancestor !== root && !existsSync(ancestor))
...
export function validateManifest(root, manifest) {
    ...
    const rawExpected = manifest.expected;
    ...
    for (const raw of rawExpected) {
        if (!raw || typeof raw !== 'object')
            return { ok: false, error: 'invalid_manifest' };
        const r = raw;
        if (typeof r.kind !== 'string' || typeof r.target_path !== 'string')
            return { ok: false, error: 'invalid_manifest' };
        if (!KIND_ENUM.includes(r.kind))
            return { ok: false, error: 'invalid_manifest' };
        ...
        const keys = Array.isArray(r.candidate_keys) ? r.candidate_keys : [];
    ...
    for (const req of REQUIRED_KINDS) {
        if (!kinds.has(req))
            return { ok: false, error: 'missing_required_kind' };
    }
    ...
    for (const e of expected) {
        for (const k of e.candidate_keys) {
            if (seenKeys.has(k))
                return { ok: false, error: 'candidate_key_conflict' };
            seenKeys.add(k);
        }
    }
...
let lockTokenSeq = 0;
function lockHeldByCurrentProcess(root, moduleId) {
...
export function acquireLock(root, moduleId) {
    ...
    try {
        ...
        for (let attempt = 0; attempt < 8; attempt++) {
            ...
            if (owner === 'unparseable')
                return false;
            if (typeof owner.pid !== 'number')
                return false;
            if (owner.pid === process.pid)
                return false; // 本进程已持有 → 公共 acquire 仍拒绝重入
            ...
            if (!holdMarker) {
                ...
                if (rc !== 'unparseable' && typeof rc.pid === 'number' && !isProcessAlive(rc.pid)) {
                    ...
                    try {
                        renameSync(marker, evict);
                    }
                    catch { /* 他人已移走 */ }
                    try {
                        rmSync(evict, { force: true });
                    }
                    catch { /* ignore */ }
                }
                continue; // 重试仲裁
        ...
        catch { /* ignore */ } // 未被 rename 消费时清理 tmp 名
        try {
            rmSync(rcTmp, { force: true });
        }
        catch { /* ignore */ }
    }
}
export function releaseLock(root, moduleId) {
    const path = lockPath(root, moduleId);
    const myToken = heldLockTokens.get(moduleId);
    // 仅释放**本进程本次获取**（pid + token 双匹配）的锁——若锁已被回收并由他人重建（token/pid 不符），不误删。
    try {
        const owner = JSON.parse(readFileSync(path, 'utf-8'));
...
function issuedLedgerPath(root, moduleId) {
    return join(runsRoot(root), `.issued-runs-${moduleId}.json`);
}
function issueHash(runId, moduleId, nonce) {
    return sha256(`${runId}:${moduleId}:${nonce}`);
}
export function newIssuedNonce() { return randomBytes(16).toString('hex'); }
function readIssuedLedger(root, moduleId) {
...
export function recordIssuedRun(root, runId, moduleId, nonce) {
...
export function isIssuedRun(root, runId, moduleId, nonce) {
    if (typeof nonce !== 'string' || nonce === '')
        return false;
    const ledger = readIssuedLedger(root, moduleId);
    return ledger[runId] === issueHash(runId, moduleId, nonce);
}
export function readRunRecord(root, runId) {
...
function readJournalStrict(root, runId) {
    const p = journalPath(root, runId);
    if (!existsSync(p))
        return null;
    let raw;
    ...
    const index = j.index;
    const transition = j.state_transition;
    const states = new Set([null, 'required', 'partial', 'seeded']);
...
export function findUnfinalizedJournal(root, moduleId) {
    let found = null;
...
export function writeSeedState(root, moduleId, state, indexEntry) {
...
export function readSeedState(root, moduleId) {
    const doc = readYamlDoc(root);
    const modules = Array.isArray(doc.modules) ? doc.modules : [];
    const mod = modules.find(m => m.id === moduleId);
    const v = mod?.baseline_seed_state;
    if (v === 'required' || v === 'partial' || v === 'seeded')
        return v;
    if (mod?.baseline_seed_required === true)
        return 'required';
    return null;
}
/** 从（已就位的）目标文档计算模块派生索引条目。 */
export function computeIndexEntry(root, moduleId, at) {
...
export function appendEventOnce(root, event) {
    const p = eventsPath(root);
    if (existsSync(p)) {
 ...
 */
export function commitSeededTransaction(root, moduleId, runId, targets, fromState, at, keys = []) {
    ...
    if (existsSync(yp))
        copyFileSync(yp, yBackup);
    const journalTargets = targets.map(t => ({
...
function resolvedIntact(root, runId, journal) {
    for (const t of journal.targets) {
        const rp = join(resolvedDir(root, runId), t.target_path);
        if (!existsSync(rp) || sha256File(rp) !== t.new_sha256)
            return false;
    }
    return true;
}
 ...
 */
export function recoverJournal(root, runId, at) {
    const journal = readJournalStrict(root, runId);
    if (!journal || journal.phase === 'committed')
        return { recovered: false, outcome: 'none' };
    ...
    for (const t of journal.targets) {
        const abs = join(root, t.target_path);
        if (sha256File(abs) === t.old_sha256)
            continue;
        const bak = join(backupDir(root, runId), t.target_path);
    ...
    for (const t of journal.targets) {
        ...
        if (t.old_sha256 === null) {
            if (existsSync(abs))
                rmSync(abs, { force: true }); // 原本不存在 → 删除新建文件
        }
 ...
 */
export function readGate(root, moduleId, at, opts) {
 ...
 */
export function withBaselineReadLock(root, moduleId, at, fn) {
    // 外层 withRecoveredReadLocks 已持本模块锁时直接复用同一临界区；公共 acquireLock 语义仍保持“拒绝重入”。
    if (lockHeldByCurrentProcess(root, moduleId))
        return { ok: true, value: fn() };
    if (!acquireLock(root, moduleId))
        return { ok: false, error: 'baseline_commit_in_progress' };
    try {
        const pending = findUnfinalizedJournal(root, moduleId);
        if (pending)
...
export function listProjectModuleIds(root) {
    const doc = readYamlDoc(root);
    const modules = Array.isArray(doc.modules) ? doc.modules : [];
    return modules.map(m => (typeof m.id === 'string' ? m.id : null)).filter((x) => x !== null);
}
/** 只有 adopted/历史 skipped 模块允许 baseline-seed 写 resources；普通模块无需创建读锁目录。 */
export function listBaselineSeedModuleIds(root) {
    const doc = readYamlDoc(root);
    const modules = Array.isArray(doc.modules) ? doc.modules : [];
    return modules
 ...
 */
export function withRecoveredReadLocks(root, at, moduleIds, fn) {
    ...
    try {
        for (const m of sorted) {
            currentModule = m;
            if (lockHeldByCurrentProcess(root, m))
                continue;
            if (!acquireLock(root, m))
                return { ok: false, inProgress: [m] };
            held.push(m);
        }
        for (const m of sorted) {
            currentModule = m;
        ...
        if (e instanceof BaselineCommitInProgressError)
            return { ok: false, inProgress: [currentModule] };
        throw e;
    }
    finally {
        for (const m of held)
...
//# sourceMappingURL=baseline-seed-txn.js.map