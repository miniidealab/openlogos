...
function resolveModule(root, moduleArg, locale, slug) {
    ...
    if (modules.length === 1)
        return modules[0].id; // 单模块归属该唯一模块（可能非 core，不硬编码）
    const core = modules.find(m => m.id === 'core');
    if (core)
        return 'core'; // 多模块含 core → 默认挂靠 core
    if (modules.length === 0)
        return 'core'; // 退化：无模块注册，保留既有兜底（不改行为）
...
export function change(slug, moduleArg) {
