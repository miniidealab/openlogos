...
export function checkUiHashMatchCommand(slug, format = 'text') {
    const root = process.cwd();
    if (!existsSync(`${root}/logos/logos.config.json`)) {
        const msg = 'logos/logos.config.json not found.';
        if (format === 'json')
            console.error(JSON.stringify(makeErrorEnvelope(COMMAND, 'PROJECT_NOT_INITIALIZED', msg)));
        else
    ...
    if (res.ok) {
        ...
        if (format === 'json')
            process.stdout.write(JSON.stringify(makeEnvelope(COMMAND, data)) + '\n');
        else
            console.log(res.advisory
...
//# sourceMappingURL=check-ui-hash-match.js.map