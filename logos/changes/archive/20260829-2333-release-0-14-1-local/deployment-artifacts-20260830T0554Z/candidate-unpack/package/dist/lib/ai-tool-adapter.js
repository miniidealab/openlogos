...
export class AiToolAdapterRegistry {
    ...
    get(id) {
        ...
        if (!definition)
            throw new Error(`未知的 AI Adapter id：${id}`);
        return definition;
    }
    has(id) {
...
export function parseRegisteredAiTool(value) {
    if (value === 'all')
        return 'all';
    return aiToolAdapterRegistry.has(value) ? value : undefined;
}
export function expandRegisteredAiTools(rawAiTool) {
 ...
 */
export function resolveConfiguredAiTools(rawAiTool) {
    const rawValues = Array.isArray(rawAiTool) ? rawAiTool : [rawAiTool];
    for (const value of rawValues) {
...
function sourceTreeMatches(source, target) {
    ...
    for (const entry of readdirSync(source)) {
        ...
        if (sourceStat.isDirectory()) {
            if (!sourceTreeMatches(sourceEntry, targetEntry))
                return false;
        }
...
function walkFiles(root, prefix = '') {
    ...
    for (const entry of readdirSync(root)) {
        ...
        if (statSync(full).isDirectory())
            result.push(...walkFiles(full, relative));
        else
            result.push(relative);
    }
    return result.sort();
}
export function validateZCodeTemplate(source) {
    const required = [
    ...
    for (const relative of required) {
        if (!existsSync(join(source, relative)))
            throw new Error(`ZCode 模板缺少：${relative}`);
    }
    const manifest = parseManifest(join(source, '.zcode-plugin', 'plugin.json'));
    if (manifest.name !== 'openlogos')
        throw new Error('ZCode plugin identity 必须为 openlogos');
    parseManifest(join(source, 'hooks', 'hooks.json'));
    for (const file of walkFiles(source).filter(file => /^skills\/[^/]+\/SKILL\.md$/.test(file)
...
export function preflightZCodeTarget(root, source) {
    ...
    if (!existsSync(targetManifest)) {
        ...
        if (entries.length > 0)
            throw new Error(`ZCode 目标存在不完整或未知 owner：${ZCODE_PLUGIN_REL_DIR}`);
        return;
    }
    ...
    commit() {
        ...
        try {
            if (existsSync(this.target))
                cpSync(this.target, this.staging, { recursive: true, force: false });
            else
            ...
            if (existed)
                renameSync(this.target, this.backup);
            ...
            if (existsSync(this.backup))
                rmSync(this.backup, { recursive: true, force: true });
            ...
            if (!existsSync(this.target) && this.backup && existsSync(this.backup))
                renameSync(this.backup, this.target);
            rmSync(transactionRoot, { recursive: true, force: true });
            this.staging = null;
            this.backup = null;
            throw error;
...
export function deployZCodeAssets(root, source, sharedAssets = {}) {
    ...
    if (sharedAssets.commands && existsSync(sharedAssets.commands))
        cpSync(sharedAssets.commands, join(prepared, 'commands'), { recursive: true, force: true });
    if (sharedAssets.agents && existsSync(sharedAssets.agents))
        cpSync(sharedAssets.agents, join(prepared, 'agents'), { recursive: true, force: true });
...
export function localizedZCodeResult(locale, result, newSession = true) {
    const action = locale === 'zh'
        ? { installed: '已安装', updated: '已更新', unchanged: '未变化' }[result.status]
        : { installed: 'installed', updated: 'updated', unchanged: 'unchanged' }[result.status];
...
export function createZCodeAgentsInstruction(locale, lifecycle) {
    return locale === 'zh'
...
export function validateQoderTemplate(source) {
    const required = [
    ...
    for (const relative of required) {
        if (!existsSync(join(source, relative)))
            throw new Error(`Qoder 模板缺少：${relative}`);
    }
    let manifest;
    let hooks;
    try {
        manifest = JSON.parse(readFileSync(join(source, '.qoder-plugin', 'plugin.json'), 'utf8'));
        hooks = JSON.parse(readFileSync(join(source, 'hooks', 'hooks.json'), 'utf8'));
    }
    catch (error) {
        throw new Error(`Qoder 插件 JSON 无法解析：${error instanceof Error ? error.message : '未知错误'}`);
    }
    if (manifest.name !== 'openlogos')
        throw new Error('Qoder plugin identity 必须为 openlogos');
...
export function preflightQoderTarget(root, source) {
    ...
    if (!existsSync(targetManifest)) {
        if (readdirSync(target).length > 0)
            throw new Error(`Qoder 目标存在不完整或未知 owner：${QODER_PLUGIN_REL_DIR}`);
        return;
    }
    let manifest;
    try {
...
class QoderManagedAssetTransaction {
    ...
    commit() {
        ...
        try {
            if (existsSync(this.target))
                cpSync(this.target, staging, { recursive: true, force: false });
            else
            ...
            const existed = existsSync(this.target);
            if (existed)
                renameSync(this.target, backup);
            if (process.env.OPENLOGOS_QODER_TXN_FAIL_AT === 'after-backup')
                throw new Error('注入的 Qoder 事务故障');
            ...
            if (!existsSync(this.target) && existsSync(backup))
                renameSync(backup, this.target);
            rmSync(transactionRoot, { recursive: true, force: true });
            throw error;
        }
    }
}
export function deployQoderAssets(root, source, sharedAssets = {}) {
    ...
    try {
        ...
        if (configBefore && !readFileSync(config).equals(configBefore))
            throw new Error('.qoder/settings.json 被意外修改');
        return {
...
export function localizedQoderResult(locale, result) {
    const action = locale === 'zh'
        ? { installed: '已安装', updated: '已更新', unchanged: '未变化' }[result.status]
        : result.status;
    const preserved = result.preserved.length > 0
...
export function createQoderAgentsInstruction(locale, lifecycle) {
    return locale === 'zh'
...
export function validateWorkBuddyTemplate(source) {
    const required = [
    ...
    for (const relative of required) {
        if (!existsSync(join(source, relative)))
            throw new Error(`WorkBuddy 模板缺少：${relative}`);
    }
    let manifest;
    let hooks;
    try {
        manifest = JSON.parse(readFileSync(join(source, '.workbuddy-plugin', 'plugin.json'), 'utf8'));
        hooks = JSON.parse(readFileSync(join(source, 'hooks', 'hooks.json'), 'utf8'));
    }
    catch (error) {
    ...
    if (manifest.name !== 'openlogos')
        throw new Error('WorkBuddy plugin identity 必须为 openlogos');
    const hooksText = JSON.stringify(hooks);
...
export function preflightWorkBuddyTarget(root, source) {
    ...
    if (!existsSync(targetManifest)) {
        if (readdirSync(target).length > 0)
            throw new Error(`WorkBuddy 目标存在不完整或未知 owner：${WORKBUDDY_PLUGIN_REL_DIR}`);
        return;
    }
    let manifest;
    try {
...
class WorkBuddyManagedAssetTransaction {
    ...
    commit() {
        ...
        try {
            ...
            if (existed)
                renameSync(this.target, backup);
            if (process.env.OPENLOGOS_WORKBUDDY_TXN_FAIL_AT === 'after-backup')
                throw new Error('注入的 WorkBuddy 事务故障');
            renameSync(staging, this.target);
            if (existsSync(backup))
                rmSync(backup, { recursive: true, force: true });
            rmSync(transactionRoot, { recursive: true, force: true });
...
export function deployWorkBuddyAssets(root, source, sharedAssets = {}) {
    ...
    try {
        ...
        if (settingsBefore && !readFileSync(settings).equals(settingsBefore))
            throw new Error('.workbuddy/settings.json 被意外修改');
        return {
...
export function localizedWorkBuddyResult(locale, result) {
    const action = locale === 'zh'
        ? { installed: '已安装', updated: '已更新', unchanged: '未变化' }[result.status]
        : result.status;
    const preserved = result.preserved.length > 0
...
export function createWorkBuddyAgentsInstruction(locale, lifecycle) {
    return locale === 'zh'
...
//# sourceMappingURL=ai-tool-adapter.js.map