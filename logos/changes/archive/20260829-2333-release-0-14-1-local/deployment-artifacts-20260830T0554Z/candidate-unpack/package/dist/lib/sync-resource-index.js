...
export function scanCandidateFiles(root) {
    const results = [];
    const scanDir = (absDir) => {
...
export function inferResourceDesc(relPath, locale) {
...
// ---------------------------------------------------------------------------
function parseExistingPaths(yamlContent) {
    const paths = new Set();
...
function appendToResourceIndex(yamlContent, entries) {
    if (entries.length === 0)
        return yamlContent;
    const block = entries
        .map(e => `  - path: ${e.path}\n    desc: ${e.desc}`)
...
export function syncResourceIndex(root, locale) {
...
//# sourceMappingURL=sync-resource-index.js.map