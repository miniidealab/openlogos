...
import { appendFileSync, closeSync, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, realpathSync, readdirSync, renameSync, writeFileSync, } from 'node:fs';
...
function parseCodeTasks(content) {
    const tasks = [];
    let inCode = false;
    let current = null;
...
export function computeTaskFingerprint(tasksContent) {
...
export function computeSpecFingerprint(root, targets) {
...
function safeProjectPath(root, raw) {
    if (!raw || raw.includes('\0') || raw.includes('\n') || raw.includes('\r') || raw.startsWith('/'))
        return null;
...
function extractTableTestIds(content) {
    const ids = [];
    for (const line of content.replace(/\r\n/g, '\n').split('\n')) {
...
export function extractDefinedVerificationIds(root) {
...
export function extractChangedTestIds(proposalDir) {
    const ids = walkMarkdown(join(proposalDir, 'deltas', 'test'))
        .flatMap(path => extractTableTestIds(readFileSync(path, 'utf8')));
    return uniqSorted(ids);
}
function violation(code, path, message, fixHint) {
...
function parseManifestShape(raw, violations) {
    const top = asRecord(raw);
    if (!top) {
        violations.push(violation('test-slice-manifest-invalid', '$', 'manifest 根必须是对象', '按 v1 schema 重建 manifest。'));
        return null;
    }
    if (top.schema !== TEST_SLICE_SCHEMA)
        return null;
    const strings = ['change', 'module', 'task_fingerprint', 'spec_fingerprint', 'generated_at'];
    ...
    top.slices.forEach((item, index) => {
        ...
        if (!taskText)
            violations.push(violation('test-slice-manifest-invalid', `${base}.task_text`, 'task_text 为空', '复制对应顶层 [code] task 文本。'));
...
function readCheckpointRows(proposalDir) {
...
function emptyState(status, reason, summary, violations = []) {
    return {
...
export function shouldUseSliceVerification(proposalDir) {
    if (existsSync(join(proposalDir, 'VERIFY_PASS')) && !existsSync(join(proposalDir, TEST_SLICE_MANIFEST)))
        return false;
...
export function deriveSliceVerificationState(root, proposalDir, expected) {
    if (!shouldUseSliceVerification(proposalDir))
        return null;
    ...
    if (!changeSet.valid) {
        return {
            ...
            }, [violation(changeSet.code, changeSet.path, changeSet.message, '恢复可信 SPEC_MERGED.test_change_set；禁止从 Delta、Git 或 slice manifest 反推。')]),
    ...
    let actualSpecFingerprint = null;
    try {
        actualSpecFingerprint = computeSpecFingerprint(root, allSpecTargets);
    }
    catch { /* 路径违规已报告 */ }
...
export function eligibleIdsFingerprint(ids) {
    return prefixedSha256(Buffer.from(JSON.stringify(uniqSorted(ids)), 'utf8'));
}
export function appendSliceCheckpoint(proposalDir, state, result, timestamp = new Date().toISOString()) {
    if (state.manifest_status !== 'valid' || state.verify_mode !== 'slice-checkpoint'
        || !state.attempted_slice_id || !state.manifest.sha256)
        return false;
    const row = {
    ...
    if (result === 'PASS') {
        const duplicate = readCheckpointRows(proposalDir).some(existing => existing.slice_id === row.slice_id
...
export function writeTestSliceManifestAtomic(path, manifest) {
    mkdirSync(dirname(path), { recursive: true });
...
//# sourceMappingURL=test-slice-manifest.js.map