...
import { ARCHIVE_WATCH_ERROR_CODES, ArchiveWatchError, assertArchiveSlug, coordinateArchiveWatch, findRecoverableArchiveWatchSession, isWindowsArchiveBusyError, nodeArchiveWatchFs, reconcileArchiveDiskState, writeArchiveWatchResultBestEffort, } from '../lib/archive-watch.js';
function archiveDirName(slug, now) {
    const pad = (n) => String(n).padStart(2, '0');
    const date = `${now.getFullYear()}${pad(now.getMonth() + 1)}${pad(now.getDate())}`;
    const time = `${pad(now.getHours())}${pad(now.getMinutes())}`;
    return `${date}-${time}-${slug}`;
}
function removeMatchingGuard(guardPath, slug, fs, log = true) {
...
function exitArchiveWatchError(locale, error) {
...
function exitLegacyWatcherBusy(locale) {
    console.error(t(locale, 'archive.watch.legacyBusy'));
    process.exit(1);
}
function findRecoveredArchivedPath(archiveDir, archiveDirname, fs) {
    const archivePath = join(archiveDir, archiveDirname);
    if (!fs.exists(archivePath) || fs.isSymlink(archivePath))
        return null;
    return { archivePath, archiveDirname };
}
export function archive(slug, options = {}) {
        ...
        if (platform === 'win32') {
            ...
            if (recovery && archived) {
                ...
                if (disk.guardActiveChange === slug) {
                    try {
                        removeMatchingGuard(guardPath, slug, fs, false);
                    }
                    catch { /* 由下一次三态裁决处理 */ }
                    disk = reconcileArchiveDiskState(changePath, archived.archivePath, guardPath, fs, slug);
                }
                if (disk.status !== 'archived') {
                    const error = new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.inconsistent, 'live-archive-guard-state-inconsistent');
    ...
    let session = { mode: 'non-windows', expectedInstances: [] };
    if (platform === 'win32') {
        try {
    ...
    try {
        ...
        if (disk.status === 'archived') {
            ...
            terminalWatchError = new ArchiveWatchError(ARCHIVE_WATCH_ERROR_CODES.inconsistent, 'live-archive-guard-state-inconsistent');
        }
        else {
            finalResult = {
                status: 'not-archived',
            ...
            if (isWindowsArchiveBusyError(error))
                terminalLegacyBusy = true;
            else
                throw error;
        }
    }
    ...
    if (terminalWatchError)
        exitArchiveWatchError(locale, terminalWatchError);
    if (terminalLegacyBusy)
        exitLegacyWatcherBusy(locale);
