...
import { BaselineCommitInProgressError, withBaselineReadLock, listRunIds, readRunRecord, } from './baseline-seed-txn.js';
/**
 ...
 */
export function effectiveBaselineSeedState(root, moduleId, explicit, opts) {
    if (explicit)
        return { state: explicit, legacy: false };
    const derive = () => {
    ...
    if (opts?.assumeLocked)
        return { state: derive(), legacy: true };
    const res = withBaselineReadLock(root, moduleId, new Date().toISOString(), derive);
    if (!res.ok) {
        throw new BaselineCommitInProgressError(`baseline_commit_in_progress — 模块 ${moduleId} 的 legacy seed 状态无法在恢复成功前派生`);
    }
    return { state: res.value, legacy: true };
}
//# sourceMappingURL=baseline-jit.js.map