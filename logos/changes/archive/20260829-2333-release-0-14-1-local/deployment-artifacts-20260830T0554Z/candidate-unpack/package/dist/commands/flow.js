...
function normalizeNode(node) {
    const out = { ...node, skipped: node.skipped ?? false, overlay_op: node.overlay_op ?? null };
    // S29：coverage_threshold 未设置（null/省略）时必须整键省略、绝不输出 null（保 flow show 零漂移）
    if (out.coverage_threshold == null)
        delete out.coverage_threshold;
    return out;
}
...
function renderText(result, locale) {
    ...
    for (const sub of result.flow.subflows) {
        ...
        for (const node of sub.nodes) {
            const attrs = [];
            if (node.skill)
                attrs.push(`skill: ${node.skill}`);
            if (node.for_each)
                attrs.push(`for_each: ${node.for_each}`);
            if (node.when)
                attrs.push(`when: ${node.when}`);
    ...
    lines.push('');
    if (!result.resolved)
        lines.push(t(locale, 'flow.resolvedHint'));
    return lines;
}
export function flowShow(format = 'text', opts = {}) {
    ...
    let result;
    try {
        result = loadFlow(root, { resolved: opts.resolved, lifecycle: opts.lifecycle });
    }
    catch (e) {
