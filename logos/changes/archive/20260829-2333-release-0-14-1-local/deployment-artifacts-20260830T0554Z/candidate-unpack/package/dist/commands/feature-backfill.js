...
function isResolved(s, features) {
    if (typeof s.feature !== 'string')
        return false;
    const mod = s.module ?? 'core';
    return features.some((f) => f.id === s.feature && f.module === mod);
}
function firstLines(path, n) {
    try {
 ...
 */
function conventionScenarioCandidateDocs(root, moduleId) {
    ...
    for (const f of readdirSync(base, { recursive: true })) {
        ...
        if (rel.includes('verify/baseline-seed-runs/'))
            continue; // run 私有目录副本不算主文档
        if (rel === name || rel.endsWith(`/${name}`))
            out.push(`logos/resources/${rel}`);
    }
    return out;
}
/**
 ...
 */
function scenarioCandidateTargets(root, moduleId) {
    const committed = listRunIds(root)
        .map((id) => readRunRecord(root, id))
        .filter((r) => !!r && r.module === moduleId && r.status === 'committed')
    ...
    if (latest) {
        ...
        if (sc.length > 0) {
            ...
            const keys = new Set(sc.flatMap((e) => e.candidate_keys));
            return { targets, keys };
        }
    }
    ...
    if (convTargets.length > 0)
        return { targets: convTargets, keys: null };
    return null;
}
/**
 ...
 */
function readModuleScenarioCandidates(root, moduleId, hasIndexEntry) {
    ...
    for (const rel of [...targets].sort()) {
        ...
        if (!section || !section.parse_ok)
            return { candidates: [], invalid: { reason: 'unparseable', paths: [rel] } };
...
function buildPrompt(scenarios, features, baselineCandidates, featureSpecs, currentYaml) {
...
export function featureBackfill(format = 'text', moduleId) {
    ...
    const locked = withRecoveredReadLocks(root, at, targetModuleIds, () => {
        ...
        for (const m of targetModuleIds) {
            ...
            if (invalid) {
                ...
                for (const p of invalid.paths)
                    invalidPaths.add(p);
                if (invalid.reason === 'unparseable')
                    sawUnparseable = true;
            }
            else {
        ...
        const registeredKeys = new Set(allScenarios.map((s) => `${s.module ?? 'core'} ${s.name ?? ''}`));
        const dedupedCandidates = baselineCandidates.filter((c) => !registeredKeys.has(`${c.module} ${c.name}`));
...
//# sourceMappingURL=feature-backfill.js.map