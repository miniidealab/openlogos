...
import { applyMergeTransaction, createMergeTransaction, listMergeTransactionPlanTargets, sealMergeTransaction, } from '../lib/merge-transaction.js';
import { checkUiHashMatch, commitVerifiedPrototypes, recoverCommitJournal, readPlanApproved, classifyProvenance, PROTOTYPE_DELTA_SUBPATH, } from '../lib/ui-provenance.js';
/** 原型资产：2-page-design 下的 .html（由 commitVerifiedPrototypes 落盘，merge-executor 不碰）。 */
function isPrototypeAsset(relativePath) {
    return relativePath.replace(/\\/g, '/').includes('/2-product-design/2-page-design/')
        && relativePath.endsWith('.html');
}
/** 读 guard 文件（activeChange / module）；缺失或损坏返回 null。 */
function readGuard(root) {
...
export function scanDeltas(deltasDir) {
    ...
    if (ioBroken)
        throw new DeltaScanUnreadableError(ioBroken);
    return entries
...
function noDeltaSpecMergedMarker() {
...
export function merge(slug) {
    ...
    if (closureActive) {
        ...
        const closureCodes = new Set(BASELINE_CLOSURE_VIOLATION_CODES);
    ...
    const conservationSectionWriters = new Map(); // code-r1 F1：`${targetRel}#${sectionLine}` → 写者列表
