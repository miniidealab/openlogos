 ...
 */
export function isDependencyExemptPath(path) {
 ...
 */
function buildAllowedWriteSet(paths) {
 ...
 * 返回违规清单；违规路径不回收。
 */
function copyBackAllowedFiles(sandboxRoot, workspaceRoot, allowedPaths) {
    const violations = [];
    let sandboxRealRoot;
    try {
 ...
 */
function findEscapingSymlinks(root) {
    const escaping = [];
    if (!existsSync(root))
        return escaping;
    const walk = (dir) => {
...
let cachedWriteProtectionProbe;
/**
 * 运行期写保护能力探测（结果按进程缓存）：
 ...
 */
export function detectRuntimeWriteProtection() {
    ...
    if (cachedWriteProtectionProbe)
        return cachedWriteProtectionProbe;
    let probe;
 ...
 */
function buildWriteProtection(kind, protectedRoot, sandboxDir) {
