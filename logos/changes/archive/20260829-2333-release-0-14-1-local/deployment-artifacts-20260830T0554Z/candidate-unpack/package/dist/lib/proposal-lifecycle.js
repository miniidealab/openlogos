...
import { authorityScan, extractUniqueAuthoritySection, isTableDelimiterRow, tableRowCells, } from './markdown-scan.js';
...
import { evaluateProposalStructure } from './plan-package-contract.js';
import { evaluatePlanPackage } from './plan-package.js';
import { deriveManagedAssetsDiagnostic, readBundledAssetManifest } from './asset-manifest.js';
const MERGE_SUPPORTED_DELTA_DIRS = ['prd', 'api', 'database', 'scenario', 'test', 'decisions', 'spec', 'skills'];
...
const DEPLOYMENT_FIELD_PLACEHOLDERS = {
    部署原因: '[说明为什么需要或不需要部署]',
    影响环境: '[本地 / 测试 / 预发 / 生产 / 无]',
};
function isTemplateBooleanChoice(value) {
    return value !== null && /^是\s*\/\s*否$/.test(value.trim());
}
export function isDeploymentSectionTemplateFilled(content) {
    const sectionResult = extractUniqueAuthoritySection(content, '部署影响');
    if (sectionResult.status === 'missing')
        return true;
    if (sectionResult.status === 'duplicate' || sectionResult.content === null)
        return false;
    const section = sectionResult.content;
    if (hasDuplicateDeploymentFields(section))
        return false;
...
export function isProposalTemplateFilled(content) {
    const normalized = content.trim();
    if (!normalized)
        return false;
    // 新 scaffold 走 Plan Package evaluator；旧已启动提案保留缺 deployment
    // canonical 章节的读取兼容，避免因 CLI 升级直接回退 writing。
    const strictFilled = evaluateProposalStructure(normalized).length === 0;
    const legacyFilled = normalized.includes('## 变更原因')
...
export function isTasksTemplateFilled(content) {
    const normalized = content.trim();
    if (!normalized)
        return false;
...
export function isTasksCodeFilled(content) {
...
function hasTestDeltaSignal(proposalDir, tasksContent) {
...
function proposalDeclaresCodeRequired(content) {
    return /(?:代码级|code-level|code\s+level|业务代码|实现代码|代码实现|runner|reporter)/i.test(content);
}
function proposalDeclaresNoCode(content) {
    return /(?:无需代码|无代码|纯文档|不涉及代码|no\s+code|docs-only|documentation-only)/i.test(content);
}
export function isCodeRequiredForProposal(proposalDir, tasksContent, sections) {
    ...
    if (testDeltaRequiresCode)
        return true;
    if (proposalDeclaresNoCode(proposalContent))
        return false;
...
function isAcceptedTestId(candidate) {
    if (TEST_ID_WILDCARD_RE.test(candidate))
        return false; // 减法拒绝①：通配族名整候选拒绝
    if (!TEST_ID_ANCHORED_RE.test(candidate))
        return false; // 兼容基线：整串匹配（match[0] === 候选全串）
    const tail = candidate.split('-').pop() ?? '';
    if (TEST_ID_PLACEHOLDER_TAILS.has(tail.toLowerCase()))
        return false; // 减法拒绝②：占位尾段
    return true;
}
/**
 ...
 */
export function parseTestCaseIds(text) {
    const ids = [];
 ...
 */
export function extractStructuredTestIds(content) {
    const lines = content.split(/\r?\n/);
    const scan = authorityScan(lines);
    const ids = [];
    let i = 0;
    while (i < lines.length) {
        if (scan.masked[i] || !scan.text[i].includes('|')) {
            i++;
            continue;
        }
        let j = i;
        while (j < lines.length && !scan.masked[j] && scan.text[j].includes('|'))
            j++;
 ...
 */
export function classifyTestEvidenceStage(proposalDir, tasksContent) {
    ...
    if (hasSpecCompleteMarker(proposalDir)) {
        ...
        if (sections && Object.prototype.hasOwnProperty.call(sections, 'delta')) {
            ...
            if (deltaProducing.length > 0 && deltaProducing.every(i => i.checked))
                stage = 'spec-complete';
        }
    }
    return stage;
 ...
 */
export function parseReuseDeclaration(proposalContent, mergedIds) {
    ...
    if (start === -1)
        return { present: false, entries: [], validIds: [], allValid: false };
    const entries = [];
    const seen = new Set();
    const validIds = [];
...
export function readMergedStructuredTestIds(proposalDir) {
    ...
    for (const file of listFiles(mergedTestDir)) {
        if (!file.endsWith('.md'))
            continue;
        try {
            for (const id of extractStructuredTestIds(readFileSync(join(mergedTestDir, file), 'utf-8')))
                ids.add(id);
        }
        catch { /* 不可读文件不构成存在性 */ }
    }
 ...
 */
export function evaluateTestIdEvidence(proposalDir, tasksContent) {
    const taskText = tasksContent ?? readIfExists(join(proposalDir, 'tasks.md'));
...
export function hasRealTestIdsForProposal(proposalDir, tasksContent) {
...
export function getProposalStepReason(proposalDir, step, tasksContent) {
    const taskText = tasksContent ?? readIfExists(join(proposalDir, 'tasks.md'));
    const sections = parseTaskSections(taskText);
...
export function getCodePlanningDiagnostic(proposalDir, tasksContent) {
    const taskText = tasksContent ?? (existsSync(join(proposalDir, 'tasks.md'))
        ? readFileSync(join(proposalDir, 'tasks.md'), 'utf-8') : '');
    const sections = parseTaskSections(taskText);
    if (!isCodeRequiredForProposal(proposalDir, taskText, sections))
        return null;
...
export function isCodeRequiredButUnplanned(proposalDir, tasksContent) {
    const taskText = tasksContent ?? (existsSync(join(proposalDir, 'tasks.md'))
        ? readFileSync(join(proposalDir, 'tasks.md'), 'utf-8') : '');
...
function replaceCodeSectionBody(content, placeholder) {
    const lines = content.split(/\r?\n/);
    const out = [];
    let inCode = false;
    for (const line of lines) {
 ...
 */
export function resetCodeSection(proposalDir, trigger, locale = 'zh') {
    const tasksPath = join(proposalDir, 'tasks.md');
    if (!existsSync(tasksPath))
        return false;
    const content = readFileSync(tasksPath, 'utf-8');
    if (!isTasksCodeFilled(content))
        return false; // 幂等：已占位不动
...
export function countMergeableDeltaFiles(proposalDir) {
    let count = 0;
...
export function hasSpecCompleteMarker(proposalDir) {
 ...
 */
export function deriveProposalFacts(proposalDir, tasksContent) {
    const taskText = tasksContent ?? (existsSync(join(proposalDir, 'tasks.md'))
        ? readFileSync(join(proposalDir, 'tasks.md'), 'utf-8') : '');
    const sections = parseTaskSections(taskText);
 ...
 */
export function writeSlicesApprovedMarker(proposalDir) {
    const path = join(proposalDir, SLICES_APPROVED_MARKER);
    if (existsSync(path))
        return; // 快路径：已存在不重写（approved_at 一次写定）
 ...
 */
export function readSlicesApprovedAt(proposalDir) {
    const path = join(proposalDir, SLICES_APPROVED_MARKER);
    if (!existsSync(path))
        return null;
    try {
        ...
        if (parsed.schema !== 'openlogos/slices-approved@1')
            return null;
        if (typeof parsed.approved_at !== 'string' || parseStrictTimestampMs(parsed.approved_at) == null)
            return null;
...
export function allTasksChecked(content) {
 ...
 */
export function parseTaskSections(content) {
    const lines = content.split(/\r?\n/);
...
export function extractTaskSectionItems(content, tag) {
    const items = [];
    const lines = content.split(/\r?\n/);
    const sectionPattern = /^## \[([a-z][a-z0-9-]*)\]/i;
    let inSection = false;
...
export function readTaskSectionItems(proposalDir, tag) {
    const tasksPath = join(proposalDir, 'tasks.md');
    if (!existsSync(tasksPath))
        return [];
    return extractTaskSectionItems(readFileSync(tasksPath, 'utf-8'), tag);
}
function resolveTasksExecution(sections) {
    for (const scope of ['delta', 'deploy', 'code']) {
        if (sections?.[scope]) {
            return {
...
export function derivePlanState(proposalDir, step, deploymentDecision, tasksContent) {
    ...
    const clarificationEvaluation = evaluateProposalClarification(proposalContent, parseProposalDeploymentDecision(proposalContent)?.deployment_required ?? null);
    ...
    const planReady = planPackage.ready && !conflict;
    ...
    return {
        ...
        completion_contract_version: planPackage.contract_version,
        completion_issues: planPackage.issues,
        proposal_filled: planPackage.proposal.filled,
        tasks_plan_filled: planPackage.tasks.plan_filled,
        tasks_code_required: planPackage.tasks.code_required,
...
export function parseProposalDeploymentDecision(content) {
    const sectionResult = extractUniqueAuthoritySection(content, '部署影响');
    if (sectionResult.status !== 'unique' || sectionResult.content === null)
        return null;
    const section = sectionResult.content;
    if (hasDuplicateDeploymentFields(section))
        return null;
...
function getTaskSectionSummary(tasksContent, tag) {
...
export function getDeploySectionSummary(tasksContent) {
    return getTaskSectionSummary(tasksContent, 'deploy').summary;
}
function deploymentConflictWarning(deploymentRequired, hasDeploySection) {
...
export function resolveDeploymentProgress(proposalDir) {
    const tasksPath = join(proposalDir, 'tasks.md');
    if (!existsSync(tasksPath)) {
        return {
            checked: 0,
...
export function resolveDeploymentDocument(root, proposalSlug) {
...
function readProposalDeploymentDecision(proposalDir) {
    const proposalPath = join(proposalDir, 'proposal.md');
    if (!existsSync(proposalPath))
        return null;
    return parseProposalDeploymentDecision(readFileSync(proposalPath, 'utf-8'));
}
export function resolveProposalDeploymentDecision(proposalDir, moduleDefaults = {}) {
    const tasksPath = join(proposalDir, 'tasks.md');
...
function getSectionSummary(sections, tag) {
    return sections?.[tag] ?? null;
}
export function hasSmokeCasesForProposal(proposalDir) {
    return listFiles(join(proposalDir, '..', '..', 'resources', 'test', 'smoke')).length > 0;
}
export function detectProposalStep(proposalDir, moduleDefaults = {}) {
...
//# sourceMappingURL=proposal-lifecycle.js.map