...
export const LOCAL_RELEASE_CANDIDATE_SCHEMA = 'openlogos/local-release-candidate@1';
export const LOCAL_RELEASE_PACKAGE_NAME = '@miniidealab/openlogos';
export const LOCAL_RELEASE_CANDIDATE_VERSION = '0.14.1';
export const LOCAL_RELEASE_ROLLBACK_VERSION = '0.14.0';
...
export function freezeLocalReleaseCandidateIdentity(input) {
    if (!exactKeys(Object.keys(input), IDENTITY_KEYS))
        throw new Error('local_candidate_invalid:fields');
    if (input.schema !== LOCAL_RELEASE_CANDIDATE_SCHEMA)
        throw new Error('local_candidate_invalid:schema');
    if (input.package_name !== LOCAL_RELEASE_PACKAGE_NAME)
        throw new Error('local_candidate_invalid:package_name');
    if (input.package_version !== LOCAL_RELEASE_CANDIDATE_VERSION)
        throw new Error('local_candidate_invalid:package_version');
    if (input.asset_manifest_version !== LOCAL_RELEASE_CANDIDATE_VERSION)
        throw new Error('local_candidate_invalid:asset_manifest_version');
    if (!SHA256_PATTERN.test(input.tarball_sha256))
        throw new Error('local_candidate_invalid:tarball_sha256');
    if (!isAbsolute(input.command_path))
        throw new Error('local_candidate_invalid:command_path');
    if (!isAbsolute(input.package_root))
        throw new Error('local_candidate_invalid:package_root');
    if (!contained(input.package_root, input.command_path))
        throw new Error('local_candidate_invalid:command_outside_package');
...
export function assertLocalReleaseCommandGraph(commands) {
    const forbidden = [
    ...
    for (const item of commands) {
        ...
        if (match)
            throw new Error(`local_candidate_public_release_forbidden:${match[0]}`);
    }
}
/** 生成只引用固定 0.14.0 tarball 的可复制回滚与版本复核命令。 */
export function buildLocalReleaseRollbackPlan(prefix, rollbackTarball, installedEntry) {
    if (!isAbsolute(prefix))
        throw new Error('local_candidate_rollback_invalid:prefix');
    if (!isAbsolute(rollbackTarball))
        throw new Error('local_candidate_rollback_invalid:tarball');
...
//# sourceMappingURL=local-release-candidate.js.map