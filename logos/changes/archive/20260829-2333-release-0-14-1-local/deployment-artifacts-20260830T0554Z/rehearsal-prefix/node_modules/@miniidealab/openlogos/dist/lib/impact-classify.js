 ...
 */
export function isRevArgLexicallyValid(value) {
 ...
 */
export function isValidGitTopLevelPath(path) {
    if (!path || path.startsWith('/'))
        return false;
    return path.split('/').every(seg => seg !== '' && seg !== '.' && seg !== '..');
}
/** `--prefix` / `git rev-parse --show-prefix` 同段级不变量：空前缀合法；非空剥尾 `/` 后须为合法 top-level 相对路径。 */
export function isValidPrefixArg(prefix) {
    if (prefix === '')
        return true;
 ...
 */
export function stripProjectPrefix(path, prefix) {
...
/** 分类项目根相对路径（契约 v1 查表）。 */
export function classifyRootRelativePath(rel) {
    if (rel === PATH_CLASSES_V1.guardFile)
        return 'lifecycle';
    for (const dir of PATH_CLASSES_V1.lifecycleDirs) {
        if (matchesDir(rel, dir))
            return 'lifecycle';
    }
    if (matchesDir(rel, PATH_CLASSES_V1.projectDir))
        return 'project';
    return 'external';
}
/** 分类原始输入路径：剥前缀失败（前缀外）直接 external。 */
export function classifyInputPath(path, prefix) {
    const rel = stripProjectPrefix(path, prefix);
    if (rel === null)
        return 'external';
    return classifyRootRelativePath(rel);
}
const CLASS_SAFETY = { lifecycle: 0, project: 1, external: 2 };
/** R/C 双侧取更不安全一侧。 */
 ...
 */
export function classifyImpact(stream, prefix) {
    const reasons = [];
    const files = [];
    const nonLifecycle = new Set();
    ...
    while (i < tokens.length) {
        ...
        if (statusToken === 'A' || statusToken === 'M' || statusToken === 'D') {
            ...
            if (classifyInputPath(path, prefix) !== 'lifecycle')
                nonLifecycle.add(path);
            i += 2;
        }
            ...
            if (!isValidGitTopLevelPath(oldPath) || !isValidGitTopLevelPath(newPath)) {
                parseFailed = true;
                reasons.push(`parse failure: invalid git path ${JSON.stringify(!isValidGitTopLevelPath(oldPath) ? oldPath : newPath)} (fail-closed)`);
                break;
            }
            ...
            if (oldClass !== 'lifecycle')
                nonLifecycle.add(oldPath);
            if (newClass !== 'lifecycle')
                nonLifecycle.add(newPath);
            i += 3;
        }
            ...
            parseFailed = true;
            const path = tokens[i + 1];
            reasons.push(`unknown status ${JSON.stringify(statusToken)}${path ? ` for path ${JSON.stringify(path)}` : ''} (fail-closed)`);
    ...
    const lifecycleOnly = !parseFailed && files.length > 0 && files.every(f => f.class === 'lifecycle');
    let operations = [];
    let changes = [];
    if (lifecycleOnly) {
 ...
 * 混合 / 不可推断形态 → 两字段空数组。
 */
function inferArchiveOperation(files, prefix) {
    ...
    for (const f of files) {
        ...
        if ((f.status === 'R' || f.status === 'C') && f.old_path !== null) {
            ...
            if (slugMatch && slugMatch[1] !== 'archive' &&
    ...
    if (guardDeletes !== 1 || slugs.size === 0)
        return none;
    return { operations: ['archive'], changes: [...slugs].sort() };
}
//# sourceMappingURL=impact-classify.js.map