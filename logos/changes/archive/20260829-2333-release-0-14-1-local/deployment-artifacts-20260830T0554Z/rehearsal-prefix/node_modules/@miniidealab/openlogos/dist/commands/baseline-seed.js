...
import { parseProvenanceSection, isCanonicalKey, replaceProvenanceSection, reconcileModuleCandidates, extractRawCandidates, validateSeedCandidate, listModuleProvenanceDocs, } from '../lib/baseline-provenance.js';
...
import { validateManifest, isSafeModuleId, isSafeRunId, acquireLock, releaseLock, readGate, recoverJournal, findUnfinalizedJournal, listRunIds, readRunRecord, runRecordPath, stagingDir, runDir, atomicWriteJson, writeSeedState, readSeedState, commitSeededTransaction, recordIssuedRun, isIssuedRun, newIssuedNonce, } from '../lib/baseline-seed-txn.js';
/**
 ...
 */
function assertManifestCoversHoldingDocs(root, moduleId, expected) {
    ...
    if (uncovered.length > 0) {
        fail('json', 'provenance_doc_uncovered', `manifest 未覆盖仍持有 ${moduleId} 现状候选的文档：${uncovered.join(', ')}（改名/换址须在 manifest 显式纳入旧文档以迁移/清空其 provenance）`);
    }
}
function nowIso() {
...
function assertRegisteredAdoptedModule(root, moduleId, format) {
    const parsed = readProjectYaml(root);
    const mod = parsed.data?.modules?.find(m => m.id === moduleId);
    if (!mod)
        fail(format, 'unknown_run', `模块 ${moduleId} 未在 logos-project.yaml 注册`);
    if (!isAdoptedBootstrap(mod.bootstrap))
        fail(format, 'unknown_run', `模块 ${moduleId} 非 adopted（brownfield）模块，不接受 baseline-seed`);
}
...
function fail(_format, code, message) {
...
function runCommand(format, fn) {
    try {
        ...
        if (e instanceof SeedFailure) {
            if (format === 'json')
                console.error(JSON.stringify(makeErrorEnvelope('baseline-seed', e.code, e.message)));
            else
                console.error(`✗ baseline-seed: ${e.message} (${e.code})`);
            process.exit(1);
        }
        throw e;
    }
...
function allocateRunId(root, moduleId) {
...
export function baselineSeedBegin(moduleId, manifestPath, format) {
    runCommand(format, () => {
        const root = process.cwd();
        if (!moduleId)
            fail(format, 'invalid_manifest', '缺少 --module');
        if (!isSafeModuleId(moduleId))
            fail(format, 'path_escape', `module id 非法：${moduleId}`);
        ...
        if (!manifestPath || !existsSync(join(root, manifestPath)))
            fail(format, 'invalid_manifest', `manifest 不存在：${manifestPath}`);
        let manifest;
        try {
        ...
        if (!acquireLock(root, moduleId))
            fail(format, 'run_locked', `模块 ${moduleId} 正被其它提交占用`);
        try {
            const pending = findUnfinalizedJournal(root, moduleId);
            if (pending)
                recoverJournal(root, pending.runId, nowIso());
            ...
            const record = { run_id: runId, module: moduleId, status: 'open', expected, created_at: nowIso(), issued_nonce: issuedNonce };
            ...
            if (format === 'json')
                console.log(JSON.stringify(makeEnvelope('baseline-seed', payload)));
            else {
...
function classifyStaged(root, runId, moduleId, expected) {
    ...
    for (const item of expected) {
        ...
        for (const raw of rawSec.raw) {
            if (validateSeedCandidate(raw, moduleId) !== null)
                return { hardError: 'invalid_provenance' };
        }
...
export function baselineSeedCommit(moduleId, runId, format) {
    runCommand(format, () => {
        const root = process.cwd();
        if (!moduleId)
            fail(format, 'invalid_manifest', '缺少 --module');
        if (!isSafeModuleId(moduleId))
            fail(format, 'path_escape', `module id 非法：${moduleId}`);
        if (!runId)
            fail(format, 'invalid_manifest', '缺少 --run-id');
        if (!isSafeRunId(runId))
            fail(format, 'path_escape', `run id 非法：${runId}`); // F2：防 --run-id 路径注入
        ...
        try {
            ...
            if (!record)
                fail(format, 'unknown_run', `未知 run_id：${runId}`);
            if (record.module !== moduleId)
                fail(format, 'unknown_run', `run ${runId} 不属于模块 ${moduleId}`);
            // F2：run 记录自身的 run_id 必须与 --run-id 一致——防手工放入 run.json（run_id 指向他值）冒充签发。
            if (record.run_id !== runId)
                fail(format, 'unknown_run', `run 记录 run_id 与 --run-id 不一致（记录 ${record.run_id}）`);
            ...
            if (record.status === 'superseded')
                fail(format, 'stale_run', `run ${runId} 已被新 begin 取代`);
            ...
            if (!manifestCheck.ok)
                fail(format, manifestCheck.error, 'run 记录 manifest 校验未通过（被篡改或非法）');
            ...
            if ('hardError' in cls)
                fail(format, cls.hardError, 'staged 产物 candidates[] 非法：key 非本模块规范键或与 manifest candidate_keys 不一致');
            ...
            if (allValid && cls.committed.length > 0) {
                ...
                const stagedByDoc = new Map();
                const stagedContentByDoc = new Map();
                for (const e of record.expected) {
                    ...
                    if (!stagedSection)
                        continue; // classifyStaged 已保证合法，防御性跳过
...
export function baselineSeedStatus(moduleId, format) {
    runCommand(format, () => {
        const root = process.cwd();
        if (!moduleId)
            fail(format, 'invalid_manifest', '缺少 --module');
        ...
        if (!gate.ok)
            fail(format, gate.error, '提交进行中，请稍后重试');
        ...
        const openRuns = listRunIds(root)
            ...
            .filter((r) => r !== null && r.module === moduleId && r.status === 'open');
        ...
        if (run) {
            ...
            for (const item of run.expected) {
                if (existsSync(join(stagingDir(root, run.run_id), item.target_path)))
                    staged += 1;
                else
                    missing.push(item.target_path);
            }
...
//# sourceMappingURL=baseline-seed.js.map