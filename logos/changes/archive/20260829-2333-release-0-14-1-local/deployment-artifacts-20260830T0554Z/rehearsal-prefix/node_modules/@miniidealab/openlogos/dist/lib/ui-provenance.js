 ...
 */
import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, rmSync, renameSync, copyFileSync, statSync, } from 'node:fs';
...
export function readPlanApproved(proposalDir) {
    ...
    const prov = { present: true, empty: false, raw };
    // ui_prototype_rendered：布尔才采信；存在但非布尔 → malformed
    if (typeof rec.ui_prototype_rendered === 'boolean')
        prov.ui_prototype_rendered = rec.ui_prototype_rendered;
    else if (rec.ui_prototype_rendered !== undefined)
        prov.malformed = true;
    // pages：必须是纯字符串数组；存在任一非字符串成员或非数组 → malformed（保留可解析部分供诊断，但不据此判 full）
    if (rec.pages !== undefined) {
        if (!Array.isArray(rec.pages) || rec.pages.some(x => typeof x !== 'string'))
            prov.malformed = true;
        prov.pages = Array.isArray(rec.pages) ? rec.pages.filter((x) => typeof x === 'string') : [];
    }
    ...
    if (rec.hashes !== undefined) {
        if (!rec.hashes || typeof rec.hashes !== 'object' || Array.isArray(rec.hashes)) {
            ...
            const h = {};
            for (const [k, v] of Object.entries(rec.hashes)) {
 ...
 */
export function writePlanApprovedMarker(proposalDir, provenance) {
    const p = join(proposalDir, PLAN_APPROVED);
    if (!provenance) {
        if (!existsSync(p))
            writeFileSync(p, '');
 ...
 */
export function classifyProvenance(prov) {
 ...
 */
export function isFullProvenanceValid(prov) {
    ...
    for (const p of pages) {
        if (!isValidPrototypeBasename(p))
            return false; // 非法/路径穿越 basename
        if (pageSet.has(p))
            return false; // 重复
        pageSet.add(p);
    }
    for (const k of hashKeys) {
        if (!isValidPrototypeBasename(k))
            return false;
    }
    // pages 集合 == hashes 键集合
...
export function computePrototypeHashes(dir) {
    const out = {};
    if (!existsSync(dir))
        return out;
 ...
 */
export function checkUiHashMatch(proposalDir, sourceDir) {
...
function writeJournalAtomic(path, journal) {
    const tmp = `${path}.tmp`;
 ...
 */
export function commitVerifiedPrototypes(proposalDir, root) {
    const prov = readPlanApproved(proposalDir);
    const cls = classifyProvenance(prov);
    ...
    try {
        for (const e of journal.targets) {
            if (e.backup)
                copyFileSync(e.target, e.backup);
        ...
        return {
            ...
            reason: rb ? `commit_failed:${err.message}` : `commit_failed_rollback_incomplete:${err.message}`,
            rolledBack: rb,
        };
    }
    // 落盘后复核（full 模式）
 ...
 */
function rollbackAllToOld(journal, journalPath) {
    for (const e of journal.targets) {
        try {
            if (e.backup && existsSync(e.backup))
                copyFileSync(e.backup, e.target);
            else if (targetReplaced(e))
 ...
 */
function abortTransaction(journal, journalPath, stagingDir, backupDir) {
    journal.intent = 'abort';
    writeJournalAtomic(journalPath, journal); // 先持久化 abort intent，崩溃后恢复才会继续回滚而非误前滚
    if (!rollbackAllToOld(journal, journalPath))
        return false;
    rmSync(stagingDir, { recursive: true, force: true });
    rmSync(backupDir, { recursive: true, force: true });
    rmSync(journalPath, { force: true });
 ...
 */
function isValidJournalEntry(e, proposalDir, root) {
    if (!e || typeof e !== 'object')
        return false;
    const r = e;
    if (typeof r.basename !== 'string' || !isValidPrototypeBasename(r.basename))
        return false;
    if (typeof r.staged !== 'string' || typeof r.target !== 'string')
        return false;
    if (r.backup !== null && typeof r.backup !== 'string')
        return false;
    ...
    const backupDir = join(proposalDir, BACKUP_DIR);
    const targetDir = join(root, PROTOTYPE_RESOURCE_SUBPATH);
    if (!isWithin(stagingDir, r.staged) || basename(r.staged) !== r.basename)
        return false;
    if (!isWithin(targetDir, r.target) || basename(r.target) !== r.basename)
        return false;
    if (r.backup !== null && (!isWithin(backupDir, r.backup) || basename(r.backup) !== r.basename))
        return false;
    return true;
}
/**
 ...
 */
export function recoverCommitJournal(proposalDir) {
    const journalPath = join(proposalDir, COMMIT_JOURNAL);
    ...
    try {
        ...
        return 'failed';
    } // 损坏/截断 JSON：保留 journal，fail closed（不删恢复意图）
    const rec = (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : null;
    if (!rec || !Array.isArray(rec.targets))
        return 'failed';
    if (rec.intent !== 'commit' && rec.intent !== 'abort')
        return 'failed'; // 缺/非法 intent → fail closed（保留 journal）
    const targets = rec.targets;
    for (const e of targets) {
        if (!isValidJournalEntry(e, proposalDir, root))
            return 'failed'; // 越界/非法 → 不动文件、fail closed
    }
    const journal = { intent: rec.intent, targets: targets };
    const stagingDir = join(proposalDir, STAGING_DIR);
    const backupDir = join(proposalDir, BACKUP_DIR);
    ...
    try {
        for (const e of journal.targets) {
            ...
            if (e.backup && existsSync(e.target) && !existsSync(e.backup))
                copyFileSync(e.target, e.backup);
            safeRename(e.staged, e.target);
            e.done = true;
...
//# sourceMappingURL=ui-provenance.js.map