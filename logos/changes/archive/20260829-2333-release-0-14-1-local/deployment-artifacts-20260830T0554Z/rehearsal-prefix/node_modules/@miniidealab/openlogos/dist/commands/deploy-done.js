...
import { parseTaskSections, resolveProposalDeploymentDecision, } from './status.js';
 ...
 */
async function isVerifyPassedForDeploy(root, proposalDir) {
    ...
    if (!verify)
        return (existsSync(join(proposalDir, 'VERIFY_PASS')) && !existsSync(join(proposalDir, 'VERIFY_FAIL'))) ? { passed: true } : { passed: false };
    ...
    const assertVerifyPred = (field, pred) => {
        if (typeof pred === 'string' && (pred.startsWith('cmd:') || pred.startsWith('marker:')))
            return;
    ...
    const markerHit = (pred) => typeof pred === 'string' && pred.startsWith('marker:') && existsSync(join(proposalDir, pred.slice('marker:'.length).trim()));
    ...
    const evalCmd = async (field, pred) => {
    ...
    if (isCmdPred(fail)) {
        const r = await evalCmd('fail_when', fail);
        if (r.satisfied)
            return { passed: false, cmd: r };
    }
    else if (markerHit(fail))
        return { passed: false };
    ...
    if (typeof done === 'string' && done.startsWith('marker:'))
        return markerHit(done) ? { passed: true } : { passed: false };
    return { passed: false };
}
export function readActiveProposalGuard(root) {
...
export function checkTaskSection(content, tag) {
...
function resolveModuleDefaults(root, moduleId) {
...
function readDeploySummary(proposalDir) {
...
function clearSmokeMarkers(proposalDir) {
    const cleared = [];
...
function checkVerifyIsCmdGate(root) {
    const flow = loadFlow(root, { lifecycle: 'launched', resolved: true }).flow;
    for (const s of flow.subflows)
        for (const n of s.nodes)
            if (n.id === 'verify')
                return isCmdPred(n.done_when) || isCmdPred(n.fail_when);
    return false;
}
/** S30：执行 verify 判定；FlowError/CmdSpawnError/未通过一律 fail loud；通过则返回结果（带 cmdEval 回灌用）。 */
async function verifyOrFail(root, proposalDir, format, locale) {
    let verifyResult;
    try {
        ...
        if (e instanceof FlowError)
            failRaw(format, e.code, e.message);
        if (e instanceof CmdSpawnError) {
            const field = e.field ?? 'done_when';
...
export async function deployDone(format = 'text', environment) {
    ...
    if (data.environment)
        console.log(`  ${t(locale, 'deployDone.environment', { environment: data.environment })}`);
...
//# sourceMappingURL=deploy-done.js.map