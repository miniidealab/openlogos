...
import { validate as compileOpenApi30, } from '@hyperjump/json-schema/openapi-3-0';
import { validate as compileOpenApi31 } from '@hyperjump/json-schema/openapi-3-1';
import { parseDocument } from 'yaml';
import { authorityScan, scanMarkdownAuthorityStructure, } from './markdown-scan.js';
import { extractStructuredTestIds, extractTaskSectionItems, resolveProposalDeploymentDecision, } from './proposal-lifecycle.js';
import { DELTA_TO_RESOURCE, } from './delta-classify.js';
import { BaselineCommitInProgressError, listBaselineSeedModuleIds, withRecoveredReadLocks, } from './baseline-seed-txn.js';
export const BASELINE_CLOSURE_POLICY = 'on-touch-v1';
export const BASELINE_CLOSURE_MODES = ['MODIFY', 'CREATE', 'SKIP', 'AMBIGUOUS'];
...
function isCanonicalScenarioArray(value, allowEmpty) {
    if (!Array.isArray(value) || (!allowEmpty && value.length === 0))
        return false;
    if (value.some(v => typeof v !== 'string' || !/^S[0-9]+$/.test(v)))
        return false;
 ...
 */
export function classifyCanonicalTargetCategory(targetPath) {
    const path = targetPath.replace(/\\/g, '/');
    if (path.startsWith('logos/resources/prd/1-product-requirements/'))
        return 'requirement';
    if (path.startsWith('logos/resources/prd/2-product-design/1-feature-specs/'))
        return 'feature';
    if (path.startsWith('logos/resources/prd/2-product-design/2-page-design/'))
        return 'feature';
    if (path.startsWith('logos/resources/prd/3-technical-plan/1-architecture/'))
        return 'architecture';
    if (path.startsWith('logos/resources/prd/3-technical-plan/2-scenario-implementation/'))
        return 'scenario';
    if (path.startsWith('logos/resources/prd/3-technical-plan/3-deployment/'))
        return 'deployment';
    if (path.startsWith('logos/resources/api/'))
        return 'api';
    if (path.startsWith('logos/resources/database/'))
        return 'database';
    if (path.startsWith('logos/resources/test/smoke/'))
        return 'smoke';
    if (path.startsWith('logos/resources/test/'))
        return 'test';
    if (path.startsWith('logos/resources/scenario/'))
        return 'orchestration';
    if (path.startsWith('logos/resources/decisions/'))
        return 'decision';
 ...
 */
export function resolveCanonicalMergeTarget(root, proposalDir, rawDeltaPath) {
    if (typeof rawDeltaPath !== 'string' || rawDeltaPath.trim() !== rawDeltaPath || rawDeltaPath === '')
        return null;
    if (rawDeltaPath.includes('\0') || isAbsolute(rawDeltaPath) || /^[A-Za-z]:[\\/]/.test(rawDeltaPath))
        return null;
    ...
    if (segs.length < 3 || segs[0] !== 'deltas' || segs.some(s => s === '..' || s === ''))
        return null;
    ...
    if (!existingPathContained(proposalDir, deltaAbs))
        return null;
    // CREATE 时类别根本身可以尚不存在；此时以项目根检查现存祖先，待类别根出现后再收紧到该根。
    if (!existingPathContained(existsSync(targetBase) ? targetBase : root, targetAbs))
        return null;
...
export function canonicalTargetFromDeltaPath(rawDeltaPath) {
    if (typeof rawDeltaPath !== 'string' || isAbsolute(rawDeltaPath) || /^[A-Za-z]:[\\/]/.test(rawDeltaPath))
        return null;
    const slash = rawDeltaPath.replace(/\\/g, '/');
    if (slash.split('/').some(s => s === '..' || s === ''))
        return null;
    const normalized = posix.normalize(slash);
    const segs = normalized.split('/');
    if (segs.length < 3 || segs[0] !== 'deltas')
        return null;
...
function extractClosureYaml(proposalContent) {
    const lines = proposalContent.split(/\r?\n/);
    const scan = authorityScan(lines);
    const headings = [];
    for (let i = 0; i < lines.length; i++) {
        if (!scan.masked[i] && /^##\s+基线闭包计划\s*$/.test(scan.text[i]))
            headings.push(i);
    }
    if (headings.length === 0)
        return { present: false };
    if (headings.length !== 1)
        return { present: true, error: '「## 基线闭包计划」必须恰出现一次' };
    const start = headings[0] + 1;
    let end = lines.length;
    for (let i = start; i < lines.length; i++) {
        if (!scan.masked[i] && /^##\s+/.test(scan.text[i])) {
            end = i;
            break;
        }
    }
 ...
 */
export function hasBaselineClosureSignal(proposalContent, tasksContent) {
    if (extractClosureYaml(proposalContent).present)
        return true;
...
export function parseBaselineClosurePlan(root, proposalDir, proposalContent, proposalRelPath) {
    const extracted = extractClosureYaml(proposalContent);
    if (!extracted.present)
        return { violations: [] };
    if (extracted.error || extracted.yaml === undefined)
        return { violations: [malformed(proposalRelPath, extracted.error ?? '缺少 YAML')] };
    ...
    if (!closure)
        return { violations: [malformed(proposalRelPath, 'baseline_closure 必须是对象')] };
    const planKeyProblem = exactKeys(closure, PLAN_KEYS);
    if (planKeyProblem)
        return { violations: [malformed(proposalRelPath, `baseline_closure ${planKeyProblem}`)] };
    ...
    if (!literalsOk)
        return { violations: [malformed(proposalRelPath, 'on-touch-v1 固定字面量、schema_version 或布尔字段不匹配')] };
    ...
    for (let i = 0; i < closure.targets.length; i++) {
        ...
        const at = `targets[${i}]`;
        if (!rawTarget) {
            violations.push(malformed(proposalRelPath, `${at} 必须是对象`));
            continue;
        }
        const keyProblem = exactKeys(rawTarget, TARGET_KEYS);
        if (keyProblem) {
            violations.push(malformed(proposalRelPath, `${at} ${keyProblem}`));
            continue;
        }
        const category = rawTarget.category;
        const mode = rawTarget.mode;
        if (typeof category !== 'string' || !BASELINE_CLOSURE_CATEGORIES.includes(category)) {
            violations.push(malformed(proposalRelPath, `${at}.category 不在闭合枚举中`));
            continue;
        }
        if (typeof mode !== 'string' || !BASELINE_CLOSURE_MODES.includes(mode)) {
            violations.push(malformed(proposalRelPath, `${at}.mode 不在闭合枚举中`));
            continue;
        }
        if (!isCanonicalScenarioArray(rawTarget.scenario_ids, false)
            || rawTarget.scenario_ids.some(id => !touchedSet.has(id))) {
            violations.push(malformed(proposalRelPath, `${at}.scenario_ids 必须非空、去重、升序且属于 touched_scenario_ids`));
            continue;
        }
        if (typeof rawTarget.reason !== 'string' || rawTarget.reason.trim() === ''
            || rawTarget.reason.trim() === mode) {
            violations.push(malformed(proposalRelPath, `${at}.reason 必须是有解释力的非空字符串`));
            continue;
        }
        if (!isUniqueNonEmptyStringArray(rawTarget.evidence, mode === 'AMBIGUOUS')
            ...
            violations.push(malformed(proposalRelPath, `${at} 的 evidence/missing_evidence 类型、非空性或唯一性不合法`));
            continue;
        }
        ...
        if ((nonMaterial && rawTarget.delta_path !== null)
            ...
            violations.push(malformed(proposalRelPath, `${at}.delta_path 与 mode=${mode} 的组合不合法`));
            continue;
        }
        if (mode !== 'AMBIGUOUS' && rawTarget.missing_evidence.length !== 0) {
            violations.push(malformed(proposalRelPath, `${at}.missing_evidence 仅 AMBIGUOUS 可非空`));
            continue;
        }
        if (mode === 'AMBIGUOUS' && rawTarget.missing_evidence.length === 0) {
            violations.push(malformed(proposalRelPath, `${at}.missing_evidence 在 AMBIGUOUS 时必须非空`));
            continue;
        }
        let resolution = null;
        if (typeof rawTarget.delta_path === 'string') {
            ...
            if (resolution.semanticCategory === null) {
                violations.push(malformed(proposalRelPath, `${at}.delta_path 的 canonical target 不属于 on-touch-v1 可判定语义类别：${resolution.targetPath}`));
                continue;
            }
            if (resolution.semanticCategory !== category) {
                violations.push(malformed(proposalRelPath, `${at}.category=${category} 与 canonical target ${resolution.targetPath} 的语义类别=${resolution.semanticCategory} 不一致`));
                continue;
            }
        }
        targets.push({
...
export function parseBaselineClosureTaskTargets(root, proposalDir, tasksContent) {
    const items = extractTaskSectionItems(tasksContent, 'delta');
    const targets = [];
    const invalid = [];
    let hasModeSyntax = false;
    for (const item of items) {
        if (TASK_CLOSURE_MODE_PREFIX_RE.test(item.text.trim()))
            hasModeSyntax = true;
        ...
        targets.push({ checked: item.checked, mode: m[1], ...resolution });
    }
    return { hasModeSyntax, targets, invalid };
}
const NON_MD_MARKER = /^## (ADDED|MODIFIED) — (.+?)(（新文件，整文件）|（整文件替换）)$/;
function duplicateAwareObject(payload, extension) {
    if (extension === '.json') {
        ...
        if (ydoc.errors.length > 0)
            return { error: ydoc.errors.map(e => e.message).join('；') };
        try {
            const value = JSON.parse(payload);
            return asRecord(value) ? { value: value } : { error: '根必须是对象' };
        }
        catch (e) {
...
const OFFICIAL_OPENAPI_VALIDATORS = {};
let officialOpenApiCompilerError = null;
/*
...
try {
    ...
    OFFICIAL_OPENAPI_VALIDATORS['3.0'] = openApi30;
    OFFICIAL_OPENAPI_VALIDATORS['3.1'] = openApi31;
}
catch (e) {
    officialOpenApiCompilerError = String(e);
}
function officialOpenApiValidator(version) {
    const validate = OFFICIAL_OPENAPI_VALIDATORS[version];
    return validate
        ? { validate }
        : { error: officialOpenApiCompilerError ?? `OpenAPI ${version} 官方 schema 未完成编译` };
}
function formatSchemaErrors(output) {
    if (output.valid)
        return '';
    const leaves = [];
    const visit = (items) => {
...
function validateOpenApi(payload, extension) {
    const parsed = duplicateAwareObject(payload, extension);
    if (!parsed.value)
        return `OpenAPI 解析失败：${parsed.error}`;
    ...
    if (!version)
        return 'openapi 必须是受支持的 3.0.x 或 3.1.x';
    const official = officialOpenApiValidator(version);
    if (!official.validate)
        return `OpenAPI ${version} 官方 schema validator 不可用：${official.error ?? '未知错误'}`;
    ...
    for (const pathItem of Object.values(paths)) {
        ...
        for (const [method, rawOp] of Object.entries(item)) {
            if (!/^(get|put|post|delete|patch|options|head|trace)$/.test(method))
                continue;
            const op = asRecord(rawOp);
            if (!op || typeof op.operationId !== 'string' || op.operationId.trim() === '')
                return `operation ${method} 缺 operationId`;
            if (operationIds.has(op.operationId))
                return `operationId 重复：${op.operationId}`;
            operationIds.add(op.operationId);
        }
    }
    if (operationIds.size === 0)
        return 'paths 中没有 operation';
    const refs = [];
    collectRefs(root, refs);
    const broken = refs.find(r => !pointerExists(root, r));
    if (broken)
        return `本地 $ref 无法解析：${broken}`;
    const lower = payload.toLowerCase();
    if (!/(security|bearer|oauth|api[-_ ]?key)/i.test(lower))
        return '缺鉴权/security 定义';
    if (!/(deprecated|compatib|兼容|弃用)/i.test(lower))
        return '缺兼容或弃用语义';
    if (!/(4\d\d|5\d\d|error|错误)/i.test(lower))
        return '缺错误响应语义';
    return null;
}
function validateOpenLogosRootJsonSchema(payload) {
    const parsed = duplicateAwareObject(payload, '.json');
    if (!parsed.value)
        return `JSON Schema 严格解析失败：${parsed.error}`;
    const root = parsed.value;
    ...
    if (typeof root.$id !== 'string' || root.$id.trim() === '')
        return '根 JSON Schema 必须声明非空 `$id`';
    if (root.type !== 'object')
        return '根 JSON Schema 顶层 type 必须为 object';
    if (!asRecord(root.properties) || Object.keys(asRecord(root.properties)).length === 0)
        return '根 JSON Schema 必须声明非空 properties';
    return null;
}
...
export function resolveProjectDatabaseDialect(root) {
    const indexPath = join(root, 'logos', 'logos-project.yaml');
    if (!existsSync(indexPath))
        return { error: '缺少 logos/logos-project.yaml，无法确定 SQL 方言' };
    const doc = parseDocument(readFileSync(indexPath, 'utf-8'), { uniqueKeys: true, strict: true, prettyErrors: false });
    if (doc.errors.length > 0)
        return { error: `logos-project.yaml 严格解析失败：${doc.errors.map(e => e.message).join('；')}` };
    let rootValue;
    try {
        rootValue = doc.toJS({ maxAliasCount: 100 });
    }
    catch (e) {
        return { error: `logos-project.yaml 转换失败：${String(e)}` };
    }
    ...
    if (candidates.length === 0)
        return { error: 'tech_stack.database 未声明 SQL 方言' };
    const normalized = [...new Set(candidates.map(normalizeDatabaseDialect))];
    if (normalized.includes(null))
        return { error: `tech_stack.database 含不支持的方言：${candidates.join('、')}` };
    if (normalized.length !== 1)
        return { error: `tech_stack.database 方言冲突：${candidates.join('、')}` };
    return { dialect: normalized[0] };
}
function validateSql(payload, dialect) {
    // 类别最低完整度与真正 parser/执行预检是两层门；关键词只负责提示缺少哪种设计资产。
    if (!/\bCREATE\s+TABLE\b/i.test(payload))
        return '缺 CREATE TABLE';
    if (!/\bPRIMARY\s+KEY\b/i.test(payload))
        return '缺主键';
    if (!/\b(CONSTRAINT|FOREIGN\s+KEY|UNIQUE|CHECK)\b/i.test(payload))
        return '缺约束';
    if (!/\bCREATE\s+(?:UNIQUE\s+)?INDEX\b/i.test(payload))
        return '缺索引';
    if (!/(migration|migrate|迁移)/i.test(payload) || !/(rollback|回滚)/i.test(payload))
        return '缺迁移/回滚语义';
    if (dialect !== 'sqlite') {
    ...
    if (sqlite.error)
        return `SQLite validator 不可用：${sqlite.error.message}`;
    if (sqlite.status !== 0)
        return `SQLite 执行预检失败：${(sqlite.stderr || '').trim()}`;
    ...
    if (tables < 1)
        return 'SQLite schema 预检后没有用户表';
    if (indexes < 1)
        return 'SQLite schema 预检后没有显式索引';
    return null;
}
/**
 * 校验并剥离 API/DB non-Markdown delta 首行。返回的 payload 不含 marker，调用方不得把 marker 写入目标。
 */
export function validateAndStripNonMarkdownDelta(content, mode, canonicalTargetPath, options = {}) {
    const newline = content.indexOf('\n');
    if (newline < 0)
        return { ok: false, message: '整文件 delta 缺 payload 或首行行结束符' };
    const first = content.slice(0, newline).replace(/\r$/, '');
    const marker = NON_MD_MARKER.exec(first);
    if (!marker)
        return { ok: false, message: '首行控制 marker 不合法' };
    const expectedOp = mode === 'CREATE' ? 'ADDED' : 'MODIFIED';
    const expectedSuffix = mode === 'CREATE' ? '（新文件，整文件）' : '（整文件替换）';
    if (marker[1] !== expectedOp || marker[3] !== expectedSuffix)
        return { ok: false, message: `首行 mode 与 ${mode} 不一致` };
    if (marker[2] !== canonicalTargetPath)
        return { ok: false, message: `首行 target 与 canonical target 不一致：${marker[2]}` };
    const payload = content.slice(newline + 1);
    if (payload.trim() === '')
        return { ok: false, message: '剥离 marker 后 payload 为空' };
    if (/^## (?:ADDED|MODIFIED) — /m.test(payload))
        return { ok: false, message: 'payload 内残留控制 marker' };
    if (/\b(?:TODO|TBD)\b|后续补充|\[新增的完整内容\]/i.test(payload))
        return { ok: false, message: 'payload 含模板/TODO 骨架' };
    ...
    if (canonicalTargetPath.startsWith('logos/resources/api/') && ['.yaml', '.yml', '.json'].includes(ext)) {
        ...
        if (!resolved.dialect)
            return { ok: false, message: resolved.error ?? '无法确定 SQL 方言' };
...
function markdownCreatePayload(content) {
    ...
    if (added.length === 0)
        problems.push('Markdown CREATE 缺围栏外 ADDED 控制段');
    const forbidden = [...new Set(markers.filter(marker => marker.op !== 'ADDED').map(marker => marker.op))];
    if (forbidden.length > 0)
        problems.push(`Markdown CREATE 禁止 ${forbidden.join('、')} 控制段`);
    const chunks = [];
...
function matchingHeadings(structure, predicate) {
    return structure.headings.filter(heading => predicate(normalizedHeading(heading.text)));
}
function sectionEnd(structure, heading) {
...
function sectionHasAuthorityBody(structure, heading) {
    ...
    for (let i = heading.line + 1; i < end; i++) {
        if (structure.scan.masked[i] || headingLines.has(i))
            continue;
        if (structure.scan.text[i].trim().length > 0)
            return true;
    }
    return false;
}
function requiredSectionProblem(structure, predicate, label) {
    const headings = matchingHeadings(structure, predicate);
    if (headings.length === 0)
        return `${label}章节缺失`;
    if (!headings.some(heading => sectionHasAuthorityBody(structure, heading)))
        return `${label}章节为空`;
    return null;
}
function uniqueSectionProblems(structure, predicate, label) {
    const headings = matchingHeadings(structure, predicate);
    if (headings.length === 0)
        return [`${label}章节缺失`];
    if (headings.length > 1)
...
function stepSectionProblems(structure, heading) {
    const end = sectionEnd(structure, heading);
    let currentRun = 0;
    let longestRun = 0;
    let itemCount = 0;
    ...
    for (let i = heading.line + 1; i < end; i++) {
        ...
        if (line.trim() === '' || (currentRun > 0 && /^(?: {2,}|\t)\S/.test(line)))
            continue;
        currentRun = 0;
    }
    const problems = [];
    if (itemCount === 0)
        problems.push('步骤章节无有序列表');
    else {
        if (longestRun < 3)
            problems.push('步骤有序列表少于 3 项');
        if (hasEmptyItem)
            problems.push('步骤有序列表存在空项');
    }
    return problems;
}
function mermaidProblems(structure) {
    ...
    if (diagrams.length === 0)
        return ['时序缺合法 Mermaid sequenceDiagram'];
...
function scenarioCreateCompletenessProblems(payload) {
    ...
    if (goal)
        problems.push(goal);
    const participants = requiredSectionProblem(structure, text => /^(?:参与者|participants?)$/i.test(text), '参与者');
    if (participants)
        problems.push(participants);
    const pre = requiredSectionProblem(structure, text => /前置|preconditions?/i.test(text), '前置');
    const post = requiredSectionProblem(structure, text => /后置|postconditions?/i.test(text), '后置');
    if (pre || post)
        problems.push(pre && post ? '前后置章节缺失或为空' : (pre ?? post));
    ...
    if (stepHeadings.length === 0)
        problems.push('步骤章节缺失');
    else if (stepHeadings.length > 1)
        problems.push('步骤章节重复');
    else
        problems.push(...stepSectionProblems(structure, stepHeadings[0]));
    problems.push(...uniqueSectionProblems(structure, text => /^(?:异常|边界|异常(?:与|及|\/|&)边界|异常路径|边界条件|exceptions?|boundar(?:y|ies)|exceptions?\s*(?:and|&|\/)\s*boundar(?:y|ies))$/i.test(text), '异常/边界'));
    problems.push(...uniqueSectionProblems(structure, text => /^(?:追溯|可追溯性|trace|traceability)$/i.test(text), '追溯'));
    if (structure.malformed.length > 0)
        problems.push(`Markdown 结构无法确定：${structure.malformed.join('、')}`);
    return problems;
}
function createCompletenessProblems(category, content) {
    const create = markdownCreatePayload(content);
    if (create.problems.length > 0)
        return create.problems;
    const payload = create.payload;
    if (/\b(?:TODO|TBD)\b|后续补充|\[(?:新增|修改).*内容\]/i.test(payload))
        return ['含模板/TODO 骨架'];
    if (category === 'scenario')
        return scenarioCreateCompletenessProblems(payload);
    ...
    if (category === 'test') {
        ...
        if (!ids.some(id => /^UT-/.test(id)) || !ids.some(id => /^ST-/.test(id)))
            missing.push('真实 UT/ST ID');
    }
    return missing;
}
function closureViolation(code, path, message, fixHint) {
    return { code, path, message, fix_hint: fixHint };
}
function addDuplicateViolations(values, violations) {
    const grouped = new Map();
    for (const value of values)
        grouped.set(value.canonicalTargetPath, [...(grouped.get(value.canonicalTargetPath) ?? []), value.sourcePath]);
    for (const [target, paths] of grouped) {
        if (paths.length < 2)
            continue;
        for (const path of paths) {
            violations.push(closureViolation('delta_target_duplicate', path, `canonical target ${target} 出现 ${paths.length} 次（${paths.join('、')}）`, '按 canonical target 聚合场景与改动，只保留一条 task 和一份最终态 delta'));
        }
    }
}
/** 共享 ClosureEvaluator：legacy 返回 active=false；on-touch-v1 对 plan/spec 执行完整闭包。 */
export function evaluateBaselineClosure(options) {
    const locked = withRecoveredReadLocks(options.root, new Date().toISOString(), listBaselineSeedModuleIds(options.root), () => evaluateBaselineClosureLocked(options));
    if (!locked.ok) {
        throw new BaselineCommitInProgressError(`baseline_commit_in_progress — 模块 ${locked.inProgress.join(', ')} 的未终结 journal 无法在闭包读取前恢复`);
    }
    return locked.value;
}
function evaluateBaselineClosureLocked(options) {
    ...
    const extracted = extractClosureYaml(proposalContent);
    if (!extracted.present) {
        if (!parsedTasks.hasModeSyntax)
            return { active: false, violations: [] };
        return {
            active: true,
            violations: [closureViolation('baseline_closure_declaration_missing', proposalRel, '[delta] 已使用 [MODIFY]/[CREATE]，但 proposal 缺少 on-touch-v1 基线闭包声明', '在 proposal.md 的「## 基线闭包计划」写入唯一 baseline_closure fenced YAML；不得从 tasks 反推计划')],
        };
    }
    const parsed = parseBaselineClosurePlan(root, proposalDir, proposalContent, proposalRel);
    if (!parsed.plan)
        return { active: true, violations: parsed.violations };
    ...
    for (const target of plan.targets) {
        if (target.mode === 'AMBIGUOUS') {
            violations.push(closureViolation('baseline_closure_ambiguous', proposalRel, `${target.category}/${target.scenarioIds.join(',')} 仍为 AMBIGUOUS：${target.missingEvidence.join('；')}`, `在既有 plan-exit 前补齐证据并改为 MODIFY/CREATE/SKIP；缺失证据：${target.missingEvidence.join('；')}`));
        }
    }
    ...
    for (const scenario of plan.touchedScenarioIds) {
        for (const category of REQUIRED_SCENARIO_DIMENSIONS) {
            ...
            if (!covered) {
                violations.push(closureViolation('baseline_closure_target_missing', proposalRel, `touched scenario ${scenario} 缺少 ${category} ${strong ? '强制目标' : '条件维度 disposition'}`, `为 ${scenario} 增加 ${category} 的 MODIFY/CREATE${strong ? '' : '/SKIP/AMBIGUOUS'} target；不得从其它场景或 tasks 反推`));
            }
        }
        ...
        if ((apiMaterial && !orchestrationMaterial) || (apiSkipOnly && !orchestrationSkipOnly)) {
            violations.push(closureViolation('baseline_closure_target_missing', proposalRel, `${scenario} 的 API 与 orchestration disposition 不一致`, 'API 为 MODIFY/CREATE 时 orchestration 同为 MODIFY/CREATE；API 为 SKIP 时 orchestration 只能 SKIP'));
        }
    }
    ...
    if (deploymentDecision.deployment_decision_conflict) {
        violations.push(closureViolation('baseline_closure_target_missing', proposalRel, `部署决策与 tasks [deploy] 冲突：${deploymentDecision.deployment_decision_conflict_reason ?? '未知冲突'}`, '先统一 proposal「部署影响」与 tasks [deploy] section，再按同一决定填写 deployment/smoke disposition'));
    }
    ...
    if (!hasExplicitProposalDeploymentDecision) {
        violations.push(closureViolation('baseline_closure_target_missing', proposalRel, `on-touch-v1 的部署结论必须来自当前 proposal，实际来源=${deploymentDecision.deployment_decision_source}`, '在当前 proposal「部署影响」中同时明确「是否需要部署」与「是否需要 smoke」；不得用 tasks、模块默认或 legacy fallback 代替'));
    }
    else if (strictDeploymentRequired === null || strictSmokeRequired === null) {
        violations.push(closureViolation('baseline_closure_target_missing', proposalRel, 'on-touch-v1 要求 proposal 同时明确「是否需要部署」与「是否需要 smoke」', '在 proposal「部署影响」中把两个字段明确为是/否，再让 deployment/smoke targets 与之对账'));
    }
    else if (!strictDeploymentRequired && strictSmokeRequired) {
        violations.push(closureViolation('baseline_closure_target_missing', proposalRel, 'proposal 声明无需部署但需要 smoke，部署决策自身不一致', '修正部署影响：无需部署时 smoke 必须为否；需要发布后 smoke 时部署必须为是'));
    }
    const dispositionMatches = (scenario, category, required) => {
        if (required === null)
            return;
        ...
        if ((required && !material) || (!required && !skipOnly)) {
            violations.push(closureViolation('baseline_closure_target_missing', proposalRel, `${scenario} 的 ${category} disposition 与 proposal 决策不一致：决策=${required ? '需要' : '不需要'}，targets=${dispositions.map(d => d.mode).join('/') || '缺失'}`, required
    ...
    for (const target of planMaterial) {
        ...
        if (exists !== expected) {
            violations.push(closureViolation('delta_target_mode_mismatch', proposalRel, `${target.mode} 与磁盘事实不一致：${target.targetPath} 当前${exists ? '存在' : '缺失'}`, specMerged
    ...
    for (const [key, target] of planByTarget) {
        ...
        if (!task) {
            violations.push(closureViolation('baseline_closure_target_missing', tasksRel, `proposal 目标 ${key} 未进入 [delta] task`, `新增唯一 [${target.mode}] \`${target.deltaPath}\` task，并聚合同目标全部场景`));
        }
        else if (task.mode !== target.mode) {
            violations.push(closureViolation('delta_target_mode_mismatch', tasksRel, `${key} 的 task mode=${task.mode}，proposal mode=${target.mode}`, `把 task mode 改为 ${target.mode}，不得以 task 覆盖 proposal 权威计划`));
        }
    }
    for (const [key, task] of taskByTarget) {
        if (!planByTarget.has(key)) {
            violations.push(closureViolation('delta_target_unplanned', tasksRel, `[delta] task ${key} 不在 proposal targets`, `删除未规划 task，或先在 proposal baseline_closure.targets 声明 ${task.mode} 目标并补证据`));
        }
    }
    const actual = [];
    for (const entry of deltaEntries) {
        if (!(entry.mergeDisposition === 'mergeable' && entry.lintValidity === 'valid'))
            continue;
        const resolution = resolveCanonicalMergeTarget(root, proposalDir, entry.relativePath);
        if (resolution)
            actual.push({ entry, resolution });
    }
    ...
    const stage = parsedTasks.targets.length > 0
        && parsedTasks.targets.every(t => t.checked) ? 'spec' : 'plan';
    if (stage === 'spec') {
        for (const [key] of taskByTarget) {
            if (!actualByTarget.has(key)) {
                violations.push(closureViolation('delta_target_unplanned', tasksRel, `已完成 task ${key} 缺少实际 delta`, '写入该目标唯一 delta，或把尚未完成的 task 恢复为未勾选'));
            }
        }
    }
    for (const [key, item] of actualByTarget) {
        if (!taskByTarget.has(key) || !planByTarget.has(key)) {
            violations.push(closureViolation('delta_target_unplanned', `logos/changes/${slug}/${item.entry.relativePath}`, `实际 delta ${key} 未同时出现在 proposal 与 tasks`, '删除未规划 delta；如确属变更范围，先在 proposal 声明并新增唯一 task'));
        }
    }
    ...
    for (const [key, item] of actualByTarget) {
        ...
        if (nonMarkdown) {
            const checked = validateAndStripNonMarkdownDelta(content, target.mode, key, { root });
            if (!checked.ok) {
                violations.push(closureViolation('non_markdown_delta_invalid', `logos/changes/${slug}/${item.entry.relativePath}`, checked.message ?? 'non-Markdown delta 无效', '按精确首行协议声明 canonical target；只剥离首行后对 OpenAPI/SQL payload 做严格解析/执行预检'));
            }
        }
        else if (prototypeAsset) {
            if (content.trim() === '') {
                violations.push(closureViolation('create_target_incomplete', `logos/changes/${slug}/${item.entry.relativePath}`, 'prototype CREATE payload 为空', '写入经 UI provenance 校验的非空原型资产'));
            }
        }
        else if (target.mode === 'CREATE') {
            const missing = createCompletenessProblems(target.category, content);
            if (missing.length > 0) {
                violations.push(closureViolation('create_target_incomplete', `logos/changes/${slug}/${item.entry.relativePath}`, `${target.category} CREATE 缺最低完整度字段：${missing.join('、')}`, '补成可独立合并的最终态全量文档，不得只追加补充段、TODO 或模板骨架'));
            }
        }
    }
    const counts = (mode) => plan.targets.filter(t => t.mode === mode).length;
    const summary = {
...
export function effectiveTargetView(root, target, deltaContents) {
    const locked = withRecoveredReadLocks(root, new Date().toISOString(), listBaselineSeedModuleIds(root), () => effectiveTargetViewLocked(root, target, deltaContents));
    if (!locked.ok)
        throw new BaselineCommitInProgressError();
    return locked.value;
}
function effectiveTargetViewLocked(root, target, deltaContents) {
    if (!target.targetPath || !target.deltaPath)
        return { ok: false, reason: 'SKIP/AMBIGUOUS 无 effective target view' };
    let mergedBytes = null;
    const abs = join(root, target.targetPath);
    if (existsSync(abs)) {
        try {
            mergedBytes = readFileSync(abs, 'utf-8');
        }
        catch {
            return { ok: false, reason: `无法读取 ${target.targetPath}` };
        }
    }
...
export function scanCommittedEvidenceFiles(root, paths) {
    const locked = withRecoveredReadLocks(root, new Date().toISOString(), listBaselineSeedModuleIds(root), () => scanCommittedEvidenceFilesLocked(root, paths));
    if (!locked.ok)
        throw new BaselineCommitInProgressError();
    return locked.value;
}
function scanCommittedEvidenceFilesLocked(root, paths) {
    const resources = join(root, 'logos', 'resources');
    let realResources;
    try {
        realResources = realpathSync(resources);
    }
    catch {
        return [];
    }
    return paths.map(p => p.replace(/\\/g, '/')).filter(p => {
        if (p.includes('/baseline-seed-runs/') || /\/(?:staging|resolved|backup)\//.test(`/${p}`))
            return false;
        const abs = join(root, ...p.split('/'));
        if (!existsSync(abs))
            return false;
        try {
            return contained(realpathSync(abs), realResources);
        }
        catch {
            return false;
        }
...
/** 供测试/诊断显示 root 相对位置。 */
export function projectRelativePath(root, path) {
    return relative(root, path).replace(/\\/g, '/');
}
//# sourceMappingURL=baseline-closure.js.map