...
import { readProjectYaml as readRecoveredProjectYaml } from '../lib/project-yaml.js';
function readModuleYaml(root) {
    const path = join(root, 'logos', 'logos-project.yaml');
    if (!existsSync(path))
        return { raw: {}, modules: [], diagnostics: null };
    try {
        const raw = (parseYaml(readFileSync(path, 'utf-8')) ?? {});
        return { raw, modules: raw.modules ?? [], diagnostics: null };
    }
    catch {
        const recovered = readRecoveredProjectYaml(root);
        return {
            raw: null,
            modules: (recovered.data?.modules ?? []),
            diagnostics: recovered.yaml_diagnostics ?? { parse_status: 'error', messages: [] },
        };
    }
}
function yamlErrorDetail(diagnostics) {
 ...
 */
function rejectDegradedYaml(read, command, format, forWrite) {
    if (!read.diagnostics)
        return;
...
function writeProjectYaml(root, data) {
    const path = join(root, 'logos', 'logos-project.yaml');
    writeFileSync(path, stringifyYaml(data, { lineWidth: 0 }));
}
function warnIfActiveChange(root, operation) {
...
function walkAndCollect(dir, prefix, results) {
    if (!existsSync(dir))
        return;
...
function updateCrossReferences(root, oldName, newName) {
    const updated = [];
    const scanDirs = [
        join(root, 'logos'),
        join(root, 'spec'),
    ...
    function scanDir(dir) {
        ...
        for (const entry of readdirSync(dir)) {
            ...
            if (st.isDirectory()) {
                ...
                const updated_content = content.replace(new RegExp(`(?<![a-z0-9-])${oldName}-`, 'g'), `${newName}-`);
...
export function moduleList(format = 'text') {
    ...
    const sorted = [...modules].sort((a, b) => {
        if (a.lifecycle === 'initial' && b.lifecycle !== 'initial')
            return -1;
        if (b.lifecycle === 'initial' && a.lifecycle !== 'initial')
            return 1;
        return 0;
    });
...
export function moduleAdd(name, productType) {
    const root = process.cwd();
    checkConfig(root);
    if (!name) {
 ...
 */
export function moduleSetProductType(moduleId, productType, format = 'text') {
    const root = process.cwd();
    checkConfig(root);
    // 缺参 → 用法错误，不写文件。
    if (!moduleId || !productType) {
        if (format === 'json') {
            process.stderr.write(JSON.stringify(makeErrorEnvelope('module set-product-type', 'MISSING_ARGUMENT', 'Usage: openlogos module set-product-type <module-id> <enum>')) + '\n');
        }
        else {
    ...
    if (!isValidProductType(productType)) {
        ...
        if (format === 'json') {
            process.stderr.write(JSON.stringify(makeErrorEnvelope('module set-product-type', 'INVALID_PRODUCT_TYPE', msg)) + '\n');
        }
        else {
            console.error(`Error: ${msg}`);
        }
    ...
    if (!mod) {
        ...
        if (format === 'json') {
            process.stderr.write(JSON.stringify(makeErrorEnvelope('module set-product-type', 'MODULE_NOT_FOUND', msg)) + '\n');
        }
        else {
            console.error(`Error: ${msg}`);
        }
...
export function moduleRename(oldName, newName) {
    const root = process.cwd();
    checkConfig(root);
    ...
    if (refUpdated.length > 0) {
        ...
        for (const f of refUpdated)
            console.log(`  ${f}`);
    }
}
export function moduleRemove(name) {
    const root = process.cwd();
    checkConfig(root);
    ...
    rl.question(`\nConfirm removal of module "${name}"? (yes/no): `, (answer) => {
        rl.close();
