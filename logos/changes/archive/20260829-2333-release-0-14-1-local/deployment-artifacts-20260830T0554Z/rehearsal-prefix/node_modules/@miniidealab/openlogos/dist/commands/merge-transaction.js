...
import { abortMergeTransaction, applyMergeTransaction, createMergeTransaction, MergeTransactionError, MERGE_TRANSACTION_ACTION_COMMANDS, readMergeTransaction, recoverMergeTransaction, sealMergeTransaction, submitMergeContent, } from '../lib/merge-transaction.js';
function resolveIdentity(root, explicitSlug) {
    ...
    if (!slug || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug))
        throw new Error('缺少或非法的提案 slug');
    const proposalDir = join(root, 'logos', 'changes', slug);
    if (!existsSync(proposalDir))
        throw new Error(`提案不存在：${slug}`);
    return { slug, proposalDir };
}
function emit(format, projection) {
    if (format === 'json')
        console.log(JSON.stringify(makeEnvelope('merge transaction', { merge_transaction: projection })));
    else {
...
function emitError(format, error) {
    ...
    if (format === 'json') {
        ...
        console.error(`Error: merge transaction 失败（${txError?.classification ?? 'internal_failure'}）：${message}`);
    process.exit(1);
    throw new Error('unreachable');
}
export function mergeTransactionCommand(subcommand, args, format) {
    ...
    try {
        ...
        let result;
        if (command === 'status')
            result = readMergeTransaction(proposalDir);
        else if (command === 'seal')
            result = sealMergeTransaction(root, proposalDir);
        else if (command === 'apply')
            result = applyMergeTransaction(root, proposalDir);
        else if (command === 'recover')
            result = recoverMergeTransaction(root, proposalDir);
        else if (command === 'abort')
            result = abortMergeTransaction(proposalDir);
        else {
            ...
            if (!slot || !fileArg)
                throw new MergeTransactionError('slot_identity_mismatch', 'submit-content 需要 --slot 与 --file', true);
            const file = isAbsolute(fileArg) ? fileArg : resolve(root, fileArg);
            if (!existsSync(file))
                throw new MergeTransactionError('slot_identity_mismatch', `content 文件不存在：${fileArg}`, true);
            result = submitMergeContent(proposalDir, slot, file);
        }
        if (result.slug !== slug)
            throw new MergeTransactionError('target_set_mismatch', 'transaction slug 漂移', false, result);
        emit(format, result);
    }
    catch (error) {
...
export function beginMergeTransaction(root, proposalDir, slug) {
