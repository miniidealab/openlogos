...
import { classifyImpact, isRevArgLexicallyValid, isValidPrefixArg, IMPACT_SCHEMA_VERSION, } from '../lib/impact-classify.js';
const COMMAND = 'impact';
function opError(format, code, message) {
    if (format === 'json') {
...
function readStdinUtf8() {
    ...
    while (true) {
        ...
        if (count === 0)
            break;
        total += count;
        if (total > GIT_MAX_BUFFER_BYTES)
            throw new Error(`stdin exceeds ${GIT_MAX_BUFFER_BYTES} bytes`);
        chunks.push(Buffer.from(buffer.subarray(0, count)));
    }
    return Buffer.concat(chunks, total).toString('utf8');
}
function gitReadOnly(cwd, args) {
...
function resolveCommitOid(cwd, rev) {
...
export function impact(args, format = 'text') {
...
function printText(locale, result) {
