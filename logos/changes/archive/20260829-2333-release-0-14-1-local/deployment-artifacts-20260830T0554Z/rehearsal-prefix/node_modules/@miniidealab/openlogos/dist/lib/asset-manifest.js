...
export const ASSET_MANIFEST_SCHEMA = 'openlogos/asset-manifest@1';
function sha256(bytes) {
    return createHash('sha256').update(bytes).digest('hex');
}
function canonicalPayload(value) {
    const sorted = { ...value };
    for (const group of ['skills', 'templates', 'schemas', 'plugins']) {
        sorted[group] = [...value[group]].sort((a, b) => a.path.localeCompare(b.path));
    }
    return JSON.stringify(sorted);
}
export function buildAssetManifest(version, planContractVersion, sources) {
    const groups = { skills: [], templates: [], schemas: [], plugins: [] };
...
export function validateAssetManifest(manifest, packageRoot) {
    ...
    for (const group of ['skills', 'templates', 'schemas', 'plugins']) {
        for (const entry of manifest[group]) {
            if (entry.path.startsWith('/') || entry.path.split('/').includes('..'))
                throw new Error(`asset path 非法：${entry.path}`);
            ...
            if (!existsSync(path)) {
                ...
                if (prefix)
                    path = resolve(packageRoot, developmentFallbacks[prefix], entry.path.slice(prefix.length));
            }
            const actual = existsSync(path) ? sha256(readFileSync(path)) : 'missing';
            if (actual !== entry.sha256)
                throw new Error(`asset hash 不匹配：${entry.path} expected=${entry.sha256} actual=${actual}`);
        }
    }
}
...
export function readBundledAssetManifest(path = bundledManifestPath()) {
    const parsed = JSON.parse(readFileSync(path, 'utf8'));
    if (parsed.schema !== ASSET_MANIFEST_SCHEMA)
        throw new Error(`不支持 asset manifest schema：${String(parsed.schema)}`);
    return parsed;
}
export function writeSyncStamp(path, manifest, syncedAt = new Date().toISOString()) {
    if (existsSync(path)) {
        try {
            ...
            if (current.cliVersion === manifest.version
                ...
                && typeof current.syncedAt === 'string')
                return current;
        }
...
export function assetCacheKey(manifest) {
    return `${manifest.version}-${manifest.payloadHash}`;
}
export function deriveManagedAssetsDiagnostic(root, manifest) {
    ...
    try {
        ...
        if (typeof stamp.managedAssetsHash !== 'string' || typeof stamp.planContractVersion !== 'string')
...
export function applyManagedAssetsTransaction(writes, failAfter = -1) {
    ...
    try {
        for (let index = 0; index < writes.length; index++) {
            ...
            if (actual !== write.sha256)
                throw new Error(`managed asset source hash 不匹配：${write.sourcePath}`);
            mkdirSync(dirname(write.targetPath), { recursive: true });
            const temp = `${write.targetPath}.openlogos-tmp-${process.pid}-${index}`;
            copyFileSync(write.sourcePath, temp);
            temps.push(temp);
            if (sha256(readFileSync(temp)) !== write.sha256)
                throw new Error(`managed asset staged hash 不匹配：${write.targetPath}`);
        }
        for (let index = 0; index < writes.length; index++) {
            renameSync(temps[index], writes[index].targetPath);
            if (index === failAfter)
                throw new Error(`managed asset fault injection: ${index}`);
        }
    }
    catch (error) {
        for (const backup of backups) {
            if (backup.existed)
                writeFileSync(backup.targetPath, backup.bytes);
            else
