...
import { readVerifyConfig, } from '../lib/verify-config.js';
import { buildInitialSandboxData, normalizeSandboxConfig, runSandboxedCommand, } from '../lib/sandbox.js';
import { getDeployTasks } from './status.js';
import { checkSmokeCoverage, resolveSmokeCommand } from '../lib/smoke-coverage.js';
import { deriveAutomationDiagnostic } from '../lib/automation-diagnostic.js';
import { appendSliceCheckpoint, deriveSliceVerificationState, } from '../lib/test-slice-manifest.js';
...
function writeMergedResults(root, resultPath, contents) {
...
function skippedCompatPreRunCommand(config) {
    if (!config.preRunCommand || !(config.regressionCommand || config.incrementalCommand))
        return null;
    return {
        stage: 'pre_run',
...
function appendSandboxDiagnostics(preRun, sandbox) {
    for (const line of sandbox.diagnostics) {
        if (!preRun.diagnostics.includes(line))
            preRun.diagnostics.push(line);
    }
    for (const line of sandbox.suggestions) {
        if (!preRun.suggestions.includes(line))
            preRun.suggestions.push(line);
    }
}
function executeVerifyCommand(root, stage, command, format, sandboxConfig, allowedWritePaths) {
    if (format !== 'json') {
...
function toPreRunCommandResult(stage, command, result, sandbox) {
    return {
        stage,
        command,
...
function mergeSandboxStatus(current, next) {
    const rank = {
        fail: 4,
        warn: 3,
        pass: 2,
...
export function runVerifyPreRunWithSandbox(root, config, format) {
    ...
    if (config.regressionResultPath)
        allowPaths.add(config.regressionResultPath);
    if (config.incrementalResultPath)
        allowPaths.add(config.incrementalResultPath);
    ...
    const stageContents = [];
    const runStage = (stage, command, stageResultPath) => {
        if (!command)
            return;
...
export function runVerifyPreRun(root, config, format) {
    return runVerifyPreRunWithSandbox(root, config, format).preRun;
}
function addCoverageDiagnostics(root, data) {
    if (data.gate.reason === 'incomplete_coverage' && data.pre_run.mode === 'none') {
        ...
        data.pre_run.suggestions.push(t(locale, 'verify.coverageLocalSuggestionPreRun'), t(locale, 'verify.coverageLocalSuggestionTwoPhase'));
    }
    return data;
}
function readSmokeCommandConfig(root) {
    const configPath = join(root, 'logos', 'logos.config.json');
    let command = null;
    let resultPath = 'logos/resources/verify/smoke-results.jsonl';
    try {
        const config = JSON.parse(readFileSync(configPath, 'utf-8'));
        if (typeof config.smoke?.command === 'string')
            command = config.smoke.command;
        if (typeof config.smoke?.result_path === 'string')
            resultPath = config.smoke.result_path;
    }
    catch {
        // 使用默认 smoke 配置。
    }
...
function applySmokePrecheck(root, data) {
    ...
    if (check.changed_case_ids.length === 0 || check.result === 'PASS')
        return data;
    ...
    if (data.verify_mode === 'slice-checkpoint'
        && check.changed_case_ids.every(id => pending.has(id)))
        return data;
    ...
    data.pre_run.suggestions.push('为本提案新增的 SMOKE-* 用例实现 scripts/smoke-*.sh 或等效 runner。', '确保 smoke runner 写入 logos/resources/verify/smoke-results.jsonl 或 smoke.result_path。', '将 smoke.command 指向 scripts/run-smoke.js 统一 dispatcher，或显式执行新增 runner。');
    return data;
}
function ensureResultFileWhenPreRunFailed(fullResultPath, preRun) {
 ...
 */
export function selectEffectiveResult(rows) {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (compareStrictTimestamps(times[i], times[best]) >= 0)
            best = i;
    }
    return rows[best];
}
...
function consistencyReasonForInvalid(reason) {
    if (reason === 'invalid_status')
        return 'invalid_test_result_status';
    if (reason === 'invalid_json')
        return 'invalid_test_result_json';
...
export function parseJsonlWithDiagnostics(content) {
...
function extractDefinedIndex(root) {
    const dir = join(root, TEST_CASES_DIR);
    if (!existsSync(dir))
        return { ids: [], utCount: 0, stCount: 0, manualCount: 0, manualIds: [] };
    const idSet = new Set();
    ...
    try {
        ...
        for (const file of files) {
            ...
            for (const line of content.split('\n')) {
                ...
                if (!TABLE_CELL_ID_PATTERN.test(firstCell))
                    continue;
...
export function extractAcTrace(root) {
    const dir = join(root, TEST_CASES_DIR);
    if (!existsSync(dir))
        return [];
    const entries = [];
    try {
        ...
        for (const file of files) {
            const content = readFileSync(join(dir, file), 'utf-8');
            if (!AC_TABLE_HEADER.test(content))
                continue;
...
export function generateReport(defined, results, passed, failed, skipped, uncovered, coveragePct, passRatePct, gateResult, checklist, acTrace, _resultIds, manualCount, sliceVerification) {
    ...
    if (acTrace.length > 0) {
        ...
        const acManual = acTrace.filter(ac => ac.linkedCaseIds.length > 0 && !ac.linkedCaseIds.some(cid => resultMap.has(cid))).length;
        ...
        if (acManual > 0)
            md += ` 🔵 ${acManual} criteria are manual-only (pending human verification).`;
        if (acWithoutLinks > 0)
...
export function buildVerifyCountMismatches(summary) {
    const mismatches = [];
...
function buildVerifyConsistency(params) {
    ...
    if (params.unknownResultIds.length > 0)
        pushUnique(reasons, 'unknown_test_result_id');
    if (params.manualResultIds.length > 0)
        pushUnique(reasons, 'manual_test_result_id');
    if (params.outsideEligibleResultIds.length > 0)
        pushUnique(reasons, 'result_outside_eligible_scope');
    if (params.countMismatches.length > 0)
        pushUnique(reasons, 'result_count_mismatch');
    return {
        ok: reasons.length === 0,
        reasons,
...
export function collectVerifyData(root, preRun, sliceVerification) {
    ...
    const manualResultIds = [];
    const outsideEligibleResultIds = [];
    for (const result of allResults) {
        if (definedSet.has(result.id))
            continue;
    ...
    const acFailed = acTrace.filter(ac => {
        if (ac.linkedCaseIds.length === 0)
            return true;
        const automatedIds = ac.linkedCaseIds.filter(cid => resultMap.has(cid));
        if (automatedIds.length === 0)
            return false; // 全 manual → MANUAL_PENDING，不失败
    ...
    const gateResult = isPass ? 'PASS' : 'FAIL';
    let gateReason = null;
    if (!isPass) {
        if (!consistency.ok)
            gateReason = 'result_ledger_inconsistent';
        else if (failed.length > 0)
            gateReason = 'failed_cases';
        else if (uncovered.length > 0)
            gateReason = 'incomplete_coverage';
        else if (checklistUnchecked.length > 0)
            gateReason = 'checklist_incomplete';
        else
            gateReason = 'ac_trace_incomplete';
    }
    const reportPath = join(root, REPORT_DIR, 'acceptance-report.md');
    mkdirSync(dirname(reportPath), { recursive: true });
    writeFileSync(reportPath, generateReport(defined, results, passed, failed, skipped, uncovered, String(coveragePct), String(passRatePct), gateResult, checklist, acTrace, resultIds, manualCount, sliceVerification));
    ...
    if (existsSync(guardPath)) {
        try {
            ...
            if (guard.activeChange)
                proposalDir = join(root, 'logos', 'changes', String(guard.activeChange));
        }
        catch {
            proposalDir = null;
        }
    ...
    if (automationDiagnostic)
        data.automation_diagnostic = automationDiagnostic;
    return data;
}
/**
 ...
 */
function appendLoopIter(root, gatePass, sliceVerification) {
    ...
    if (activeChange && guardModule) {
        ...
        if (inferLifecycle(root) === 'launched')
            return;
        lifecycle = 'initial';
        proposalDir = null;
        ...
        if (mods.length > 1)
            return; // initial 多模块：无法归属、不写（R7）
        moduleId = mods.length === 1 ? mods[0].id : 'core';
    }
    let resolved;
    try {
        resolved = loadFlow(root, { lifecycle, resolved: true }).flow;
    }
    catch {
        return;
    }
...
function activeSliceVerification(root) {
    const guardPath = join(root, 'logos', '.openlogos-guard');
    if (!existsSync(guardPath))
        return null;
    try {
...
function stopForSliceManifestState(format, state) {
    ...
    if (format === 'json')
        ...
        for (const item of state.violations ?? [])
...
export function verify(format = 'text') {
    const root = process.cwd();
    const configPath = join(root, 'logos', 'logos.config.json');
    if (!existsSync(configPath)) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('verify', 'PROJECT_NOT_INITIALIZED', 'logos/logos.config.json not found.')));
            process.exit(1);
        }
    ...
    try {
        ...
        if (previousEligible === undefined)
            delete process.env.OPENLOGOS_VERIFY_ELIGIBLE_TEST_IDS;
        else
            process.env.OPENLOGOS_VERIFY_ELIGIBLE_TEST_IDS = previousEligible;
    }
    ...
    if (!existsSync(fullResultPath)) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('verify', 'NO_TEST_RESULTS', `No test results found at ${resultPath}.`)));
            process.exit(1);
        }
    ...
    if (defined.length === 0) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('verify', 'NO_TEST_CASES', 'No test case specs found in logos/resources/test/.')));
            process.exit(1);
        }
