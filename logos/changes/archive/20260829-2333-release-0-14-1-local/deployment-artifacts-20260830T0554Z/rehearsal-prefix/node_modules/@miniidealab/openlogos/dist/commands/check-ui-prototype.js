 ...
 * 无法唯一确定 → null
 */
export function resolveActiveProposal(root, slug) {
    if (slug && slug.trim()) {
        const s = slug.trim();
    ...
    if (existsSync(guardPath)) {
        try {
            ...
            if (active)
                return { slug: active, proposalDir: join(root, 'logos', 'changes', active) };
        }
        catch {
            /* fall through */
        }
 ...
 */
export function evaluateUiPrototype(proposalDir) {
    ...
    if (mode === 'generated') {
        ...
        const nonEmptyObject = obj !== null && typeof obj === 'object' && !Array.isArray(obj) && Object.keys(obj).length > 0;
        if (!nonEmptyObject) {
            return {
                code: 1,
 ...
 */
export function writeUiPrototypeHashes(proposalDir, outcome) {
    if (outcome.code !== 0)
        return;
    const hashes = outcome.data?.hashes;
    if (!hashes)
        return; // when 不满足（无对账数据）→ 与改造前一致：不落文件
    try {
...
export function checkUiPrototype(slug, format = 'text') {
    ...
    if (format === 'json') {
        if (outcome.code === 0) {
            ...
            console.error(JSON.stringify(makeErrorEnvelope(COMMAND, outcome.errorCode ?? 'ui_prototype_check_failed', outcome.reason)));
        }
    }
    else {
        if (outcome.code === 0) {
...
//# sourceMappingURL=check-ui-prototype.js.map