...
import { parseAiTool, detectProjectName, createLogosConfig, createAdoptLogosProject, chooseAiTool, DIRECTORIES, ensureVerifyPreRunConfig, printVerifyPreRunBackfillResult, deployAiToolAssets, deploySpecs, expandAiTools, preflightAiToolAssets, preflightInstructionFiles, writeInstructionFiles, } from './init.js';
function isTTY() {
...
function detectSourceLabel(root) {
...
export async function adopt(name, options) {
    ...
    try {
        ...
        if (deployTools.includes('zcode') || deployTools.includes('qoder') || deployTools.includes('workbuddy'))
            preflightInstructionFiles(root, locale, aiTool, true);
    }
    catch (error) {
    ...
    const config = JSON.parse(createLogosConfig(projectName, locale, aiTool));
