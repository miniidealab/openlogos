 ...
 */
export function allocateDecisionRecordIds(landedDxx, configuredNextId, candidates) {
 ...
 */
function extractDeltaBody(deltaContent) {
 ...
 */
function persistDecisionCounter(root, nextId) {
    ...
    if (dcIdx >= 0) {
        ...
        for (let i = dcIdx + 1; i < lines.length; i++) {
            if (/^\S/.test(lines[i]))
                break; // 离开该块
            if (/^\s+next_id:/.test(lines[i])) {
                lines[i] = lines[i].replace(/next_id:\s*\d+/, `next_id: ${nextId}`);
                updated = true;
                break;
            }
        }
        if (!updated)
            lines.splice(dcIdx + 1, 0, `  next_id: ${nextId}`); // 块存在但缺字段 → 插入
...
function readDecisionJournal(proposalDir) {
...
function journalMatchesDeltas(plan, parsed) {
    if (plan.targets.length !== parsed.length)
        return false;
 ...
 */
export function applyDecisionRecords(root, proposalDir, locale = 'zh') {
    ...
    if (!existsSync(deltaDir))
        return { ok: true, applied: [] }; // 无决策 delta → 零改动
    const deltaFiles = readdirSync(deltaDir).filter(f => f.endsWith('.md')).sort();
    if (deltaFiles.length === 0)
        return { ok: true, applied: [] };
    if (existsSync(specMerged))
        return { ok: true, applied: [] }; // 已提交 → 幂等 no-op
    ...
    for (const f of deltaFiles) {
        ...
        const op = explicitOp === 'MODIFIED' || (explicitOp === null && targetExists) ? 'MODIFIED' : 'ADDED';
    ...
    if (resuming) {
        ...
        const landed = (existsSync(decisionsDir) ? readdirSync(decisionsDir) : [])
            .map(parseDecisionFilenameDxx).filter((n) => n !== null);
        const configuredNextId = readProjectYaml(root).data?.decision_counter?.next_id;
        const alloc = allocateDecisionRecordIds(landed, configuredNextId, addedItems.map(p => ({ filenameDxx: p.filenameDxx, titleDxx: p.filenameDxx })));
        if (!alloc.ok)
            return { ok: false, applied: [], base: alloc.base, error: alloc.error }; // 零写入
        plan = {
            base: alloc.base,
        ...
        for (const b of [...backups].reverse()) {
            try {
                ...
                if (b.existed) {
                    ...
                    if (!now || readFileSync(b.path, 'utf-8') !== b.prev)
                        writeFileSync(b.path, b.prev);
                }
                else if (now) {
                    rmSync(b.path);
                }
    ...
    try {
        ...
        for (const t of plan.targets)
            backup(join(decisionsDir, t.file));
        backup(yamlPath);
        if (!resuming)
            writeFileSync(journalPath, JSON.stringify(plan, null, 2)); // 首个资源写入前原子落 journal
        for (const t of plan.targets)
            writeFileSync(join(decisionsDir, t.file), t.body); // 主文档（ADDED 新建 / MODIFIED 覆盖整条全量）
        syncResourceIndex(root, locale); // 权威 index 路径
        if (plan.addedCount > 0)
            persistDecisionCounter(root, plan.persistedNextId); // 仅 ADDED 批推进 counter
        ...
        if (existsSync(journalPath))
            rmSync(journalPath); // 提交成功 → 清 journal
    }
    catch (e) {
        rollback();
        // 全新事务：回滚已复原到事务前 → 删 journal；恢复事务：保留 journal 供下次重试（前滚点不丢）。
        if (!resuming && existsSync(journalPath)) {
            try {
                rmSync(journalPath);
            }
            catch { /* 尽力 */ }
        }
        return { ok: false, applied: [], base: plan.base, error: `apply 事务失败已回滚：${e.message}` };
    }
    return {
        ok: true,
