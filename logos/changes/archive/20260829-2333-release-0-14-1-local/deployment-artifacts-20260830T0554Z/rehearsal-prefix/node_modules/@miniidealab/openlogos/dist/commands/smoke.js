...
import { normalizeSandboxConfig, runSandboxedCommand, buildInitialSandboxData } from '../lib/sandbox.js';
import { parseJsonl } from './verify.js';
import { checkSmokeCoverage, resolveSmokeCommand, extractSmokeIdsFromContent, } from '../lib/smoke-coverage.js';
...
export function extractSmokeDefinedIds(root) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            for (const id of extractSmokeIdsFromContent(content))
                ids.add(id);
        }
    }
...
function generateSmokeReport(data) {
    ...
    if (data.uncovered_cases.length > 0) {
        md += '## Uncovered Cases\n\n';
        for (const id of data.uncovered_cases)
            md += `- ${id}\n`;
        md += '\n';
    }
    ...
    if (data.skipped_cases.length > 0) {
        md += '## Skipped Cases\n\n';
        for (const id of data.skipped_cases)
            md += `- ${id}\n`;
        md += '\n';
    }
    return md;
}
export function collectSmokeData(root, environment, sandbox, coverageCheck) {
    const configPath = join(root, 'logos', 'logos.config.json');
    let resultPath = DEFAULT_SMOKE_RESULT_PATH;
    let reportPath = DEFAULT_SMOKE_REPORT_PATH;
    try {
        const config = JSON.parse(readFileSync(configPath, 'utf-8'));
        if (config.smoke?.result_path)
            resultPath = config.smoke.result_path;
        if (config.smoke?.report_path)
            reportPath = config.smoke.report_path;
    }
    catch { /* use defaults */ }
    ...
    if (!isPass) {
        if (failed.length > 0)
            reason = 'failed_cases';
        else if (check.diagnostics.length > 0)
            reason = check.diagnostics[0].code;
        else
...
export function smoke(format = 'text', environment) {
    const root = process.cwd();
    const configPath = join(root, 'logos', 'logos.config.json');
    if (!existsSync(configPath)) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('smoke', 'PROJECT_NOT_INITIALIZED', 'logos/logos.config.json not found.')));
            process.exit(1);
        }
    ...
    if (!existsSync(join(root, resultPath))) {
        ...
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('smoke', 'NO_SMOKE_RESULTS', `No smoke results found at ${resultPath}.`)));
            process.exit(1);
        }
        console.error(`\nError: No smoke results found at ${resultPath}.`);
        process.exit(1);
    ...
    if (defined.length === 0) {
        if (format === 'json') {
            console.error(JSON.stringify(makeErrorEnvelope('smoke', 'NO_SMOKE_CASES', 'No smoke case specs found in logos/resources/test/smoke/.')));
            process.exit(1);
        }
