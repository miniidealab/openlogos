...
function readYamlFile(path, what) {
    let raw;
    try {
    ...
    try {
        ...
        throw new FlowError('FLOW_SCHEMA_INVALID', `${what} YAML 解析失败：${e.message}`);
    }
...
function validateDispatchDecl(d, where, what) {
    if (d == null)
        return;
    ...
    if (dd.artifacts_hint != null
        && (!Array.isArray(dd.artifacts_hint) || dd.artifacts_hint.some(x => typeof x !== 'string'))) {
        throw new FlowError('FLOW_SCHEMA_INVALID', `${what} ${where} 的 dispatch.artifacts_hint 须为字符串数组`);
    }
}
/** requires_reviewed 类型校验。 */
function validateRequiresReviewed(v, where, what) {
    if (v == null)
        return;
...
function validateDefaults(v, what) {
    if (v == null)
        return;
...
export function validateFlow(flow, what = 'flow') {
    const f = flow;
    ...
    for (const [si, sub] of f.subflows.entries()) {
        const s = sub;
        ...
        for (const [ni, node] of s.nodes.entries()) {
            ...
            if (n.coverage_threshold != null) {
                if (typeof n.coverage_threshold !== 'number' || !(n.coverage_threshold > 0 && n.coverage_threshold <= 1)) {
                    throw new FlowError('FLOW_SCHEMA_INVALID', `${what} node \`${n.id}\` 的 coverage_threshold 须为 0 < x <= 1 的 number`);
                }
                ...
                if (n.done_when !== 'all_present' || !n.for_each
                    || typeof n.produces !== 'string' || n.produces.trim() === '') {
                    throw new FlowError('FLOW_SCHEMA_INVALID', `${what} node \`${n.id}\` 的 coverage_threshold 仅 \`done_when: all_present\` + \`for_each\` + 非空 \`produces\` 的 fan-out 节点可设`);
                }
            }
        }
    }
...
export function loadBuiltinFlow(lifecycle) {
    ...
    if (!available.includes(lifecycle)) {
        throw new FlowError('FLOW_NOT_FOUND', `内置模板 spec/flow/${lifecycle}.yaml 不存在（可用：${available.join(', ') || '无'}）`);
    }
...
export function inferLifecycle(root) {
...
export function readOverlay(root, lifecycle) {
...
export function parseExtends(value) {
...
function findNode(flow, id) {
    for (const sub of flow.subflows) {
        ...
        if (index !== -1)
            return { subflow: sub, index };
    }
    return null;
}
/** 把 overlay 应用到内置 flow（深拷贝后修改），返回 resolved flow + 告警。 */
export function applyOverlay(builtin, overlay, lifecycle) {
    const flow = JSON.parse(JSON.stringify(builtin));
    const warnings = [];
    if (overlay.extends != null) {
        ...
        if (baseline !== lifecycle) {
            throw new FlowError('FLOW_SCHEMA_INVALID', `overlay \`extends\` 基线 \`${baseline}\` 与当前 lifecycle \`${lifecycle}\` 不一致`);
        }
    ...
    for (const [oi, opRaw] of ops.entries()) {
        ...
        if (kind === 'skip') {
            ...
            if (!hit)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] skip 的 target \`${op.target}\` 不存在`);
            ...
            const hit = findNode(flow, String(op.target));
            if (!hit)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] modify 的 target \`${op.target}\` 不存在`);
            ...
            if ('dispatch' in setObj) {
                ...
                setObj.dispatch = { ...hit.subflow.nodes[hit.index].dispatch, ...setObj.dispatch };
            }
            ...
            validateDispatchDecl(node.dispatch, `overlay[${oi}] add node \`${node.id}\``, 'overlay');
            validateRequiresReviewed(node.requires_reviewed, `overlay[${oi}] add node \`${node.id}\``, 'overlay');
            const anchorId = String(op.after ?? op.before ?? '');
            const hit = findNode(flow, anchorId);
            if (!hit)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] add 的锚点 \`${anchorId}\` 不存在`);
            ...
            const hit = findNode(flow, String(op.target));
            if (!hit)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] reorder 的 target \`${op.target}\` 不存在`);
            const [moved] = hit.subflow.nodes.splice(hit.index, 1);
            moved.overlay_op = 'reorder';
            const anchorId = String(op.after ?? op.before ?? '');
            const anchor = findNode(flow, anchorId);
            if (!anchor)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] reorder 的锚点 \`${anchorId}\` 不存在`);
            ...
            if (!sub)
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] set-loop 的 subflow \`${subId}\` 不存在`);
            if (!op.set || typeof op.set !== 'object' || Array.isArray(op.set)) {
            ...
            const setMaxIters = op.set.max_iters;
            if (subId !== 'implement' && typeof setMaxIters === 'number' && setMaxIters > 1) {
                throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] set-loop 的真迭代（max_iters>1）本切片仅支持 \`implement\` 子流程，不支持 \`${subId}\``);
            }
            sub.loop = { ...(sub.loop ?? {}), ...op.set };
        }
        else {
    ...
    if (implementSub && typeof implementSub.loop?.max_iters === 'number' && implementSub.loop.max_iters > 1) {
        ...
        if (verifyNode && (isCmdPred(verifyNode.done_when) || isCmdPred(verifyNode.fail_when))) {
            throw new FlowError('FLOW_SCHEMA_INVALID', '激活 loop（implement set-loop max_iters>1）与 verify 的 cmd gate 互斥：verify 的 done_when/fail_when 任一为 cmd: 时不得同时激活 loop（loop 收敛靠 LOOP_ITERS 账本，cmd gate 不写账本）');
        }
    }
 ...
 */
export function readProjectCmdTimeout(root) {
    let cfg;
    try {
    ...
    if (typeof t !== 'number' || !Number.isInteger(t) || t < 1) {
        throw new FlowError('FLOW_SCHEMA_INVALID', `项目级 flow.cmd_timeout_seconds 须为整数 ≥ 1（当前 \`${String(t)}\`）`);
    }
    return t;
}
/** S30：launched gate 可被 overlay-modify 改 cmd: 的精确 (节点,字段) 白名单。 */
const S30_CMD_GATE_FIELDS = {
 ...
 */
function validateModifyCmdGate(node, oi, lifecycle) {
    ...
    if (lifecycle !== 'launched') {
        throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] modify：cmd: 谓词仅 launched 的 verify/deploy/smoke gate 可经 modify 设置；当前 lifecycle=\`${lifecycle}\`（节点 \`${node.id}\`）`);
    }
    const allowed = S30_CMD_GATE_FIELDS[node.id];
    if (!allowed) {
        throw new FlowError('FLOW_SCHEMA_INVALID', `overlay[${oi}] modify：cmd: 谓词仅允许改到 launched 的 verify/deploy/smoke gate，不支持 builtin 节点 \`${node.id}\``);
    }
    ...
    // 空命令非法（trim 后空）
    for (const f of ['done_when', 'fail_when']) {
        const v = node[f];
        if (isCmdPred(v) && v.slice('cmd:'.length).trim() === '') {
 ...
 */
export function fanoutDone(covered, total, threshold) {
    if (total <= 0)
        return false;
    if (threshold != null)
 ...
 */
export function findActivatedLoop(flow) {
    for (const sub of flow.subflows) {
        if (sub.id !== 'implement')
            continue; // 本切片真迭代仅 implement(code/verify)
 ...
 */
export function materializeResolvedDispatch(flow) {
 ...
 * lifecycle 缺省按项目状态推断。
 */
export function loadFlow(root, opts = {}) {
