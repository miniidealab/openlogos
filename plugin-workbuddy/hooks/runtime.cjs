...
const READ_TOOLS = new Set(['Read', 'Glob', 'Grep', 'WebFetch', 'WebSearch', 'Task', 'TodoRead']);
...
function normalizeHookEvent(input) {
  ...
  const event = {
    ...
    transcriptPath: alias(input, 'transcriptPath', 'transcript_path'),
    ...
    toolUseId: alias(input, 'toolUseId', 'tool_use_id'),
    source: input.source,
    cwd: input.cwd,
  };
...
function resolveCandidate(root, rawPath) {
  ...
  if (path.isAbsolute(rawPath) || rawPath.replace(/\\/g, '/').split('/').includes('..')) {
    throw new HookInputError('写入目标必须是项目根相对路径且不得包含 ..');
  }
...
function targetFromEvent(event) {
  ...
  if (event.toolName === 'Bash' || event.toolName === 'execute_command') {
    ...
    if (typeof command !== 'string' || /[;&|><`$()\n\r]/.test(command) || !SAFE_BASH.test(command.trim())) return null;
...
function decideGuard(root, event) {
  if (READ_TOOLS.has(event.toolName)) return allow('已知只读工具');
  if (!WRITE_TOOLS.has(event.toolName)) return deny('未知或间接写工具无法证明只读，按 fail-closed 阻断');
  if (!isLaunched(root)) return allow('initial 生命周期不启用变更硬门禁');
  const target = targetFromEvent(event);
  if (target && target.safeBash) return allow('只读命令');
  if (event.toolName === 'Bash' || event.toolName === 'execute_command') return deny('无法可靠证明命令只读，按 fail-closed 阻断');
...
function run(mode, raw, cwd) {
  ...
  const expectedEvent = mode === 'session' ? 'SessionStart' : 'PreToolUse';
  if (event.hookEventName !== expectedEvent) throw new HookInputError(`hook_event_name 必须为 ${expectedEvent}`);
  if (mode === 'guard' && (!event.toolName || !event.toolInput)) {
    throw new HookInputError('PreToolUse 缺少 tool_name/tool_input');
  }
...
async function main() {
  ...
  try {
    ...
    process.stderr.write(`OpenLogos WorkBuddy Hook: ${message}\n`);
