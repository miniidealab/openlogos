...
>
> **⚠️ 口径更新（2026-06-21）**：本文 §8.1 等处标注为"M2 预留 / M1 不实现"的若干字段
> ——`cmd:` 谓词、loop 真迭代（测试绿收敛）、**fan-out 聚合阈值**、**loop 内"每实例迭代 vs 整组收敛"**——
> 已由后续切片处理：**S26（`cmd:` 谓词）、S27（loop 真迭代）、S28（`next_node`）已实现**；
> **S29（fan-out 阈值 `coverage_threshold`、loop 内整组收敛）已由 `flow-gate-fanout-loop` 落地并归档**；M2 最后一项 `modify-cmd-on-builtin` 由 **S30（提案 `flow-cmd-builtin`，进行中）** 收尾——把 `cmd:` 放开到 launched verify/deploy/smoke gate。
> **当前权威口径以 `spec/flow-spec.md §13` 边界总表为准**；本文相关"预留"措辞为历史结论，保留以备追溯。
...
- 聚合阈值、loop 内"每实例迭代 vs 整组收敛" 同为预留字段，M1 不实现。
  - **【S29 已落地并归档，2026-06-21】**：聚合阈值由变更提案 `flow-gate-fanout-loop` 实现为 fan-out 节点可选字段 `coverage_threshold`（`0<x<=1`，`covered/total>=阈值` 即 done，缺省等价 `all_present`）；
    loop 内收敛**定死为整组收敛**（收敛裁判=测试绿，fan-out 节点各自按 `all_present`/`coverage_threshold` 完成，不做 per-instance 迭代）。规格已定，代码待落地；详见 `spec/flow-spec.md §6/§7/§13`。
