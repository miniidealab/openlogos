...
function proposalMd(o: ProposalOpts = {}): string {
  ...
  return withCompleteClarification(lines.join('\n'));
}
...
describe('S35 — 同源锚与契约', () => {
  ...
  it('UT-S35-21 / UT-S35-37: 违规集契约——四字段必填、52 码闭合（含 Plan Package 与 Authority Closure）、排序稳定', () => {
    const { root, dir } = setup({
    ...
    expect(registry.size).toBe(52); // 原 36 码 + Plan Package L0 的 11 码 + Authority Closure 的 5 码
