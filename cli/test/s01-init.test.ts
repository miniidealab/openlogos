...
describe('S01 Unit Tests — aiTool config merge', () => {
  ...
  it('UT-S01-44: mergeAiToolConfig expands all to every deployable target', () => {
    expect(mergeAiToolConfig('cursor', 'all')).toEqual(['claude-code', 'opencode', 'codex', 'cursor', 'zcode', 'qoder', 'workbuddy']);
  });
});
...
describe('S01 Unit Tests — packaging and usage text', () => {
  ...
  it('UT-S01-12: help text and non-interactive usage include codex', async () => {
    ...
    expect(indexText).toContain('--ai-tool <claude-code|opencode|codex|cursor|zcode|qoder|workbuddy|other|all>');
    expect(indexText).toContain('--aitool <claude-code|opencode|codex|cursor|zcode|qoder|workbuddy|other|all>');
    expect(initText).toContain('--ai-tool <claude-code|opencode|codex|cursor|zcode|qoder|workbuddy|other|all>');
  });
});
