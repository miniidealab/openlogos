...
import { CONTRACT_VERSION, CONTRACT_VERSION_WITH_CLARIFICATION, CONTRACT_VERSION_WITH_FEATURES, CONTRACT_VERSION_WITH_PLAN_PACKAGE, CONTRACT_VERSION_WITH_MERGE_TRANSACTION } from '../src/lib/step-registry.js';
...
describe('S34 — status/next 集成（条件版本 + 分组）', () => {
  ...
  it('UT-S34-10: 条件版本发射（含 features → 1.1.0，无 features → 1.0.0）+ schema superset 支持两版', () => {
    ...
    // 打包 schema 为 superset：x-contract-version 跟随当前最高公共契约，version enum 含全部兼容版。
