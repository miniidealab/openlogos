...
describe('S37 — 契约、同源与零漂移', () => {
  ...
  it('UT-S37-31: 零回归——枚举 42 码闭合；合法 delta 零 L8 违规、既有判据不受影响', () => {
    // 37 → 42：§2.79 的 L9 接入 5 条 slice/marker 类阻塞理由（码即 ProposalBlockReason）。
    expect(CHANGE_LINT_VIOLATION_CODES).toHaveLength(42);
    expect(new Set(CHANGE_LINT_VIOLATION_CODES).size).toBe(42);
