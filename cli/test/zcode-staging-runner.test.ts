...
const runner = join(repoRoot, 'scripts', 'smoke-zcode-staging.js');

describe('ZCode staging smoke runner 合同', () => {
  it('声明完整 SMOKE 归属、真实环境前置且没有公开发布命令', () => {
    ...
    expect(contract.ids).toEqual(Array.from({ length: 8 }, (_, index) => `SMOKE-core-${100 + index}`));
    expect(contract.required_env).toContain('OPENLOGOS_ZCODE_BIN');
