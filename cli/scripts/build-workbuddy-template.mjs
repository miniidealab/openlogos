...
const target = join(cliRoot, 'workbuddy-plugin-template');
const source = join(repoRoot, 'plugin-workbuddy');
...
for (const name of readdirSync(join(target, 'commands'))) {
  ...
  let content = readFileSync(command, 'utf8').replaceAll('--ai-tool claude-code', '--ai-tool workbuddy');
  if (name === 'next.md') {
  ...
  if (name === 'status.md') content = content.replaceAll('${CLAUDE_PLUGIN_ROOT}', '${CODEBUDDY_PLUGIN_ROOT}');
...
const manifestPath = join(target, '.workbuddy-plugin', 'plugin.json');
const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
manifest.version = pkg.version;
writeFileSync(manifestPath, JSON.stringify(manifest, null, 2) + '\n');

mkdirSync(join(target, 'agents'), { recursive: true });
console.error(`WorkBuddy template prepared: ${target}`);
