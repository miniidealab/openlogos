...
export type ClarificationReason =
  | 'high-impact-user-decision-required'
  | 'data-clarification-required'
  | 'compatibility-clarification-required'
  | 'security-privacy-clarification-required'
  | 'deployment-clarification-required'
  | 'release-clarification-required'
  | 'external-commitment-clarification-required'
  | 'clarification-contract-invalid'
  | 'clarification-upgrade-required'
  | 'legacy-clarification-backfill-required';
...
interface ParsedDecision {
  id: string;
  category: ClarificationCategory;
  source: 'user' | 'policy' | 'repository_fact';
}

interface ParsedUnresolved extends ClarificationNextDecision {
  depends_on: string[];
}

const CXX_RE = /^C(?:0[1-9]|[1-9]\d+)$/;
const MODE_SET = new Set<string>(['adaptive', 'deep', 'provided']);
const CATEGORY_SET = new Set<string>(CLARIFICATION_CATEGORIES);
const IMPACT_TO_CATEGORY: Record<string, ClarificationCategory> = {
  data: 'data',
...
const REASON_BY_CATEGORY: Partial<Record<ClarificationCategory, ClarificationReason>> = {
...
function invalidOutput(
  reason: Extract<ClarificationReason, 'clarification-contract-invalid' | 'clarification-upgrade-required' | 'legacy-clarification-backfill-required'>,
  schema: string | null = null,
  mode: ClarificationMode | null = null,
): ClarificationOutput {
  return {
    schema,
    mode,
...
function extractClarificationYaml(content: string): {
  status: 'missing' | 'duplicate-heading' | 'duplicate-yaml' | 'invalid' | 'found';
  yaml?: string;
} {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && (m[1].trim() === '决策澄清' || m[1].trim() === 'Decision Clarification')) headings.push(i);
  }
  if (headings.length === 0) return { status: 'missing' };
  if (headings.length > 1) return { status: 'duplicate-heading' };

  const start = headings[0] + 1;
  let end = lines.length;
  for (let i = start; i < lines.length; i++) {
    if (!scan.masked[i] && /^ {0,3}#{1,2}(?:[ \t]+|$)/.test(scan.text[i])) { end = i; break; }
  }
  const section = lines.slice(start, end).join('\n');
  const fenced = [...section.matchAll(/(?:^|\n) {0,3}```ya?ml[ \t]*\r?\n([\s\S]*?)\r?\n {0,3}```(?=\n|$)/gi)];
  if (fenced.length === 0) return { status: 'invalid' };
  if (fenced.length > 1) return { status: 'duplicate-yaml' };
  return { status: 'found', yaml: fenced[0][1] };
}

function isUnscannedImpactReason(value: unknown): boolean {
...
function parseDecision(value: unknown, issues: string[], index: number): ParsedDecision | null {
  if (!isRecord(value)) { issues.push(`decisions[${index}] 必须是对象`); return null; }
  const id = value.id;
  const category = value.category;
  const source = value.source;
  if (!nonEmpty(id) || !CXX_RE.test(id)) issues.push(`decisions[${index}].id 非法`);
  if (!nonEmpty(category) || !CATEGORY_SET.has(category)) issues.push(`decisions[${index}].category 非法`);
  if (source !== 'user' && source !== 'policy' && source !== 'repository_fact') issues.push(`decisions[${index}].source 非法`);
  for (const key of ['question', 'answer', 'rationale'] as const) {
    if (!nonEmpty(value[key])) issues.push(`decisions[${index}].${key} 不能为空`);
  }
  for (const key of ['affects', 'rejected_options'] as const) {
    if (!Array.isArray(value[key]) || !(value[key] as unknown[]).every(nonEmpty)) issues.push(`decisions[${index}].${key} 必须是字符串数组`);
  }
  if (!nonEmpty(id) || !CXX_RE.test(id) || !nonEmpty(category) || !CATEGORY_SET.has(category)
    || (source !== 'user' && source !== 'policy' && source !== 'repository_fact')) return null;
  return { id, category: category as ClarificationCategory, source };
}

function parseUnresolved(value: unknown, issues: string[], index: number): ParsedUnresolved | null {
  if (!isRecord(value)) { issues.push(`unresolved[${index}] 必须是对象`); return null; }
  const id = value.id;
  const category = value.category;
  if (!nonEmpty(id) || !CXX_RE.test(id)) issues.push(`unresolved[${index}].id 非法`);
  if (!nonEmpty(category) || !CATEGORY_SET.has(category)) issues.push(`unresolved[${index}].category 非法`);
  for (const key of ['question', 'impact', 'recommendation', 'recommendation_reason'] as const) {
    if (!nonEmpty(value[key])) issues.push(`unresolved[${index}].${key} 不能为空`);
  }
  ...
  if (!nonEmpty(id) || !CXX_RE.test(id) || !nonEmpty(category) || !CATEGORY_SET.has(category)
    ...
    || !options.every(option => isRecord(option) && nonEmpty(option.id) && nonEmpty(option.label) && nonEmpty(option.tradeoff))) return null;
  return {
    id,
    category: category as ClarificationCategory,
    ...
    options: options.map(option => ({
      ...
      tradeoff: (option as Record<string, unknown>).tradeoff as string,
    })),
  };
}

function hasCycle(nodes: ParsedUnresolved[]): boolean {
...
function stableUnresolvedOrder(nodes: ParsedUnresolved[], decidedIds: Set<string>): string[] {
...
export function validateClarificationOutput(value: ClarificationOutput): boolean {
  ...
  if (new Set(value.required_categories).size !== value.required_categories.length
    || sorted.some((item, index) => item !== value.required_categories[index])) return false;
  if (value.next_decision !== null && value.next_decision_id !== value.next_decision.id) return false;
...
export function evaluateProposalClarification(content: string, deploymentRequired: boolean | null): ClarificationEvaluation {
  ...
  let raw: unknown;
  try { raw = parseYaml(extracted.yaml); } catch { raw = null; }
  if (!isRecord(raw)) {
  ...
  const issues: string[] = [];
  const mode = nonEmpty(raw.mode) && MODE_SET.has(raw.mode) ? raw.mode as ClarificationMode : null;
  if (mode === null) issues.push('clarification.mode 非法');
  if (raw.status !== 'pending' && raw.status !== 'complete' && raw.status !== 'invalid') issues.push('clarification.status 非法');
  if (!isRecord(raw.impacts)) issues.push('clarification.impacts 必须是对象');
  ...
  const requiredFromImpacts: ClarificationCategory[] = [];
  for (const key of IMPACT_KEYS) {
    ...
    if (item.status === 'required') requiredFromImpacts.push(IMPACT_TO_CATEGORY[key]);
  }

  if (!Array.isArray(raw.decisions)) issues.push('clarification.decisions 必须是数组');
  if (!Array.isArray(raw.unresolved)) issues.push('clarification.unresolved 必须是数组');
  if (!Array.isArray(raw.defaults)) issues.push('clarification.defaults 必须是数组');
  const decisions = Array.isArray(raw.decisions)
    ? raw.decisions.map((item, index) => parseDecision(item, issues, index)).filter((v): v is ParsedDecision => v !== null)
    : [];
  const unresolved = Array.isArray(raw.unresolved)
    ? raw.unresolved.map((item, index) => parseUnresolved(item, issues, index)).filter((v): v is ParsedUnresolved => v !== null)
    : [];

  const allIds = [...decisions.map(item => item.id), ...unresolved.map(item => item.id)];
  if (new Set(allIds).size !== allIds.length) issues.push('decisions + unresolved 的 CXX 必须全局唯一');
  const allIdSet = new Set(allIds);
  for (const item of unresolved) {
    for (const dep of item.depends_on) {
      if (!allIdSet.has(dep)) issues.push(`${item.id}.depends_on 引用不存在的 ${dep}`);
      if (dep === item.id) issues.push(`${item.id} 不得自依赖`);
    }
  }
  if (hasCycle(unresolved)) issues.push('unresolved.depends_on 存在循环依赖');
  ...
  for (const item of unresolved) {
    if (item.depends_on.some(dep => !available.has(dep))) issues.push(`${item.id} 未按稳定拓扑序持久化`);
    available.add(item.id);
  }

  const satisfied = new Set(decisions.filter(item => item.source === 'user').map(item => item.category));
  const triggered = new Set<ClarificationCategory>(requiredFromImpacts);
  if (deploymentRequired === true) triggered.add('deployment');
  for (const item of unresolved) triggered.add(item.category);
  ...
  for (const category of requiredCategories) {
    ...
    if (count !== 1) issues.push(`未满足类别 ${category} 必须恰有一个对应 unresolved，实际 ${count}`);
  }

  if (raw.status === 'complete' && unresolved.length > 0) issues.push('complete 与非空 unresolved 冲突');
  if (raw.status === 'complete' && requiredCategories.length > 0) issues.push('complete 与未满足必选类别冲突');
  if (raw.status === 'pending' && unresolved.length === 0) issues.push('pending 必须携带可回答的 unresolved');
  if (raw.status === 'invalid') issues.push('proposal 不应主动持久化 status=invalid');
