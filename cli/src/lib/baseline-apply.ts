...
export interface BaselineClosureApplySuccess {
  ok: true;
  applied: Array<{ target_path: string; mode: 'CREATE' | 'MODIFY'; sha256: string }>;
}
...
interface ApplyJournal {
  schema: 'openlogos/baseline-closure-apply@1';
  phase: 'prepared' | 'committing' | 'committed' | 'rolling_back';
...
function canonicalRootTarget(root: string, raw: string): { path: string; abs: string } | null {
  ...
  try {
    ...
    while (contained(component, root) && component !== root) {
      if (existsSync(component) && lstatSync(component).isSymbolicLink()) return null;
      component = dirname(component);
    }
    if (!contained(realpathSync(cursor), rootReal)) return null;
  } catch { return null; }
  return { path: normalized, abs };
}

function proposalIsContained(root: string, proposalDir: string): boolean {
...
function journalPath(proposalDir: string): string {
  return join(proposalDir, BASELINE_CLOSURE_APPLY_JOURNAL);
}

function txnDir(proposalDir: string): string {
  return join(proposalDir, APPLY_TXN_DIR);
}

function writeJournal(proposalDir: string, journal: ApplyJournal): void {
...
function existingHash(path: string): string | null {
  ...
  if (!st.isFile() || st.isSymbolicLink()) throw new Error(`目标不是普通文件：${path}`);
  return hashFile(path);
}

function missingParentDirs(root: string, targetAbs: string): string[] {
  const dirs: string[] = [];
...
function prepareInputs(
...
): { writes?: PreparedWrite[]; error?: string } {
  ...
  for (const input of inputs) {
    ...
    if (input.kind === 'non-markdown') {
      ...
      if (newline < 0) return { error: `${input.deltaPath} 缺首行行结束符` };
      const decoded = raw.toString('utf-8');
      if (!Buffer.from(decoded, 'utf-8').equals(raw)) return { error: `${input.deltaPath} 不是合法 UTF-8` };
      const checked = validateAndStripNonMarkdownDelta(decoded, input.mode, targetPath, { root });
      if (!checked.ok || checked.payload === undefined) return { error: `${input.deltaPath}：${checked.message}` };
      // validator 只删除首行及唯一换行；直接切原 Buffer，保证 payload 字节零格式化。
      payload = raw.subarray(newline + 1);
      if (checked.payload !== payload.toString('utf-8')) return { error: `${input.deltaPath} payload 剥离结果不确定` };
    } else {
      const resolved = canonicalRootTarget(root, input.targetPath);
      if (!resolved) return { error: `prepared target 非法或越界：${input.targetPath}` };
      targetPath = resolved.path;
      targetAbs = resolved.abs;
      payload = Buffer.isBuffer(input.bytes) ? Buffer.from(input.bytes) : Buffer.from(input.bytes);
      if (payload.length === 0) return { error: `prepared target payload 为空：${targetPath}` };
    }
    const key = process.platform === 'win32' ? targetPath.toLocaleLowerCase('en-US') : targetPath;
    if (seen.has(key)) return { error: `canonical target 重复：${targetPath}` };
    seen.add(key);
    let oldSha256: string | null;
    try { oldSha256 = existingHash(targetAbs); } catch (e) { return { error: String(e) }; }
    if (input.mode === 'CREATE' && oldSha256 !== null) return { error: `CREATE 目标已存在：${targetPath}` };
    if (input.mode === 'MODIFY' && oldSha256 === null) return { error: `MODIFY 目标不存在：${targetPath}` };
    writes.push({
...
function validateJournalPaths(root: string, proposalDir: string, journal: ApplyJournal): string | null {
  const td = txnDir(proposalDir);
  const seen = new Set<string>();
  for (const e of journal.entries) {
    const target = canonicalRootTarget(root, e.target_path);
    if (!target || seen.has(target.path)) return `journal target 非法/重复：${e.target_path}`;
    seen.add(target.path);
    if (!/^[0-9a-f]{64}$/.test(e.new_sha256)
      || (e.old_sha256 !== null && !/^[0-9a-f]{64}$/.test(e.old_sha256))) return `journal hash 非法：${e.target_path}`;
    for (const privatePath of [e.staged_path, ...(e.backup_path ? [e.backup_path] : [])]) {
      const abs = join(proposalDir, ...privatePath.split('/'));
      if (!contained(abs, td)) return `journal 私有路径越界：${privatePath}`;
    }
  }
  for (const dir of journal.created_dirs) if (!canonicalRootTarget(root, dir)) return `journal created_dir 越界：${dir}`;
  return null;
}

function restoreBackup(targetAbs: string, backupAbs: string): void {
 ...
 */
export function recoverBaselineClosureApply(
  root: string,
  proposalDir: string,
): { ok: true; recovered: 'none' | 'rolled_back' | 'committed' } | { ok: false; error: string } {
  if (!proposalIsContained(root, proposalDir)) return { ok: false, error: 'proposalDir 越界或不可解析' };
  const jp = journalPath(proposalDir);
  if (!existsSync(jp)) {
  ...
  if (!journal) return { ok: false, error: 'baseline closure apply journal 损坏或 schema 非法' };
  const pathProblem = validateJournalPaths(root, proposalDir, journal);
  if (pathProblem) return { ok: false, error: pathProblem };

  const states: Array<'old' | 'new' | 'absent'> = [];
  for (const e of journal.entries) {
    const targetAbs = join(root, ...e.target_path.split('/'));
    let current: string | null;
    try { current = existingHash(targetAbs); } catch (error) { return { ok: false, error: String(error) }; }
    if (current === e.new_sha256) states.push('new');
    else if (current === e.old_sha256 && e.old_sha256 !== null) states.push('old');
    else if (current === null && e.old_sha256 === null) states.push('absent');
    else return { ok: false, error: `目标处于 journal 之外的未知字节态：${e.target_path}` };
  }

  if (journal.phase === 'committed') {
    if (states.some(s => s !== 'new')) return { ok: false, error: 'committed journal 的目标未全部处于新字节态' };
  ...
  for (let i = 0; i < journal.entries.length; i++) {
    ...
    if (states[i] !== 'new' || e.old_sha256 === null) continue;
    if (!e.backup_path) return { ok: false, error: `缺 backup：${e.target_path}` };
  ...
  try { writeJournal(proposalDir, journal); } catch (e) { return { ok: false, error: `无法持久化 rollback intent：${String(e)}` }; }
  try {
    for (let i = journal.entries.length - 1; i >= 0; i--) {
      const e = journal.entries[i];
      if (states[i] !== 'new') continue;
      const targetAbs = join(root, ...e.target_path.split('/'));
      if (e.old_sha256 === null) {
        if (existsSync(targetAbs)) unlinkSync(targetAbs);
      } else {
    ...
    for (const rel of [...journal.created_dirs].reverse()) {
      ...
      if (existsSync(abs)) { try { rmdirSync(abs); } catch { /* 非空/并发创建则保留安全目录 */ } }
    }
    removePrivateArtifacts(proposalDir);
 ...
 */
export function applyBaselineClosureBatch(
  root: string,
  proposalDir: string,
  inputs: BaselineClosureApplyInput[],
  hook: BaselineClosureApplyHook = {},
): BaselineClosureApplyResult {
  if (inputs.length === 0) return { ok: false, error: 'apply 批次为空', rolled_back: true };
  const recovered = recoverBaselineClosureApply(root, proposalDir);
  if (!recovered.ok) return { ok: false, error: recovered.error, rolled_back: false };
  const prepared = prepareInputs(root, proposalDir, inputs);
  if (!prepared.writes) return { ok: false, error: prepared.error ?? 'apply preflight 失败', rolled_back: true };
  ...
  try {
    ...
    for (let i = 0; i < prepared.writes.length; i++) {
      ...
      if (backupRel) {
        ...
        fsyncFile(backupAbs);
        if (hashFile(backupAbs) !== w.oldSha256) throw new Error(`backup 哈希漂移：${w.targetPath}`);
      }
      entries.push({
  ...
  try {
    ...
    writeJournal(proposalDir, journal);
    for (const rel of createdDirs) mkdirSync(join(root, ...rel.split('/')));
