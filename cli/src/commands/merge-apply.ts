...
import {
  ...
  type BaselineClosurePlan,
  type BaselineClosureTarget,
} from '../lib/baseline-closure.js';
import {
...
interface MetadataTarget {
  target_path: 'logos/logos-project.yaml';
  mode: 'MODIFY';
...
interface MergeApplyManifest {
  schema: typeof MANIFEST_SCHEMA;
  slug: string;
  prepared_targets: ManifestTarget[];
  metadata_targets: MetadataTarget[];
}
...
function cleanupGeneratedPrompt(proposalDir: string): void {
...
function fail(proposalDir: string | null, message: string): never {
  if (proposalDir) cleanupGeneratedPrompt(proposalDir);
...
function readStrictJson(path: string): unknown {
  ...
  if (doc.errors.length > 0) throw new Error(`manifest 重复 key/结构解析失败：${doc.errors.map(e => e.message).join('；')}`);
  return JSON.parse(source) as unknown;
}

function parseManifest(raw: unknown): MergeApplyManifest {
  ...
  const prepared: ManifestTarget[] = top.prepared_targets.map((item, index) => {
    ...
    return value as unknown as ManifestTarget;
  });
  const metadata: MetadataTarget[] = top.metadata_targets.map((item, index) => {
    const value = asRecord(item);
    ...
    return value as unknown as MetadataTarget;
  });
  return {
...
function markdownControlMarker(content: string): string | null {
...
function isNonMarkdownTarget(target: BaselineClosureTarget): boolean {
  const path = target.targetPath ?? '';
  return (target.category === 'api' && /\.(?:ya?ml|json)$/i.test(path))
    || (target.category === 'database' && /\.sql$/i.test(path));
}

function validateMetadata(bytes: Buffer, plan: BaselineClosurePlan): string | null {
  const text = bytes.toString('utf-8');
  if (!Buffer.from(text, 'utf-8').equals(bytes)) return 'logos-project.yaml 最终字节不是合法 UTF-8';
  const doc = parseDocument(text, { uniqueKeys: true, strict: true, prettyErrors: false });
  if (doc.errors.length > 0) return `logos-project.yaml 严格解析失败：${doc.errors.map(e => e.message).join('；')}`;
  let raw: unknown;
  try { raw = doc.toJS({ maxAliasCount: 100 }); } catch (e) { return `logos-project.yaml 转换失败：${String(e)}`; }
  const root = asRecord(raw);
  if (!root) return 'logos-project.yaml 根必须是对象';
  ...
  const indexed = new Set(resourceIndex.map(item => asRecord(item)?.path).filter((path): path is string => typeof path === 'string'));
  const missingIndex = created.map(target => target.targetPath!).filter(path => !indexed.has(path));
  if (missingIndex.length > 0) return `resource_index 未登记 CREATE 目标：${missingIndex.join('、')}`;
  ...
  if (scenarioCreates.length > 0) {
    ...
    const ids = new Set(scenarios.map(item => asRecord(item)?.id).filter((id): id is string => typeof id === 'string'));
    const required = [...new Set(scenarioCreates.flatMap(target => target.scenarioIds))];
    const missing = required.filter(id => !ids.has(id));
    if (missing.length > 0) return `scenarios[] 未登记 CREATE 场景：${missing.join('、')}`;
    ...
    if (!Number.isInteger(counter) || Number(counter) <= max) return `scenario_counter.next_id 必须大于最终最大场景号 S${max}`;
  }
  ...
  if (createdDecisionIds.length > 0) {
    ...
    if (!Number.isInteger(counter) || Number(counter) <= max) return `decision_counter.next_id 必须大于本批最大决策号 D${max}`;
  }
  return null;
}
...
function preparedInput(
  ...
  if (sha(deltaBytes) !== entry.source_sha256) throw new Error(`delta source_sha256 漂移：${entry.delta_path}`);
  const before = currentHash(join(root, ...entry.target_path.split('/')));
  if (before !== entry.before_sha256) throw new Error(`目标 before_sha256 漂移：${entry.target_path}`);
  ...
  if (!finalBytes || sha(finalBytes) !== entry.sha256) throw new Error(`最终字节 base64/sha256 不一致：${entry.target_path}`);
  if (entry.target_path.endsWith('.md')) {
    const text = finalBytes.toString('utf-8');
    if (!Buffer.from(text, 'utf-8').equals(finalBytes)) throw new Error(`Markdown 最终字节不是合法 UTF-8：${entry.target_path}`);
    const marker = markdownControlMarker(text);
    if (marker) throw new Error(`Markdown 控制 marker 泄漏到最终目标：${entry.target_path}（${marker}）`);
  }
...
export function mergeApply(slug: string | undefined, manifestArg: string | undefined): void {
  ...
  if (!existsSync(proposalDir)) fail(null, `提案不存在：${slug}`);
  if (!manifestArg) fail(proposalDir, '缺少 --manifest <path>');
  ...
  try {
    if (!contained(realpathSync(manifestPath), realpathSync(proposalDir))) fail(proposalDir, 'manifest 必须位于当前提案目录内');
  } catch { fail(proposalDir, 'manifest 不存在或无法安全解析'); }

  const lint = runChangeLint(root, proposalDir, slug);
  if (!lint.ok) fail(proposalDir, `change-lint 无法完成（${lint.errorCode}）：${lint.message}`);
  ...
  if (!parsed.plan || parsed.violations.length > 0) fail(proposalDir, 'on-touch-v1 计划无法严格解析');
  const plan = parsed.plan;

  let manifest: MergeApplyManifest;
  try { manifest = parseManifest(readStrictJson(manifestPath)); } catch (e) { fail(proposalDir, String(e)); }
  if (manifest.slug !== slug) fail(proposalDir, `manifest slug=${manifest.slug} 与 ${slug} 不一致`);

  const preparedByTarget = new Map<string, ManifestTarget>();
  for (const entry of manifest.prepared_targets) {
    if (preparedByTarget.has(entry.target_path)) fail(proposalDir, `manifest target 重复：${entry.target_path}`);
  ...
  try {
    for (const target of plan.targets.filter(item => item.targetPath !== null)) {
      if (isNonMarkdownTarget(target)) {
        if (preparedByTarget.has(target.targetPath!)) throw new Error(`API/DB 目标不得由 prepared bytes 绕过 validator：${target.targetPath}`);
        inputs.push({
          kind: 'non-markdown', deltaPath: target.deltaPath!, mode: target.mode as 'CREATE' | 'MODIFY',
    ...
    if (preparedByTarget.size > 0) throw new Error(`manifest 含未规划目标：${[...preparedByTarget.keys()].join('、')}`);
    ...
    if (created) {
      ...
      if (sha(current) !== metadata.before_sha256) throw new Error('logos-project.yaml before_sha256 漂移');
      const bytes = canonicalBase64(metadata.content_base64);
      if (!bytes || sha(bytes) !== metadata.sha256) throw new Error('logos-project.yaml base64/sha256 不一致');
