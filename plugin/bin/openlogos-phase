...
#
# 本函数是 per-scenario 覆盖判定的**从属实现**——权威口径是 `cli/src/commands/status.ts`
# （判据键 `<module>-<scenarioId>`）。两处必须在恒等域（目标目录内全部为 <module>-SXX-*
# 规范命名、单模块）上得出相同的 missing 集合；域外差异见场景 S11「已知差异清单」。
#
# 两条约束（fix-merge-prototype-commit-and-phase-module-prefix）：
#  ① 扩展名模式必须以**字面量**到达 find 的 -name。`for pat in $ext_pattern` 的未加引用展开
#     同时触发**文件名展开**：脚本在项目根执行时 `*.md` 会先变成 README.md / AGENTS.md /
#     CLAUDE.md（API 臂的 `*.yaml *.yml` 同理），拼出的是 `-name S76-AGENTS.md`，与场景文件
#     毫无关系。故分词期间关闭 pathname expansion（set -f），结束后按进入时的状态恢复——
#     探测结果不得随 cwd 下有哪些文件而变（INV-SC4）。
#  ② 匹配取两形态之并：`${sid}-${pat}` 命中无前缀历史命名，`*-${sid}-${pat}` 命中
#     `spec/module-naming-convention.md` 规定的 <module>-SXX-* 规范命名。一处修正同时
#     覆盖场景时序 / API / 测试用例三个调用点。放宽匹配不得漏报真实缺失。
check_scenarios_complete() {
  ...
  # 进入时 globbing 已关闭则保持关闭（不擅自改变调用方的 shell 选项）
  local glob_was_off=false
  case "$-" in *f*) glob_was_off=true ;; esac
  set -f
  ...
  for sid in $scenario_ids; do
    ...
    for pat in $ext_pattern; do
      if find "$dir" -maxdepth 1 \( -name "${sid}-${pat}" -o -name "*-${sid}-${pat}" \) 2>/dev/null | grep -q .; then
        found=true
        break
      fi
    done
...
STATUS_JSON=""
STATUS_LIFECYCLE=""
STATUS_ACTIVE_CHANGE=""
STATUS_PROPOSAL_STEP=""
STATUS_PLAN_STATE_SUMMARY=""
...
if command -v openlogos &>/dev/null; then
  STATUS_JSON=$(openlogos status --format json 2>/dev/null || echo "")
fi

if [ -n "$STATUS_JSON" ]; then
  if command -v python3 &>/dev/null; then
    STATUS_LIFECYCLE=$(echo "$STATUS_JSON" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('data',{}).get('lifecycle',''))" 2>/dev/null || echo "")
...
print('' if not isinstance(ps,dict) else 'plan_ready=%s, plan_gate_pending=%s, plan_approved=%s, tasks_execution=%s/%s %s' % (str(bool(ps.get('plan_ready'))).lower(), str(bool(ps.get('plan_gate_pending'))).lower(), str(bool(ps.get('plan_approved'))).lower(), ps.get('tasks_execution_done',0), ps.get('tasks_execution_total',0), ps.get('tasks_execution_scope','none')))" 2>/dev/null || echo "")
    ...
    STATUS_LIFECYCLE=$(echo "$STATUS_JSON" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const data=(JSON.parse(d).data)||{};console.log(data.lifecycle||'')}catch(e){console.log('')}})" 2>/dev/null || echo "")
    STATUS_ACTIVE_CHANGE=$(echo "$STATUS_JSON" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const data=(JSON.parse(d).data)||{};const mod=(data.modules||[]).find(m=>m.active_change&&m.active_change.slug);console.log(data.active_change||mod?.active_change?.slug||'')}catch(e){console.log('')}})" 2>/dev/null || echo "")
    STATUS_PROPOSAL_STEP=$(echo "$STATUS_JSON" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const data=(JSON.parse(d).data)||{};const mod=(data.modules||[]).find(m=>m.active_change&&m.active_change.proposal_step);console.log(data.proposal_step||mod?.active_change?.proposal_step||'')}catch(e){console.log('')}})" 2>/dev/null || echo "")
    STATUS_PLAN_STATE_SUMMARY=$(echo "$STATUS_JSON" | node -e "let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>{try{const data=(JSON.parse(d).data)||{};const active=data.active_change||'';let ps=data.plan_state;for(const m of data.modules||[]){const ac=m.active_change;if(!ac) continue;if(active&&ac.slug!==active) continue;ps=ps||ac.plan_state;break;}console.log(!ps?'':\`plan_ready=\${!!ps.plan_ready}, plan_gate_pending=\${!!ps.plan_gate_pending}, plan_approved=\${!!ps.plan_approved}, tasks_execution=\${ps.tasks_execution_done??0}/\${ps.tasks_execution_total??0} \${ps.tasks_execution_scope||'none'}\`)}catch(e){console.log('')}})" 2>/dev/null || echo "")
  fi
fi
...
if [ -f "$YAML_FILE" ]; then
  ...
  _skip_result=""
  if command -v python3 &>/dev/null; then
    _py_script=$(mktemp "${TMPDIR:-/tmp}/openlogos_skip.XXXXXX")
...
PYEOF
    ...
    _js_script=$(mktemp "${TMPDIR:-/tmp}/openlogos_skip.XXXXXX")
...
GUARD_STATUS=""
ACTIVE_CHANGE="$STATUS_ACTIVE_CHANGE"
PROPOSAL_STEP="$STATUS_PROPOSAL_STEP"

if [ "$LIFECYCLE" = "launched" ] && [ -z "$ACTIVE_CHANGE" ] && [ -f "$GUARD_FILE" ]; then
  if command -v python3 &>/dev/null; then
...
if [ "$LIFECYCLE" = "launched" ]; then
  if [ -n "$ACTIVE_CHANGE" ]; then
    ...
    CHANGE_MGMT=$(change_management_message "$ACTIVE_CHANGE" "${PROPOSAL_STEP:-}" "${STATUS_PLAN_STATE_SUMMARY:-}")
  else
