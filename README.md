...
Claude Code 用户可以通过插件获得最佳体验：12 个 Skills 自动发现、斜杠命令、Session 启动时自动检测 Phase。
...
安装后可用命令：
- `/openlogos:status` — 查看当前 Phase 进度
- `/openlogos:next` — 获取下一步操作建议和可用提示词
- `/openlogos:change <slug>` — 创建变更提案
- `/openlogos:launch` — 激活变更管理

> **注意**：不安装插件也可以使用 OpenLogos —— `openlogos init` 会自动生成 `CLAUDE.md` 作为兜底方案。
