...
| `scenario/` | Phase 3: HOW | API 编排测试用例（JSON） |
...
- `logos.config.json`：项目配置文件，定义文档模块的路径和匹配模式
- `logos-project.yaml`：AI 协作索引，为 AI 助手提供项目全局上下文
...
| 目录 | 用途 |
|------|------|
| `resources/image/` | 产品图片资源（截图、图标等） |
| `resources/context/` | 额外的 AI 上下文文件 |
