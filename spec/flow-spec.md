...
overlay:                         # 按 node id 寻址的操作列表（strategic-merge）
  ...
  - op: modify                   # 深合并：仅覆盖给出的字段
    target: code
    set: { review_agent: my-code-reviewer }
  - op: add                      # 新增节点
    after: code                  # after | before：相对某 node id 定位
    node: { id: lint, name: 静态检查, skill: linter, done_when: "marker:LINT_PASS" }
  - op: reorder                  # 调整顺序
    target: smoke
    after: deploy
```
...
| 命令 | 语义 |
|---|---|
| `openlogos status` | 按 flow 顺序派生每个 node 的 done/active/skipped/blocked；fan-out 输出覆盖度 |
| `openlogos next` | 输出当前 active 节点 + 其 skill + 现成提示词；遇 human gate 输出"需人类确认" |
| `openlogos next --auto` | auto 模式：`skippable:true` 的 gate 视为已通过并写 `GATE_AUTO_PASSED`；`false` 仍卡住 |
