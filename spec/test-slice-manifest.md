...
verify、status、next、宿主和 code-implementor 禁止写 manifest。宿主禁止写 checkpoint/marker 或从 `tasks.md` 建立影子清单。
...
设 `C` 为本提案在已合并测试规格中新增或修改的全部真实 UT/ST/SMOKE ID，`O` 为所有 `owned_test_ids` 的并集：
...
未被本提案修改的定义测试为基线回归 `B = D − C`，不得为了切片方便纳入 owned。一个测试跨能力线时仍须选择一个唯一 owning slice；无法唯一选择则 `test-slice-assignment-ambiguous`，不得猜测。
...
validator 必须在 runner 启动前完成：

| code | 条件 | 可自动恢复 |
|---|---|---|
| `test-slice-manifest-missing` | 多切片 implement 缺文件 | 是 |
| `test-slice-manifest-invalid` | v1 schema/字段/路径/selector 非法 | 是，前提是归属无歧义 |
| `test-slice-manifest-stale` | task/spec fingerprint 不匹配 | 是 |
...
| `test-slice-test-id-unknown` | owned ID 不在已合并规格 | 是 |
| `test-slice-assignment-ambiguous` | 无唯一 owning slice | 否 |
| `slice-task-state-inconsistent` | checkpoint 全过但 task 未完成等 | 否，修复任务事实 |

可自动恢复状态不写 `VERIFY_FAIL`、checkpoint 或 `LOOP_ITERS`。validator 输出全部 violation、精确字段路径和修复提示，结果排序稳定。

## 9. 恢复动作合同

missing/invalid/stale 时，OpenLogos 输出 `next_node.id=plan-slices`、`skill=slice-planner`、`dispatch.idempotent=true` 和 artifacts。slice-planner 恢复模式必须保留 `[code]` 文本/顺序/checkbox、`SLICES_APPROVED` 和 checkpoint，仅重建 manifest。完全缺失时从规范化 tasks 确定性恢复 slice ID；无法保持身份则阻塞。
